...
let async_rpc_server t (async_rpc_port : Async_rpc_port.t) =
  let where_to_listen =
    ...
    | Static port -> Tcp.Where_to_listen.of_port port
    | Dynamic _ -> Tcp.Where_to_listen.of_port_chosen_by_os
  in
  let%bind server =
...
let load_state_and_check_invariants =
  Command.async'
    ...
    let%map_open () = return ()
    and root_directory = anon ("ROOT_DIRECTORY" %: Filename_unix.arg_type) in
    fun () ->
        ...
        let%bind real_server =
          ...
          | None ->
            Iron_config.Restricted_for_rpcs.load_as_per_IRON_CONFIG
              ...
              ()
        in
        let port = Option.value listen_port ~default:(Host_and_port.port real_server) in
        let%bind server =
