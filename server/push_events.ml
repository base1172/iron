module Stable = struct
  ...
  module Properties = struct
    module V1 = struct
      type t = { by_rev_size : int } [@@deriving fields, sexp]
    end
...
module Properties = struct
  ...
  let invariant t =
    Invariant.invariant [%here] t [%sexp_of: t] (fun () ->
      ...
      Fields.iter ~by_rev_size:(check positive))
  ;;

  let default = { by_rev_size = 10 }
end
...
module Push_event = struct
  ...
  let create query ~feature_path () =
    { query; feature_path; used_by_metrics = Metric_name.Hash_set.create () }
  ;;
...
let invariant t =
  Invariant.invariant [%here] t [%sexp_of: t] (fun () ->
    let { push_events; properties; serializer = _ } = t in
    ...
    Hashtbl.iteri push_events ~f:(fun ~key:feature_id ~data:by_rev ->
      ...
      List.iter (Lru.to_alist by_rev) ~f:(fun (rev, query) ->
        Query.invariant
          (fun { Iron_protocol.Push_events.Add.Action.feature_id = feature_id'; tip } ->
            Node_hash.First_12.invariant rev;
            [%test_result: Feature_id.t] ~expect:feature_id feature_id';
            [%test_result: Node_hash.First_12.t] ~expect:rev (Rev.to_first_12 tip))
          query.query)))
;;
...
let dump_events_of_user t user_name =
  ...
  |> Map.filter_map ~f:(fun by_rev_map ->
       let by_rev_map =
         ...
         |> Map.filter ~f:(fun query -> User_name.equal user_name (Query.by query.query))
       in
       if Map.is_empty by_rev_map then None else Some by_rev_map)
...
let dump t (what_to_dump : Iron_protocol.Push_events.What_to_dump.t) =
  ...
  | `Values -> [%sexp (t.push_events : Push_event.t Lru.t Feature_id.Table.t)]
  | `Stats ->
...
let deserializer =
  Deserializer.with_serializer (fun serializer ->
    ...
    in
    { push_events = Feature_id.Table.create (); properties; serializer })
;;

let add t query ~feature_path =
  let { Iron_protocol.Push_events.Add.Action.feature_id; tip } = Query.action query in
  let by_rev =
    Hashtbl.find_or_add t.push_events feature_id ~default:(fun () ->
      Lru.create ~max_size:t.properties.by_rev_size)
  in
...
let change_by_rev_size t by_rev_size =
  if by_rev_size <= 0 then raise_s [%sexp "invalid by_rev_size", (by_rev_size : int)];
...
let change t (update : Iron_protocol.Push_events.Change.Action.t) =
  ...
  | Clear_features feature_ids -> List.iter feature_ids ~f:(Hashtbl.remove t.push_events)
  | Clear_revs revs ->
  ...
  | Clear_users users ->
    ...
    Hashtbl.filter_inplace t.push_events ~f:(fun by_rev ->
      ...
      |> List.filter_map ~f:(fun (rev, event) ->
           if User_name.Set.mem users (Push_event.by event) then Some rev else None)
      |> List.iter ~f:(fun rev -> ignore (Lru.remove by_rev rev : [ `Ok | `No_such_key ]));
      not (Lru.is_empty by_rev))
;;
