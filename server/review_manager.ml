module Stable = struct
  ...
  module Cr_comments = struct
    module V1 = struct
      type t = Cr_comment.V1.t list Or_error.V1.t [@@deriving sexp]
    end
  end
end
...
module Persist = struct
  ...
  module Current_session_id = struct
    module V1 = struct
      type t = Session_id.Stable.V1.t option [@@deriving sexp]
    end
  ...
  let invariant t =
    Invariant.invariant [%here] t [%sexp_of: t] (fun () ->
      ...
      Fields.Direct.iter
        ...
        ~at_review_goal:
          (check
             (Option.invariant (fun review_goal ->
             ...
             reviewer, so we can just pick one. *)
                ...
                let missing_diffs =
                  diff4s_to_goal_internal
                    { t with at_review_goal = None }
                    review_goal
                    ~reviewer
                    ~indexed_diff4s
                in
...
let invariant t =
  Invariant.invariant [%here] t [%sexp_of: t] (fun () ->
    ...
    Fields.iter
      ...
      ~num_crs:
        (check (Or_error.invariant ([%test_pred: int] (fun num_crs -> num_crs >= 0))))
      ~num_xcrs:
        (check (Or_error.invariant ([%test_pred: int] (fun num_crs -> num_crs >= 0))))
      ~current_session:
        (check
           (Option.iter ~f:(fun review_session ->
...
let review_session_dir review_session_id =
  Relpath.of_list
    [ File_name.of_string "review-sessions"; Session_id.to_file_name review_session_id ]
;;
...
let persist_brain t =
  Serializer.set_contents
    ...
    t.brain.brain
    ~file:brain_file
    (module Persist.Brain_file)
;;

let persist_crs t =
  Serializer.set_contents t.serializer ~file:crs_file t.crs (module Persist.Cr_comments)
;;
...
let diff4s_from_brain_to_goal t ~(brain : Brain_with_goal.t) =
  ...
  | _, ((Pending_since _ | Known (Error _)) as z) -> z
  | Known (Ok review_goal), Known (Ok indexed_diff4s) ->
    let diff4s =
      Brain_with_goal.diff4s_to_goal_internal
        brain
        review_goal
        ~indexed_diff4s
        ~reviewer:(reviewer t)
...
let clear_current_session t = if is_some t.current_session then set_current_session t None
      ...
      (match diff4s with
       ...
       | _ :: _ ->
         ...
         let has_implicit_actual_diffs =
           ...
           List.exists implicitly_reviewed_diff4s ~f:(fun d ->
             not (Diff4.is_rev_update d.diff4))
         in
         ...
         then brain
           ...
           let at_review_goal =
             if all_diff4s_are_implicitly_reviewed then Some review_goal else None
           in
           let brain =
             Brain.extend
               brain.brain
               ~with_:implicitly_reviewed_diff4s
               ~reviewer
               ~mark_kind:User
           in
...
let set_crs t crs ~persist =
  ...
  match [%compare: Cr_comment.Structurally_compared.t list Or_error.t] crs t.crs with
  | 0 -> ()
  | _ ->
    t.crs <- crs;
...
let update_crs t crs ~(base_facts : Rev_facts.t Or_pending.t) =
  let crs =
    ...
    then Ok []
      ...
      | Known base_facts ->
        (match
           Rev_facts.Is_cr_clean.check base_facts.is_cr_clean (Rev_facts.rev base_facts)
         with
         | Error e -> error "Iron bug -- please report" e [%sexp_of: Error.t]
         | Ok base_is_cr_clean ->
           if base_is_cr_clean then Ok crs else error_string "base is not CR clean"))
  in
    ...
    (match t.review_goal with
     ...
     | Known (Ok goal) ->
       ...
       if must_use_it
          ...
          || (Rev.equal_node_hash (Review_session.tip session) (Review_goal.tip_rev goal)
             && Rev.equal_node_hash
...
let forget_from_brain_internal_exn t ~what_to_forget =
  (match t.current_session with
   ...
   | Some session ->
     ...
     then
       failwith
         "cannot forget -- some files are marked as reviewed in the current review \
          session");
  let brain =
    ...
    | `Files paths_in_repo ->
      ...
      let unused_files_to_forget =
        List.filter_map (Hashtbl.to_alist files_to_forget) ~f:(fun (path_in_repo, used) ->
          if !used then None else Some path_in_repo)
      in
      if not (List.is_empty unused_files_to_forget)
      then
        raise_s
          [%sexp
            "there is no diff in your brain for"
...
let register_catch_up t query review_session = function
  | [] -> Ok ()
  | _ :: _ as ids_and_kinds -> t.register_catch_up.f query review_session ids_and_kinds
;;
...
let reviewed_diff4s_output_in_current_session t =
  ...
  | Some review_session ->
    List.map
      (Review_session.reviewed_diff4s review_session)
...
let have_potentially_blocking_review_session_in_progress t =
  ...
  || List.exists (reviewed_diff4s_output_in_current_session t) ~f:is_potentially_blocking
;;
...
module Review_lines_to_goal_via_session = struct
  ...
  let review_lines
    ({ to_finish_session
     ; line_count_to_goal = { from_session_end; from_brain_if_session_was_committed = _ }
     } :
      Review_lines_of_diff4.t list t)
    =
    let to_finish_session = to_finish_session |> Review_lines_of_diff4.sum_review_lines in
    ...
    | Error from_session_end_to_goal ->
      To_goal_via_session.Partially_known { to_finish_session; from_session_end_to_goal }
  ;;
end
...
let compute_review_lines_of_diff4 t brain =
  ...
  in
  let f
    (goal_subset : Goal_subset.t)
    ({ diff4; output_num_lines } : Diff4.And_output_num_lines.t)
    ~reviewer
    =
        ...
        if match goal_subset with
           ...
           | Entire_goal_but_empty_if_satisfied_by review_analysis ->
             ...
             && (Diff4.is_forget diff4
                || Review_analysis
                   .non_wfr_obligations_will_be_satisfied_once_expected_users_have_read
                     review_analysis
...
let review_lines_to_goal_via_session t (goal_subset : Goal_subset.t) =
  let to_finish_session, brain_if_session_was_finished, brain_if_session_was_committed =
    match what_to_do_with_session t ~which_session:Current_session with
  ...
  let from_brain_to_goal (brain : Brain_with_goal.t) =
    let compute_lines = Staged.unstage (compute_review_lines_of_diff4 t brain.brain) in
  ...
  in
  { Review_lines_to_goal_via_session.to_finish_session
...
let line_count_cached_in_feature t goal_subset =
  { Line_count.Cached_in_feature.review = line_count_remaining_to_review t goal_subset
...
let review_session_to_protocol
  t
  session
  goal_subset
  ~may_be_reviewed_by
  ...
  let diff4s_in_session =
    Review_session.diff4s_and_review_kind_in_session_not_implicitly_reviewed
      session
...
let check_cr_clean t =
  try
    ...
    Ok ()
  with
  | exn -> error "not CR clean" (t.user_name, exn) [%sexp_of: User_name.t * exn]
;;

module Review_authorization = struct
  module Reviewed_by_themselves = struct
    type t = { create_catch_up_for_me : bool } [@@deriving sexp_of]
  end
    ...
    Or_error.try_with (fun () ->
      ...
      then
        raise_s
          [%sexp
            (sprintf
               "cannot use switch [%s] when reviewing for someone else"
  ...
  let unauthorized_for_user_with_only_follow_lines = function
    | Reviewed_by_someone_else t ->
      ...
      then
        Or_error.error
          "unauthorized review for a user with only lines to follow"
          t.reviewed_for
          [%sexp_of: User_name.t]
      else Ok ()
    | Reviewed_by_themselves _ | Reviewed__no_catch_up_as_per_user_info -> Ok ()
  ;;
...
let reviewed
  ...
  | Some session ->
    ...
    in
    (match
       Review_session.reviewed
         session
         query
         review_session_id
         diff4_in_session_ids
         ~compute_review_kind
         ~is_using_locked_sessions:t.is_using_locked_sessions
         ~even_if_some_files_are_already_reviewed
     with
...
let forget_from_current_session_exn
  t
  review_authorization
  query
  session_id
    ...
    (match what_to_forget with
     ...
     | `Files paths_in_repo ->
       ...
       let diff4_in_session_ids =
         paths_in_repo |> List.filter_map ~f:(Hashtbl.find ids_by_path) |> List.concat
       in
       ok_exn
         (Review_session.unreviewed current_session query session_id diff4_in_session_ids));
    maybe_advance_brain t
;;

let update_review_goal
  t
  ~review_goal
  ~indexed_diff4s
  ~is_whole_feature_follower
...
let commit_session_then_create_catch_up_to_goal_exn__allow_all t query ~reason =
  ...
  | (`Release | `Review_authorization _) as reason ->
         ...
         | `Review_authorization review_authorization ->
           (match Review_authorization.should_register_catch_up review_authorization with
            ...
            | Some kind ->
              ...
              |> Array.to_list
              |> List.map ~f:(fun id -> { Catch_up_session.Id_and_kind.id; kind }))
         | `Release ->
           ...
           |> List.filter_map ~f:(fun diff4_in_session ->
                ...
                |> Option.map ~f:(fun kind ->
                     { Catch_up_session.Id_and_kind.id =
                         Review_session.Diff4_in_session.id diff4_in_session
                     ; kind
                     }))
       in
...
let mark_fully_reviewed t query reason =
  ...
  set_brain
    t
    { Brain_with_goal.brain =
        Brain.extend t.brain.brain ~with_:diff4s ~reviewer:(reviewer t) ~mark_kind
...
let need_diff4s_starting_from t =
  ...
  Review_edge.Set.union_list
    [ for_from_brain_to_goal; for_from_session_end_to_goal; for_from_goal_to_next_goal ]
;;
...
let de_alias_brain t user_name_by_alias =
  ...
  then `Nothing_to_do
  else if have_session_in_progress t
  ...
  Deserializer.with_serializer (fun serializer ->
    ...
    and current_session =
      ...
      | Some review_session_id ->
        ...
        >>| Option.return
    and brain = one (module Persist.Brain_file) ~in_file:brain_file in
    fun ~whole_feature_followers
        ...
        ~dynamic_upgrade_state ->
      Serializer.add_cache_invalidator serializer feature_cache_invalidator;
      let is_whole_feature_follower = Set.mem whole_feature_followers user_name in
      let is_whole_feature_reviewer = Set.mem whole_feature_reviewers user_name in
      let is_using_locked_sessions = Set.mem users_using_locked_sessions user_name in
      let review_goal =
        Or_error.map review_goal ~f:(fun review_goal ->
          Review_goal.restrict_to_may_review_or_follow
            review_goal
            (Reviewer.create
               user_name
               ~is_whole_feature_follower
