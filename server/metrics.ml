...
let invariant t =
  Invariant.invariant [%here] t [%sexp_of: t] (fun () ->
    let { forest; metric_name_subscriptions; feature_path_subscriptions } = t in
...
let clear (t : t) { Iron_protocol.Metrics.Clear.Action.metric_name; which_features } =
...
let get (t : t) ({ descendants_of } : Iron_protocol.Metrics.Get.Action.t) =
  ...
  |> Map.map ~f:(fun metrics ->
       metrics |> Metric_name.Map.of_hashtbl_exn |> Map.map ~f:Metric.data_points)
;;

let complete t ~prefix =
  let add_if_prefix set metric_name =
    let name = Metric_name.to_string metric_name in
    if String.is_prefix ~prefix name then String.Set.add set name else set
  in
...
let add_values
  t
  { Iron_protocol.Metrics.Add_values.Action.feature_path; metric_name; values }
  =
  if List.is_empty values
  then
    raise_s
      [%sexp
        "Metrics.add_values: empty values list"
        , [%here]
        , { feature_path : Feature_path.t; metric_name : Metric_name.t }];
  let metric =
  ...
  List.iter values ~f:(fun value ->
    ...
    let event = { Event.feature_path; metric_name; value; added_at } in
...
let subscribe t query action =
  ...
  | Feature_path feature_path ->
    Feature_path_subscriptions.add
      t.feature_path_subscriptions
      query
      feature_path
      ~initial:None
  | Metric_name metric_name ->
    Metric_name_subscriptions.add
      t.metric_name_subscriptions
      query
      metric_name
      ~initial:None
;;
