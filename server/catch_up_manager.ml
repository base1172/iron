...
module T = struct
  ...
  module Container = Container.Make (struct
    ...
    let length = `Custom (fun t -> Heap.length t.by_creation_time)
  ...
  let invariant id invariant_a t =
    Invariant.invariant [%here] t [%sexp_of: _ t] (fun () ->
      Fields.iter ~by_id:ignore ~by_creation_time:ignore;
      ...
      Hashtbl.iteri t.by_id ~f:(fun ~key:id ~data:elt ->
        ...
        | None -> raise_s [%sexp "session missing from the heap", (id : Session_id.t)]))
  ;;

  let mem t catch_up_session = Hashtbl.mem t.by_id (Catch_up_session.id catch_up_session)
...
let invariant t = invariant Catch_up_session.id Catch_up_session.invariant t
...
let remove_all_links t ~f =
  ...
  List.iter to_remove ~f:(fun session -> remove_link t (Catch_up_session.id session));
  to_remove
;;
...
let find_all_for_feature_id t feature_id = find_all t ~f:(has_feature_id feature_id)
