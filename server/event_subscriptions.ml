...
module Subscription_t = struct
  ...
  and 'a unpacked =
    ...
    ; rpc_version : int
    ; query : 'a Query.t
    ; closed : [ `Removed | `Dropped of Error.t ] Ivar.t
    ; opened_at : Time.t
    ; mutable ticks : int
    }
  [@@deriving fields]
  ...
  let sexp_of_t (T (t, sexp_of_action)) =
    let { rpc_name; rpc_version; query; closed = _; opened_at; ticks } = t in
  ...
  let closed (T (t, _)) = Ivar.read t.closed
  let tick (T (t, _)) = t.ticks <- t.ticks + 1
  let by (T (t, _)) = Query.by t.query
  let close (T (t, _)) value = Ivar.fill_if_empty t.closed value
end
...
module Properties = struct
  ...
  let default = { max_subscriptions_per_user = 50; max_subscriptions_global = 500 }
end
...
let add t ~rpc_name ~rpc_version query ~sexp_of_action =
  ...
  in
  if user_subscription_count >= t.properties.max_subscriptions_per_user
  then
    Or_error.errorf
      "user %s exceeded the per-user feature update subscription limit (%d)"
      (User_name.to_string by)
      t.properties.max_subscriptions_per_user
  else if t.subscription_count_global >= t.properties.max_subscriptions_global
  then
    Or_error.errorf
...
let check_non_negative_exn max =
  if max < 0 then raise_s [%sexp "subscription limits must be non-negative", (max : int)]
;;
...
let set_max_subscriptions_per_user t max =
  ...
  if max <> t.properties.max_subscriptions_per_user
  then set_and_persist_properties t { t.properties with max_subscriptions_per_user = max }
;;

let set_max_subscriptions_global t max =
  check_non_negative_exn max;
  if max <> t.properties.max_subscriptions_global
  then set_and_persist_properties t { t.properties with max_subscriptions_global = max }
;;
...
let drop_all_by_user t query by_whom =
  ...
  let drop_msg =
    lazy
      (Error.create
         ...
         query
         [%sexp_of: Iron_protocol.With_event_subscriptions.Action.t Query.t])
  in
  ...
  then raise_s [%sexp "no subscriptions to drop for", (by_whom : User_name.Or_all.t)]
;;
