...
module By_machine_name = struct
  type t = Unclean_workspace_reason.t Machine_name.Table.t [@@deriving sexp_of]
...
let indexed_by_machine_name by_feature_path =
  ...
  Hashtbl.to_alist all_by_machine_name |> Machine_name.Map.of_alist_exn
;;
...
let dump t what_users =
  ...
  |> [%sexp_of:
       (User_name.t * Unclean_workspace.t list Machine_name.Map.t * unit Query.t option)
       list]
;;
...
let user_subtree user = Relpath.of_list [ File_name.of_string (User_name.to_string user) ]
...
module Persist = struct
  module User_info = struct
    ...
    include
      Register_read_write_old_version
        (struct
          let version = 1
        end)
        (struct
          ...
          let should_write_this_version t =
            match
              Dynamic_upgrade.commit_to_upgrade t.dynamic_upgrade_state ~allowed_from:U2
            with
...
let persist_user t query user_name =
  ...
  let user_info = { User_info.machines = find_user t user_name; query } in
  ...
  Serializer.set_contents
    ...
    (t, user_info)
    (module Persist.User_info.Writer)
;;
...
let deserializer ~dynamic_upgrade_state =
  Deserializer.with_serializer (fun serializer ->
    ...
    and users =
      all_subdirs
        (one
           (module Persist.User_info.Reader)
    ...
    then
      raise_s
        [%sexp
          ...
          , { user_name : User_name.t; machine_name : Machine_name.t }];
...
let update
  t
  query
  { Iron_protocol.Update_unclean_workspaces.Action.for_ = user_name
  ; machine_name
  ; unclean_workspaces
  ; clean_workspaces
  }
     ...
     (match clean_workspaces with
      | `At_least_these feature_paths ->
        Set.iter (Feature_path.Set.of_list feature_paths) ~f:(fun feature_path ->
          Option.iter
            (Hashtbl.find by_feature_path feature_path)
