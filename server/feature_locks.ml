...
module Locked : sig
  ...
  let create ~by ~reason ~is_permanent ~query =
    { query = Query.with_action query (); by; reason; is_permanent }
  ;;
  ...
  let to_protocol { by; reason; query; is_permanent } : Iron_protocol.Feature.Locked.t =
    { by; at = Query.at query; reason; is_permanent }
  ;;
end
...
let create feature_path = { feature_path; lock_by_name = Lock_name.Table.create () }
let set_feature_path t feature_path = t.feature_path <- feature_path
...
let invariant t =
  Invariant.invariant [%here] t [%sexp_of: t] (fun () ->
    ...
    Fields.iter
      ...
      ~lock_by_name:
        (check
           (Hashtbl.iteri ~f:(fun ~key:lock_name ~data:locks ->
...
let what_is_locked t =
  ...
  |> List.sort ~compare:[%compare: Lock_name.t * Locked.Compare_by_user.t list]
...
let lock t ~query ~for_ ~lock_name ~reason ~is_permanent =
  ...
  let locks =
    locked :: List.filter locks ~f:(fun locked -> not (Locked.is_locked_by locked for_))
  in
  ...
  | Some locks ->
    With_return.with_return (fun return ->
      ...
      let locks =
        List.filter locks ~f:(fun locked ->
          ...
          then (
            ...
            if Locked.is_permanent locked && not even_if_permanent
            then
              Or_error.error
                (sprintf
                   "This lock is permanent -- consider using %s"
      ...
      then (
        ...
        let message =
          sprintf
            !"not locked for %s by %{User_name}"
            (Lock_name.to_string_hum lock_name)
            for_
        in
        error_s [%sexp (message : string), { locks : Locked.t list }]))
;;
