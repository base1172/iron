...
let log ~level sexp = Log.Global.sexp ~level sexp
...
type t = { cache : Bigstring.t Or_error.t Description.Table.t } [@@deriving sexp_of]

let create () = { cache = Description.Table.create () }
...
let handle_rpc t connection ~rpc_tag ~version responder buf ~pos ~len =
  ...
  let should_cache =
    cache_rpcs_of description && not (Hashtbl.mem response_by_description description)
  in
  let cache data =
    ...
    Hashtbl.update response_by_description description ~f:(fun previous ->
      ...
      | _, Ok _ | (None | Some (Error _)), Error _ ->
        log
          ~level:`Info
          [%sexp
            "caching RPC response", { description : Description.t; data : _ Or_error.t }];
        data)
  in
  let handle_response buf ~pos ~len =
    log ~level:`Debug [%sexp "proxying RPC response", { rpc_tag : string; version : int }];
  ...
  | None ->
    log ~level:`Debug [%sexp "proxying RPC query", { rpc_tag : string; version : int }];
...
let simple_server ~where_to_listen ~real_server =
  let durable_connection =
    Async_durable.create
      ~to_create:(fun () ->
        ...
        let where_to_connect = Tcp.Where_to_connect.of_host_and_port real_server in
        ...
        | Error exn ->
          ...
          log
            ...
            [%sexp
              ...
              , { server = (real_server : Host_and_port.t); error : Error.t }];
          Error error)
  ...
  let handle_rpc () ~rpc_tag ~version responder buf ~pos ~len =
    Async_durable.with_ durable_connection ~f:(fun (proxy, connection) ->
      return
