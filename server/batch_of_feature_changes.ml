...
let enqueue_feature_updates_for_inheritable_attributes
  ~changes
  ~feature
  ...
  in
  if not (List.is_empty owners) then add (`Add_inheritable_owners owners);
  if not (Map.is_empty properties) then add (`Set_inheritable_properties properties);
...
let to_feature_updates
  ...
  let add change = Queue.enqueue changes change in
  enqueue_feature_updates_for_inheritable_attributes
    ~changes
    ~feature
    add_inheritable_attributes;
  Option.iter
  ...
  if not (Map.is_empty add_properties) then add (`Set_properties add_properties);
  Option.iter set_release_process ~f:(fun release_process ->
    add (`Set_release_process release_process));
  Option.iter set_who_can_release_into_me ~f:(fun who_can_release_into_me ->
    add (`Set_who_can_release_into_me who_can_release_into_me));
  let send_email_to = Set.diff add_send_email_to (Feature.send_email_to feature) in
  if not (Set.is_empty send_email_to) then add (`Add_send_email_to send_email_to);
  let send_email_upon = Set.diff add_send_email_upon (Feature.send_email_upon feature) in
  if not (Set.is_empty send_email_upon) then add (`Add_send_email_upon send_email_upon);
  let whole_feature_followers =
...
let add_send_email_to t ~send_email_to =
  { t with add_send_email_to = Set.union t.add_send_email_to send_email_to }
;;

let add_send_email_upon t ~send_email_upon =
  { t with add_send_email_upon = Set.union t.add_send_email_upon send_email_upon }
;;
...
let add_inheritable_attributes
  t
  ...
  { t with
    add_inheritable_attributes =
      ...
      ; release_process = Option.first_some release_process attrs.release_process
...
let add_inherited_from_parent t ~parent_inheritable_attributes =
  let ({ Inheritable_attributes.crs_shown_in_todo_only_for_users_reviewing
       ; xcrs_shown_in_todo_only_for_users_reviewing
       ; owners
       ; properties
       ; release_process
