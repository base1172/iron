...
module Whole_feature_reviewer_analysis = struct
  type t = { obligations_are_satisfied : bool } [@@deriving compare, fields, sexp_of]

  let invariant t =
    Invariant.invariant [%here] t [%sexp_of: t] (fun () ->
      Fields.iter ~obligations_are_satisfied:ignore)
  ;;
end
...
let invariant t =
  Invariant.invariant [%here] t [%sexp_of: t] (fun () ->
    ...
    Fields.iter
      ~diff2s:
        (check
           (Map.iteri ~f:(fun ~key:diff2 ~data ->
      ...
      ~whole_feature_reviewers:
        (check
           (Map.iteri ~f:(fun ~key:user_name ~data ->
...
let obligations_are_satisfied t =
  ...
  Fields.for_all
    ~diff2s:
      (check
         (Map.for_all ~f:(fun (diff2 : Diff2_analysis.t) ->
