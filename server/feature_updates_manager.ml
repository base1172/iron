...
module Invalidated_features = struct
  ...
  let add t feature =
    let invalidated_feature =
      Hashtbl.find_or_add t (Feature.feature_id feature) ~default:(fun () ->
        Invalidated_feature.create feature)
    in
...
type t =
  ...
  ; invalidated_features : (Invalidated_features.t[@sexp.opaque])
...
let broadcast_updates t =
  ...
  Hash_set.iter feature_paths_to_tick_with_updates_in_subtree ~f:(fun feature_path ->
    Path_subscriptions.tick
      t.feature_and_descendants_subscriptions
      feature_path
      `Updates_in_subtree);
...
let subscribe_feature_only t query feature_id ~when_to_first_notify =
  Id_subscriptions.add
    t.feature_only_subscriptions
    query
    feature_id
    ~initial:
      (match (when_to_first_notify : When_to_first_notify.t) with
       | Now -> Some `Updated
       | At_next_change -> None)
;;

let subscribe_feature_and_descendants t query feature_path ~when_to_first_notify =
  Path_subscriptions.add
    t.feature_and_descendants_subscriptions
    query
    feature_path
    ~initial:
...
let on_archive t feature =
  on_update t feature;
  Id_subscriptions.close
    t.feature_only_subscriptions
    (Feature.feature_id feature)
    ~reason:`Archived;
  Path_subscriptions.close
    t.feature_and_descendants_subscriptions
