...
module User_sets = struct
  ...
  let invariant t =
    Invariant.invariant [%here] t [%sexp_of: t] (fun () ->
      ...
      Fields.iter ~admins:check ~feeding_metrics:check ~using_locked_sessions:check)
  ;;
end
...
let check_no_intersections_exn t =
  ...
  let all_errors =
    ...
    |> Hashtbl.to_alist
    |> List.sort ~compare:(fun (user1, _) (user2, _) -> String.compare user1 user2)
  in
...
let invariant t =
  Invariant.invariant [%here] t [%sexp_of: t] (fun () ->
    ...
    Fields.iter
      ~existing_users:
        (check (fun existing_users -> Hash_set.iter existing_users ~f:User_name.invariant))
      ~valid_users:
        (check (fun valid_users -> Hash_set.iter valid_users ~f:User_name.invariant))
...
let persist_existing_users t =
  Serializer.set_contents
    t.serializer
    ~file:existing_users_file
    (t.existing_users |> Hash_set.to_list |> User_name.Set.of_list)
...
let persist_valid_users t =
  Serializer.set_contents
    t.serializer
    ~file:valid_users_file
    (t.valid_users |> Hash_set.to_list |> User_name.Set.of_list)
    (module Persist.User_names_set)
;;
...
let persist_aliases t =
  Serializer.set_contents
    ...
    ~file:aliases_file
    t.aliases_seen
    (module Persist.Alternate_names)
;;
...
let dump t (which_user_info : Iron_protocol.Dump.Which_user_info.t) =
  ...
  | Existing_users ->
    List.sort (Hash_set.to_list t.existing_users) ~compare:User_name.compare
    |> [%sexp_of: User_name.t list]
  | Valid_users ->
    List.sort (Hash_set.to_list t.valid_users) ~compare:User_name.compare
    |> [%sexp_of: User_name.t list]
...
let complete t ~prefix (which : Iron_protocol.Complete.Which_user_info.t) =
  ...
  (match which with
   | Existing_user ->
     Hash_set.iter t.existing_users ~f:(fun u -> maybe_add User_name.to_string u)
   | Typo ->
     User_name_by_alternate_name.iteri t.typos ~f:(fun ~key:u ~data:_ ->
       maybe_add Alternate_name.to_string u)
   | Alias ->
     User_name_by_alternate_name.iteri t.aliases_seen ~f:(fun ~key:u ~data:_ ->
       maybe_add Alternate_name.to_string u));
  !completions
;;
...
let refresh_existing_users t ~occurrences_by_user_name =
  let existing_users =
    User_name.Hash_set.of_list
      (Hashtbl.keys occurrences_by_user_name @ Hash_set.to_list t.valid_users)
  in
  let invalid_users =
    Hashtbl.filteri occurrences_by_user_name ~f:(fun ~key:user_name ~data:_ ->
...
module User_set = struct
  ...
  module Make (X : sig
    ...
    let persist t =
      Serializer.set_contents
        t.serializer
        ~file:persisted_in_file
        (t |> get_field |> Hash_set.to_list |> User_name.Set.of_list)
        (module Persist.User_names_set)
    ;;

    let maybe_s set = if Set.length set > 1 then "s" else ""
...
let alternate_names t ~which =
  ...
  | `Typos -> t.typos
  | `All ->
    User_name_by_alternate_name.merge_exn t.aliases_seen t.typos ~on_error:(fun _ _ ->
      check_no_intersections_exn t;
      assert false)
;;
...
let remove_alternate_names_exn t alternate_names_to_remove ~which =
  ...
  let unremoved, alternate_names =
    List.fold_left
      alternate_names_to_remove
      ~init:([], alternate_names)
  ...
  | _ :: _ ->
    raise_s
      [%sexp
        (sprintf "The following %s were not removed: " name : string)
...
let typo_conflict typo means =
  raise_s
    [%sexp
      ...
      , { typo : Alternate_name.t; means : User_name.t list }]
;;
...
let alias_conflict alias user_names =
  raise_s
    [%sexp
      ...
      , { alias : Alternate_name.t; user_names : User_name.t list }]
;;
...
let define_typos_exn t (definitions : Iron_protocol.Define_typos.Definition.t list) =
  let typos =
    List.fold_left definitions ~init:t.typos ~f:(fun typos { typo; means } ->
      ...
      let typo_as_unresolved =
        Unresolved_name.of_string (Alternate_name.to_string typo)
      in
      ...
      (match
       ...
       | Some not_alias ->
         failwithf
           !"%{Alternate_name} is already an alias for %{User_name}"
           typo
           not_alias
           ());
      User_name_by_alternate_name.add_exn
        typos
        ~alternate_name:typo
        ~user_name:means
...
let deserializer =
  Deserializer.with_serializer (fun serializer ->
    ...
    and user_sets =
      ...
      and using_locked_sessions = Using_locked_sessions.deserializer in
      { User_sets.admins; feeding_metrics; using_locked_sessions }
    in
