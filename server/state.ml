    ...
    (fun t query ->
      ...
      let relative_feature_path_completion_types, other_completions =
        List.partition_map types ~f:(function
          ...
          | Archived_feature_path -> `Fst `Archived
          | Metric_name -> `Snd (Metrics.complete t.metrics ~prefix)
          | Remote_repo_path ->
            `Snd
          ...
          | User_info which -> `Snd (User_info.complete t.user_info ~prefix which))
      in
...
let () =
  ...
  implement_rpc
    ...
    (fun _t query ->
      ...
      | Process_times -> Unix.times () |> [%sexp_of: Core.Unix.process_times])
;;
