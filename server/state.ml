...
module Bookmarks_without_feature = struct
  ...
  type t = Bookmark_without_feature.t list Table.t [@@deriving sexp_of]
  ...
  *)
  ...
  Fields.iter
    ...
    ~bookmarks_without_feature:
      (check (fun bookmarks_without_feature ->
         (* In theory, for:
  ...
  module Change_fully_reviewed_revisions_query = struct
    include
      Persistent.Make
        ...
        (struct
          type t =
            Iron_protocol.Change_fully_reviewed_revisions.Stable.What_to_do.V1.t
            Query.Stable.V1.t
          [@@deriving sexp]
        end)
  end
...
let archived_feature_dir archived_feature =
  Relpath.append archived_features_dir (Archived_features.feature_dir archived_feature)
;;
  ...
  then (
    ...
    | Some serializer ->
      Serializer.append_to
        serializer
        fully_reviewed_edge
        ~file:fully_reviewed_edges_file
...
let set_feature_parties t feature =
  List.iter Features_by_parties.Parties.all ~f:(fun parties ->
    Features_by_parties.set_users
      t.features_by_parties
...
let iter_review_managers_of_feature t ?of_:(whose_review_managers = `All_users) feature ~f
  =
  match whose_review_managers with
  | `User user -> f user (ok_exn (find_review_manager t feature user))
  | `All_users -> iter_review_managers_of_feature t feature ~f
  | `All_users_but users_to_skip ->
    iter_review_managers_of_feature t feature ~f:(fun user_name review_manager ->
      if not (Set.mem users_to_skip user_name) then f user_name review_manager)
;;
...
let remote_repo_path ?rev_zero t feature =
  ...
  let remote_repo_path =
    ...
    | None ->
      raise_s
        [%sexp
          "Iron bug: root feature missing remote_repo_path"
...
let feature_crs t feature =
  ...
  | true ->
    with_return (fun r ->
      let crs = ref [] in
      Hashtbl2_pair.find1_iter2 t.review_managers feature_id ~f:(fun _ review_manager ->
...
let feature_cr_summary t feature =
  ...
  Or_error.map (feature_crs t feature) ~f:(fun crs ->
    Cr_comment.Summary.create
      crs
      ~feature_owner:(Feature.first_owner feature)
      ~alternate_names)
;;
...
let find_catch_up_manager t feature_path user_name =
  ...
  | None ->
    ...
    Hashtbl2_pair.add_exn t.catch_up_managers feature_path user_name catch_up_manager;
    catch_up_manager
;;
...
let catch_up_in_session
  ...
  for_
  =
  let result = Catch_up_session.catch_up catch_up_session query diff4_in_session_ids in
  ...
  then (
    ...
    then (
      ...
      if not (Hashtbl2_pair.mem2 t.catch_up_managers for_)
      then Serializer.remove_subtree (serializer_exn t) ~dir:(catch_up_user_dir for_)));
  ok_exn result
;;

let make_register_catch_up t feature for_ =
  { Review_manager.Register_catch_up.f =
      (fun query review_session ids_with_reason ->
        let feature_path = Feature.feature_path feature in
...
module Feature_email_sent_upon = struct
  ...
  let%test "Send_email_upon.t round trip via t" =
    List.for_all Send_email_upon.all ~f:(fun t ->
      [%compare.equal: Send_email_upon.t]
        t
  ...
  let%test "we need two types" =
    ...
    List.exists ts ~f:Option.is_some && List.exists ts ~f:Option.is_none
  ;;
end

let feature_email feature ~(sent_upon : Feature_email_sent_upon.t) =
  let users us =
    us |> List.map ~f:Email_address.of_user_name |> Email_address.Set.of_list
  in
...
let seconding_allowed feature ~by ~even_though_empty ~even_though_owner ~even_if_locked =
  try
    ...
    Ok ()
  with
  | exn -> error "cannot second" exn [%sexp_of: exn]
;;

let some_whole_feature_reviewer_can_make_progress t feature =
  Set.exists (Feature.whole_feature_reviewers feature) ~f:(fun user ->
    match find_review_manager t feature user with
    | Error _ -> false
    | Ok review_manager -> Review_manager.can_make_progress review_manager Entire_goal)
;;
...
let seconding_is_recommended t ~for_ feature ~even_if_locked =
  match
    seconding_allowed
      feature
      ~by:for_
      ~even_though_empty:false
      ~even_though_owner:false
      ~even_if_locked
  with
  | Error _ -> false
  | Ok () ->
...
let build_catch_up_lines_by_user t feature =
  ...
  let feature_path = Feature.feature_path feature in
  iter_catch_up_managers_of_feature_path
    t
    feature_path
...
module Completed_review = struct
  ...
  and t = data User_name.Table.t [@@deriving sexp_of]
  ...
  | Known (Ok diff2s) ->
      ...
      Hashtbl.mapi completed_review_by_diff2 ~f:(fun ~key:diff2 ~data:completed_review ->
        let files_obligations_analysis completed_review =
          let completed_review_satisfies_files_obligations_in f =
            ...
            | `Absent -> true
            | `Present attributes ->
              Review_obligation.is_satisfied
                attributes.review_obligation
                ~by:completed_review
          in
          { Review_analysis.Attributed_files_analysis.completed_review
        ...
        in
        { Review_analysis.Diff2_analysis.actually_reviewed =
    ...
    let whole_feature_reviewers =
      ...
      |> Set.to_map ~f:(fun reviewer ->
           { Review_analysis.Whole_feature_reviewer_analysis.obligations_are_satisfied =
    ...
    in
    Some { diff2s; users_with_review_session_in_progress; whole_feature_reviewers }
;;
...
let compute_line_count_by_user_exn t feature =
  ...
  let line_count_by_user =
    ...
    |> List.map
         ...
         in
         ( user
         , { Line_count.review = To_goal_via_session.fully_known_exn review
           ; catch_up
           ; completed
           ; have_potentially_blocking_review_session_in_progress
           } ))
  ...
  in
  List.concat [ line_count_by_user; catch_up_for_features_with_no_review_remaining ]
    ...
    (match Feature.next_bookmark_update feature with
     ...
     | (No_update_expected | Update_expected_since _) as next_bookmark_update ->
       ...
       let could_wait_for_hydra =
         am_expecting_bookmark_update && not am_continuously_releasing_into
       in
       let fix_problems () : Next_step.t list =
         if am_expecting_bookmark_update then [ Wait_for_hydra ] else [ Fix_problems ]
       in
       (match Feature.review_goal feature with
        ...
        | Ok _review_goal ->
          (match feature_crs t feature with
           ...
           | Ok crs ->
             let maybe_prepend_rebase next_steps : Next_step.t list =
               ...
               | Release ->
                 ...
                 then next_steps
                           ...
                           (Feature_locks.check_unlocked (Feature.locks feature) Rebase)
             ...
             then if someone_reviewing_can_make_progress then [ CRs; Review ] else [ CRs ]
             else (
               ...
               then
                 if exists_review_manager t feature ~f:(fun user review_manager ->
...
module Next_steps = struct
  ...
  let equal t1 t2 = List.equal Next_step.equal t1 t2
end
...
let initialize_cached_feature_attributes t feature =
  ...
  in
  Feature.set_cached_attributes feature ~line_count_by_user ~next_steps ~review_analysis
;;
  ...
  let not_releasable () =
    ...
    let next_steps =
      List.filter next_steps ~f:(function
        | Release | Wait_for_continuous_release -> false
        | _ -> true)
    in
    let or_lock_error =
    ...
    error_s
      [%sexp
        ...
        , { feature = (feature_path : Feature_path.t)
          ...
          ; locked = (error_opt : (Error.t option[@sexp.option]))
          }]
  in
  match next_steps with
  | [ Release ] -> Ok ()
  | [ Wait_for_hydra ] | [ Rebase; Release ] | [ Wait_for_continuous_release ] ->
...
let feature_to_protocol ?rev_zero t feature : Iron_protocol.Feature.t =
  let feature_path = Feature.feature_path feature in
  let line_count_by_user =
    Or_error.try_with (fun () -> compute_line_count_by_user_exn t feature)
  in
  let is_rebased =
    ...
    `run_after_reaction
      (fun () ->
        Which_features.these_features
          (if is_ok (find_feature state feature_path) then [ feature_path ] else []))
  ;;

  let one get_feature_path _ action =
    `run_after_reaction
      (fun () -> Which_features.these_features [ get_feature_path action ])
  ;;
    ...
    `run_after_reaction
      (fun () ->
        Which_features.Features
          [ { feature_path = get_feature_path action; include_descendants = true } ])
  ;;
  ...
  let all_review_managers_of_users get_users state action =
    ...
    let which_features =
      let these_features =
        ...
        hset |> Feature_path.Set.of_hash_set |> Set.to_list
      in
...
let check_cached_feature_attributes t which_features ~ignore_diffs_in_errors =
  let faulty_caches =
    ...
    |> List.sort ~compare:(fun (f1, _) (f2, _) -> Feature_path.compare f1 f2)
  in
...
let crs_by_assignee t feature crs =
  ...
  List.map crs ~f:(fun cr ->
    let assignee =
      Cr_comment.Assignee.user_name
        (Cr_comment.assignee cr)
        ~feature_owner
        ~alternate_names
    in
    assignee, cr)
...
let update_review_manager_goals t feature =
  ...
  let all_users_involved =
    let may_reviewers =
      ...
      | Pending_since _ | Known (Error _) -> User_name.Set.empty
    in
    User_name.Set.union_list
      [ may_reviewers
  ...
  Set.iter all_users_involved ~f:(fun user ->
    ...
    | Known indexed_diff4s ->
      Review_manager.update_review_goal
        review_manager
        ~review_goal
        ~indexed_diff4s
...
let repartition_crs_and_cr_soons_by_assignee_for_all_features t =
  Cr_soons.repartition
    t.cr_soons
    ~alternate_names:(User_info.alternate_names t.user_info ~which:`All);
  Hashtbl.iter t.features_by_id ~f:(fun feature -> repartition_crs_by_assignee t feature)
;;
...
let change_fully_reviewed_revisions_internal
  t
...
let initialize_and_sync_file_system serializer =
  List.iter
    ~f:(fun dir -> Serializer.add_subtree serializer ~dir)
...
let feature_deserializer ~dynamic_upgrade_state =
  ...
  and review_managers =
    in_subdir review_managers_dir_in_feature_dir (all_subdirs Review_manager.deserializer)
  in
...
let add_feature_exn t feature =
  ...
  Cached.Invalidator.set_callback
    (Feature.cache_invalidator feature)
    ~callback_no_exn:(fun () ->
  ...
  | None ->
    ...
    let feature_protocol =
      Feature.to_protocol
        ...
        ~remote_repo_path
        ~has_children:false
        ~cr_summary:(Or_error.error_string "CRs unavailable for archived features")
        ~line_count_by_user:
...
let restrict_diff_from_base_to_tip
  ...
  let reviewer =
    ...
    | None | Whole_diff | Whole_diff_plus_ignored ->
    ...
    | For user_name ->
      (match feature_protocol.is_archived with
       ...
       | No ->
         ignore
           (find_review_manager t (force existing_feature) user_name |> ok_exn
             : Review_manager.t));
...
let get_maybe_archived_feature_and_reviewer_exn t ~what_feature ~what_diff =
  ...
  let sexp_of_archived_feature
    { Archived_feature.feature_id; feature_path; owners; archived_at; _ }
    =
    [%sexp
      [ { feature_id : Feature_id.t }
      ; { feature_path : Feature_path.t; owners : User_name.t list; archived_at : Time.t }
      ]]
  in
  ...
  let get_archived_feature_exn = function
    ...
    | _ :: _ :: _ as features ->
      let features =
        List.sort features ~compare:order_most_recently_archived_feature_first
      in
      raise_s
        [%sexp "multiple archived features matching", (features : archived_feature list)]
  in
  ...
  let find archived_features existing =
    ...
    | ( ( `All
      ...
      , _
      , Some feature ) -> return (feature_to_protocol t feature)
    ...
    | `Existing_or_most_recently_archived, _ :: _, None ->
      List.min_elt archived_features ~compare:order_most_recently_archived_feature_first
      |> Option.value_exn ~here:[%here]
      |> get_cached_or_reload_archived_feature_exn
    | `Archived, (_ :: _ as features), (Some _ | None) | `All, (_ :: _ as features), None
      -> get_cached_or_reload_archived_feature_exn (get_archived_feature_exn features)
    | _, [], None ->
      raise_s
        [%sexp
          "no such feature"
    ...
    | `Existing, (_ :: _ as features), None ->
      let err =
        ...
        | Ok feature ->
          Error.create "archived feature matching" feature [%sexp_of: archived_feature]
        | Error err -> err
      in
      raise_s
        [%sexp
...
let assigned_release_or_rebase t feature =
  Assigned_release_or_rebase.create
    feature
...
let deserializer =
  Deserializer.with_serializer (fun serializer ->
    ...
    and features_and_review_managers =
      in_subdir features_dir (all_subdirs (feature_deserializer ~dynamic_upgrade_state))
    and archived_features =
      in_subdir
        archived_features_dir
        (Archived_features.deserializer ~dynamic_upgrade_state)
    and catch_up_managers =
      in_subdir catch_up_dir (all_subdirs (all_subdirs Catch_up_session.deserializer))
    and fully_reviewed_edges =
      sequence_of (module Persist.Fully_reviewed_edges) ~in_file:fully_reviewed_edges_file
    ...
    fun ~server_config ->
      ...
      let features_and_review_managers =
        ...
        |> Map.data
        |> List.sort ~compare:(fun (feature1, _) (feature2, _) ->
             Int.compare
    ...
    M.implement_deferred_rpc (fun state ~version query ->
      ...
      Monitor.try_with ~run:`Now (fun () ->
        let (`run_after_reaction checked_features) =
          Post_check_in_features.which_features
            check_cached_in_features
            state
            (Query.action query)
        in
        f state query
        ...
        (match
           check_cached_feature_attributes
             state
             checked_features
          ...
          | exception post_condition_exn ->
            error_s
              [%sexp
                ...
                , { post_condition_exn : Exn.t; reaction : M.reaction Or_error.t }])
      in
      let response = Response.create reaction ~query_uuid ~server_received_query_at in
      Rpc_stats.add_hit
        state.rpc_stats
        ~took:(Response.server_took response)
        { by = Query.by query; rpc_name = M.name; rpc_version = version };
      ...
      then (
        ...
        Log.Global.sexp
          ~level:`Info
          ~tags:[ "what", "query" ]
          [%sexp (M.name, query, response : string * query * response)]);
      response)
  in
...
let implement_deferred_pipe_rpc (type action reaction) ?override_sexp_of_action ~log m f =
      ...
      (match subscription_or_error with
       ...
       | Ok subscription ->
         ...
         | Ok reader' ->
           ...
           don't_wait_for
             (Event_subscriptions.Subscription.closed subscription
             >>| function
             | `Removed -> ()
             | `Dropped dropped ->
...
let implement_pipe_rpc ?override_sexp_of_action ~log m f =
  implement_deferred_pipe_rpc ?override_sexp_of_action ~log m (fun state query ->
    Async.return (f state query))
;;
...
let only_server_user_is_authorized_exn query =
  ...
  then
    raise_s
      [%sexp
        "Unauthorized rpc for user"
        , { query_user : User_name.t; authorized_user : User_name.t }]
;;
...
let user_has_admin_privileges t user =
  User_name.equal user User_name.unix_login || User_info.Admins.mem t.user_info user
;;
      ...
      [%sexp
        ...
        , { user = (by : User_name.t)
          ; users_with_admin_privileges =
              (users_with_admin_privileges t : User_name.Set.t)
          }]
;;
  ...
  then ()
    ...
    raise_s
      [%sexp
        ...
        , { for_ : User_name.t; requested_by = (by : User_name.t) }]
;;

let review_authorization_may_skip_user_exn
  t
  feature
  review_manager
  query
  ...
  let review_authorization =
    Review_manager.Review_authorization.create
      ...
      ~current_feature_goal_subset:(what_goal_subset_needs_to_be_reviewed feature)
      ~reason
      ~create_catch_up_for_me
      query
    |> ok_exn
  ...
  | Error err ->
    (match is_reviewing_for with
     | `User _ ->
       if user_has_admin_privileges t (Query.by query) && not am_functional_testing
...
let review_authorization_exn
  ...
  =
  match
    review_authorization_may_skip_user_exn
      t
      feature
      review_manager
      query
  ...
  implement_rpc
    ...
    (fun t query ->
      ...
      let { Remove_alternate_names.Action.alternate_names; which; may_repartition_crs } =
        Query.action query
      in
      repartition_crs_and_cr_soons_by_assignee_if_needed
        t
        ~may_repartition_crs
        ~f:(fun () ->
...
let () =
  let module Update_valid_users_and_aliases = Iron_protocol.Update_valid_users_and_aliases
  in
  implement_rpc
    ~log:true
    ...
    (fun t query ->
      only_user_with_admin_privileges_is_authorized_exn t query;
      let { Update_valid_users_and_aliases.Action.valid_users_and_aliases
          ; may_repartition_crs
          }
        =
        Query.action query
      ...
      repartition_crs_and_cr_soons_by_assignee_if_needed
        ...
        ~may_repartition_crs
        ~f:(fun () ->
        User_info.update_valid_users_and_aliases_exn t.user_info valid_users_and_aliases))
;;
...
let () =
  ...
  implement_rpc
    ...
    (module User_set.Get)
    (fun t query ->
      let module M = (val User_info.get_user_set (Query.action query)
                        : User_info.User_set.S)
      in
  ...
  implement_rpc
    ...
    (fun t query ->
      let { User_set.Change.Action.user_set; user_names; change; idempotent } =
        Query.action query
      in
      let () =
      ...
      | `Using_locked_sessions ->
        ...
        Set.iter user_names ~f:(fun user_name ->
          iter_review_managers_of_user t user_name ~f:(fun _ review_manager ->
            Review_manager.set_is_using_locked_sessions
              review_manager
...
let () =
  ...
  implement_rpc
    ...
    (fun t query ->
      only_user_with_admin_privileges_is_authorized_exn t query;
      let () = Query.action query in
      User_info.refresh_existing_users
        t.user_info
...
let () =
  ...
  implement_rpc
    ...
    (fun t query ->
      only_user_with_admin_privileges_is_authorized_exn t query;
      let { Define_typos.Action.definitions; may_repartition_crs } = Query.action query in
    ...
    let is_fully_reviewed =
      ...
      | [ Unlock Release ] ->
        ...
        || raise_s
             [%sexp
               (concat
                  [ "release is locked -- consider using "
      ...
      | next_steps ->
        List.exists next_steps ~f:(function
          | Release | Wait_for_continuous_release -> true
          | _ -> false)
    in
    if not is_fully_reviewed
    then raise_s [%sexp "feature is not fully reviewed", (feature_path : Feature_path.t)];
    { Fully_reviewed_edge.rev_zero
  ...
  implement_rpc
    ...
    (fun t query ->
      ...
      let edge_opt =
        ...
        | `Feature_base_to_tip
            { feature_path; even_if_release_is_locked; if_feature_is_empty } ->
          ...
          then (
            ...
            Some
              (ok_exn
                 (create_fully_reviewed_edge rev_zero feature ~even_if_release_is_locked))
      in
      match edge_opt with
      | None -> ()
...
let ensure_can_archive feature ~for_ ~must_be_owner =
  ...
  if Feature.is_permanent feature then failwith "cannot archive a permanent feature"
;;
...
let archive_feature t query feature ~for_ ~must_be_owner ~reason_for_archiving =
  ...
  let archived_feature =
    Archived_feature.create feature ~archived_at:(Query.at query) ~reason_for_archiving
  in
...
let () =
  ...
  implement_rpc
    ...
    (fun t query ->
      let { Archive_feature.Action.feature_path; rev_zero; for_; reason_for_archiving } =
        Query.action query
      in
      let feature = find_feature_exn t feature_path in
...
let () =
  ...
  implement_rpc
    ~log:true
    (Post_check_in_features.one Brain_forget.Action.feature_path)
    (module Brain_forget)
    (fun t query ->
      ...
      in
      Review_manager.forget_from_brain_exn
        review_manager
        review_authorization
        ~what_to_forget;
      ...
      then (
        ...
        | Some catch_up_manager ->
          ...
          List.iter catch_up_sessions ~f:(fun catch_up_session ->
            let ids_to_catch_up =
              List.filter_map
                (Catch_up_session.diff4s_to_catch_up catch_up_session)
...
let () =
  let module Clear_bookmarks_without_feature =
    Iron_protocol.Clear_bookmarks_without_feature
  in
  implement_rpc
    ~log:true
    Post_check_in_features.none
    ...
    (fun t query ->
      only_user_with_admin_privileges_is_authorized_exn t query;
      let { Clear_bookmarks_without_feature.Action.remote_repo_path } =
        Query.action query
      in
      Hashtbl2_pair.remove_all1 t.bookmarks_without_feature remote_repo_path)
;;

let change_feature t feature query (updates : Iron_protocol.Change_feature.Update.t list) =
  let users =
    User_name.Set.union_list
      (List.map updates ~f:(function
        ...
        | `Set_owners us -> User_name.Set.of_list us
        ...
        | `Set_whole_feature_reviewers us -> us
        ...
        | `Set_xcrs_shown_in_todo_only_for_users_reviewing _ -> User_name.Set.empty))
  in
  ...
  let updates, errors =
    ...
    List.rev !updates_ref, List.rev !errors_ref
  in
  let whole_feature_followers = Feature.whole_feature_followers feature in
  let whole_feature_reviewers = Feature.whole_feature_reviewers feature in
  ...
  if whole_feature_reviewers_changed || whole_feature_followers_changed
...
let change_feature_exn t feature query updates =
  List.iter (change_feature t feature query updates) ~f:(fun (_, or_error) ->
    ok_exn or_error)
;;
...
let () =
  ...
  implement_rpc
    ~log:true
    (Post_check_in_features.one Change_feature.Action.feature_path)
...
let () =
  ...
  implement_rpc
    ...
    (fun t query ->
      let { Expect_next_base_update.Action.feature_path; for_; expected_base } =
        Query.action query
      in
      let feature = find_feature_exn t feature_path in
...
let () =
  ...
  implement_rpc
    ...
    (fun t query ->
      let { Change_fully_reviewed_revisions.Action.what_to_do } = Query.action query in
      only_user_with_admin_privileges_is_authorized_exn t query;
...
let () =
  ...
  implement_rpc
    ...
    (fun t query ->
      only_user_with_admin_privileges_is_authorized_exn t query;
      let { Check_cached_feature_attributes.Action.which_features
...
let () =
  let module With_cached_attributes_errors = Iron_protocol.With_cached_attributes_errors
  in
  implement_rpc
    ~log:false
    Post_check_in_features.none
    ...
    (fun t query ->
      ...
      | Get ->
        ...
        if not (List.is_empty errors) then Error.raise (Error.of_list errors)
      | Clear ->
        only_user_with_admin_privileges_is_authorized_exn t query;
...
let () =
  let module With_timed_event_errors = Iron_protocol.With_timed_event_errors in
  implement_rpc
    ~log:false
    Post_check_in_features.none
...
let need_diff4s_starting_from t feature =
  let users_by_review_edge =
    alist_of_iter (iter_review_managers_of_feature t feature)
    |> List.concat_map ~f:(fun (user, review_manager) ->
         List.map
           (Set.to_list (Review_manager.need_diff4s_starting_from review_manager))
...
let worker_cache_feature_revs t ?need_diff4s_starting_from:need_diff4s feature =
  ...
  in
  { Worker_cache.Feature_revs.diff4s_revs
  ; base = Feature.base feature
  ; tip = Feature.tip feature
  }
;;
...
let () =
  ...
  implement_rpc
    ...
    (fun t query ->
      ...
      | Set_max_size max_size -> Worker_cache.set_max_size t.worker_cache ~max_size
      | Set_status status -> Worker_cache.set_status t.worker_cache ~status
  ...
  implement_rpc
    ...
    (fun t query ->
      let is_authorized_exn user =
        if (not am_functional_testing) && not (User_name.equal user (Query.by query))
        then only_user_with_admin_privileges_is_authorized_exn t query
      in
      match Query.action query with
      | Remove_user user ->
      ...
      | Remove_machine (user, machine) ->
        is_authorized_exn user;
        Unclean_workspaces_manager.remove_machine_exn
          t.unclean_workspaces
          query
          user
          machine)
;;
  ...
  then
    raise_s
      [%sexp
        ...
        , { requested_by : User_name.t; requested_for = (for_ : User_name.t) }]
;;
    ...
    (fun t query ->
      ...
      let relative_feature_path_completion_types, other_completions =
        List.partition_map types ~f:(function
          | Feature_path -> First `Existing
          | Feature_path_with_catch_up -> First `With_catch_up
          | Absolute_feature_path ->
            Second (Feature_forest.complete t.features ~prefix `Of_full_name)
          | Archived_feature_path -> First `Archived
          | Metric_name -> Second (Metrics.complete t.metrics ~prefix)
          | Remote_repo_path ->
            Second
          ...
          | User_info which -> Second (User_info.complete t.user_info ~prefix which))
      in
      let relative_feature_path_completions =
        Feature_path.complete ~prefix `Of_partial_name ~iter_features:(fun ~f ->
...
let create_feature_exn t query =
  let { Iron_protocol.Create_feature.Action.feature_path
      ; owners
      ; is_permanent
      ; description
      ; base
       ...
       | None ->
         ...
         Or_pending.iter (Feature.tip_facts parent) ~f:(fun facts ->
           ...
           | Error err ->
             raise_s
               [%sexp
                 (sprintf !"invalid base revision %{Rev#hum}" base : string)
                 , (err : Error.t)]);
         base)
  in
  ...
  let batch_of_feature_changes =
    ...
    in
    Batch_of_feature_changes.add_whole_feature_reviewers
      changes
...
let () =
  ...
  implement_rpc
    ...
    (fun t query ->
      ...
      { Create_feature.Reaction.feature_id = Feature.feature_id feature
...
module Rename = struct
  ...
  let to_hg_rename { from_feature; to_ } =
    { Iron_hg.Rename.feature_id = Feature.feature_id from_feature
...
let renames_exn t ~from_feature ~to_ ~to_may_exist ~check_rename_lock =
  ...
  then
    raise_s
      [%sexp "the parent of the target feature has to exist", (to_ : Feature_path.t)];
  let from_features =
    from_feature
    :: ok_exn
         (Feature_forest.strict_descendants
            t.features
  ...
  List.map from_features ~f:(fun from_feature ->
    let to_ =
      Feature_path.of_parts_exn
        (to_parts
        @ List.drop
            (Feature_path.parts (Feature.feature_path from_feature))
            from_num_parts)
    in
...
let rename_features ?(between_removes_and_adds = ignore) t query renames =
  ...
  let renames_in_increasing_order_of_num_parts =
    List.sort renames ~compare:(fun (r1 : Rename.t) r2 ->
      Int.compare (Feature_path.num_parts r1.to_) (Feature_path.num_parts r2.to_))
  in
  (* Remove all features from the feature forest in decreasing order of num_parts, and
     ...
     archived. *)
  List.iter
    (List.rev renames_in_increasing_order_of_num_parts)
...
let prepare_to_compress t feature ~for_ ~check_rename_lock =
  ...
  let feature_has_review_remaining =
    Map.exists
      (Feature.line_count_by_user feature |> ok_exn)
      ~f:(fun line_count ->
        let review =
...
let () =
  ...
  implement_rpc
    ...
    (fun t query ->
      ...
      let feature = find_feature_exn t feature_path in
      let remote_repo_path = remote_repo_path t feature ~rev_zero in
      let parent, renames = prepare_to_compress t feature ~for_ ~check_rename_lock:true in
...
let () =
  ...
  implement_rpc
    ~log:false
    (Post_check_in_features.compress Compress.Action.feature_path)
    (module Compress)
    (fun t query ->
        ...
        archive_feature
          ...
          ~for_
            (* [must_be_owner:true] is already checked in [prepare_to_compress] and thus
              cannot fail here. *)
          ~must_be_owner:true
          ~reason_for_archiving:"compressed"))
;;
...
let () =
  ...
  implement_rpc
    ...
    (fun t query ->
      let { Prepare_to_rename.Action.from; to_; rev_zero } = Query.action query in
      ...
      { Prepare_to_rename.Reaction.from_feature_path = from
...
let () =
  ...
  implement_rpc
    ~log:true
    (Post_check_in_features.rename ~to_:Rename_feature.Action.to_)
...
let () =
  ...
  implement_rpc
    ...
    (module Copy_feature)
    (fun t query ->
      let { Iron_protocol.Copy_feature.Action.from_
          ; to_
          ; rev_zero
      ...
      | Ok _ ->
        raise_s
          [%sexp "feature that is being copied to already exists", (to_ : Feature_path.t)]
      | Error _ ->
        let from_parent = ok_exn (find_parent t from_) in
        let to_parent = ok_exn (find_parent t to_) in
        if not
             (Feature_id.equal
                (Feature.feature_id from_parent)
                (Feature.feature_id to_parent))
        then
          failwith "parents must be the same to copy.  Consider using copy then rename.";
         ...
         | true, Ok _ ->
           failwith
             (concat
                ...
                ; " because the feature is fully reviewed, and thus review will be copied"
                ])
         | false, Error _ ->
           let who_has_done_some_review =
             list_of_iter (fun ~f ->
               iter_review_managers_of_feature t feature ~f:(fun user review_manager ->
                 if Review_manager.have_done_some_review review_manager then f user))
           in
           if not (List.is_empty who_has_done_some_review)
           then (
             let who_has_done_some_review =
               List.sort who_has_done_some_review ~compare:User_name.compare
             in
             raise_s
               [%sexp
                 (concat
                    [ "feature is not fully reviewed, review has been done and will not \
                       be copied -- consider using "
                    ; Switch.without_copying_review
                    ]
                   : string)
        ...
        let create_query =
          Query.with_action
            query
            { Iron_protocol.Create_feature.Action.feature_path = to_
            ; owners = Feature.owners feature
            ; is_permanent = Feature.is_permanent feature
            ; description = Feature.description feature
        ...
        Feature.set_review_is_enabled copy query (Feature.review_is_enabled feature);
        Feature.set_crs_are_enabled copy query (Feature.crs_are_enabled feature);
        Feature.set_crs_shown_in_todo_only_for_users_reviewing
          copy
          query
          (Feature.crs_shown_in_todo_only_for_users_reviewing feature);
        Feature.set_xcrs_shown_in_todo_only_for_users_reviewing
          copy
          query
          (Feature.xcrs_shown_in_todo_only_for_users_reviewing feature);
        { Copy_feature.Reaction.feature_id = Feature.feature_id feature
        ; remote_repo_path = remote_repo_path t feature
        ; tip = Feature.tip feature
        })
;;
...
let () =
  ...
  implement_rpc
    ...
    (fun t query ->
      let { Forget_session.Action.feature_path; for_; review_session_id; what_to_forget } =
        Query.action query
      in
      let feature = find_feature_exn t feature_path in
      let review_manager = ok_exn (find_review_manager t feature for_) in
...
let () =
  ...
  implement_rpc
    ...
    (fun t query ->
      let { Session_set_lock.Action.feature_path; for_; which_session; set_is_locked_to } =
        Query.action query
      in
      let feature = find_feature_exn t feature_path in
      let review_manager = ok_exn (find_review_manager t feature for_) in
...
let () =
  ...
  implement_rpc
    ...
    (fun t query ->
      ...
      iter_review_managers_of_feature
        t
        (find_feature_exn t feature_path)
        ...
        in
        r := user :: !r);
      { de_aliased = User_name.Set.of_list !de_aliased
      ; did_not_de_alias_due_to_review_session_in_progress =
          User_name.Set.of_list !did_not_de_alias_due_to_review_session_in_progress
      ; nothing_to_do = User_name.Set.of_list !nothing_to_do
      })
;;
...
let () =
  ...
  implement_rpc
    ...
    (fun t query ->
      ...
      { Get_brain.Reaction.reviewer = Review_manager.reviewer review_manager
      ; brain
      ; remote_rev_zero = Feature.rev_zero feature
      ; remote_repo_path = remote_repo_path t feature
      })
...
let () =
  let module Get_lines_required_to_separate_ddiff_hunks =
    Iron_protocol.Get_lines_required_to_separate_ddiff_hunks
  in
  implement_rpc
    ~log:false
    Post_check_in_features.none
    ...
    (fun t query ->
      ...
      List.Assoc.map (Feature_forest.roots t.features) ~f:(fun feature ->
        Option.value
          ~default:Constants.lines_required_to_separate_ddiff_hunks_default
...
let () =
  ...
  implement_rpc
    ...
    (fun t query ->
      let { Get_review_session.Action.feature_path
          ; for_
          ; rev_zero
          ; ensure_reviewing
          ; which_session
      ...
      let may_be_reviewed_by =
        Allow_review_for.may_be_reviewed_by
          ~reviewed_for:for_
          (Ref.Permissioned.get (Feature.allow_review_for feature))
      in
      let review_session =
        Review_manager.get_session_exn
          review_manager
          (what_goal_subset_needs_to_be_reviewed feature)
          ~may_be_reviewed_by
      ...
      let status =
        ...
        | `Review_session _ as result ->
          ...
          result
      in
      let may_second = seconding_is_recommended t ~for_ feature ~even_if_locked:false in
      { Get_review_session.Reaction.status
      ; feature_tip = Feature.tip feature
      ; remote_rev_zero = Feature.rev_zero feature
      ; remote_repo_path
      ; may_second
...
let () =
  ...
  implement_rpc
    ...
    (fun t query ->
      let { Clear_catch_up_sessions.Action.feature_path
          ; for_
      ...
      let de_alias unresolved_name =
        let user =
          User_name_by_alternate_name.to_user_name user_name_by_alias unresolved_name
        in
      ...
      let should_clear diff4_to_catch_up =
        ...
        | Create_catch_up_for_me | Follower | Unfinished_review ->
          (match only_those_reviewed_by with
           | True -> true
           | _ -> false)
        | Reviewed_by_someone_else { reviewed_by; reason = _ } ->
          let reviewed_by = reviewed_by |> User_name.to_unresolved_name |> de_alias in
  ...
  implement_rpc
    ...
    (fun t query ->
      let review_managers feature of_ =
        list_of_iter (fun ~f ->
          iter_review_managers_of_feature t feature ~of_ ~f:(fun _user review_manager ->
            f review_manager))
      in
      match Query.action query with
      ...
      | State ->
        ...
        t |> [%sexp_of: t]
      | Dynamic_upgrade_state -> Dynamic_upgrade.State.dump t.dynamic_upgrade_state
      ...
      | Feature feature_path -> find_feature_exn t feature_path |> [%sexp_of: Feature.t]
      ...
      | Event_subscriptions ->
        [%sexp
          ...
          ; feature_updates =
              (Feature_updates_manager.dump_subscriptions t.feature_updates_manager : Sexp
                                                                                      .t)
          }
      ...
      | Bookmarks_without_feature (r, u) ->
        ...
        (match r, u with
         ...
         | None, `User user_name ->
           ...
           |> [%sexp_of:
                (Remote_repo_path.t, Bookmark_without_feature.t list) Hashtbl.t option]
...
let () =
  ...
  implement_rpc
    ...
    (fun t query ->
      ...
      then raise_s [%sexp "not a fully-reviewed edge", { from : Rev.t; to_ : Rev.t }])
;;
...
let () =
  ...
  implement_rpc
    ...
    (fun t query ->
      let { Enable_review.Action.feature_path; add_whole_feature_reviewers } =
        Query.action query
      in
      ...
      change_feature_exn
        ...
        feature
        query
        [ `Set_crs_are_enabled true; `Set_review_is_enabled true ])
;;
...
let () =
  ...
  implement_deferred_rpc
    ...
    (fun t query ->
      let open Async in
      let%map feature = get_maybe_archived_feature_exn t (Query.action query) in
...
let () =
  ...
  implement_rpc
    ~log:true
    (Post_check_in_features.one Fn.id)
    (module Force_retry)
    (fun t query ->
      let feature_path = Query.action query in
      let feature = find_feature_exn t feature_path in
      ...
      then Feature.force_hydra_retry feature
        ...
        raise_s [%sexp "feature's bookmark is missing", (feature_path : Feature_path.t)])
;;
...
let () =
  ...
  implement_rpc
    ...
    (fun t query ->
      let { Force_set_cached_feature_attributes.Action.feature_path
...
let () =
  ...
  implement_rpc
    ...
    (fun _t query ->
      ...
      | Gc_quick_stat -> Gc.quick_stat () |> [%sexp_of: Gc.Stat.t]
      | Process_times -> Unix.times () |> [%sexp_of: Core_unix.process_times])
;;
...
let () =
  ...
  implement_rpc
    ...
    (fun t query ->
      let { Get_cr_summary.Action.feature_path } = Query.action query in
  ...
  implement_deferred_rpc
    ...
    (fun t query ->
      let { Get_feature_by_id.Action.feature_id; even_if_archived } =
        Query.action query
      in
      get_maybe_archived_feature_exn
        t
...
let () =
  ...
  implement_rpc
    ...
    (fun t query ->
      ...
      let feature = find_feature_exn t feature_path in
      let remote_repo_path = remote_repo_path t feature ?rev_zero in
      { base = Feature.base feature; tip = Feature.tip feature; remote_repo_path })
;;
...
let () =
  ...
  implement_deferred_rpc
    ...
    (fun t query ->
      let open Async in
      let { Get_feature_maybe_archived.Action.what_feature; what_diff } =
        Query.action query
      in
      let%map _reviewer, feature_protocol =
...
let add_span_since_push_value_to_metric_if_available t ~feature ~tip ~metric_name =
  ...
  | Some push_event ->
    ...
    then (
      ...
      | `Ok ->
        let span_since_push =
          Time.diff (Time.now ()) (Push_event.at push_event) |> Time.Span.to_sec
        in
        Metrics.add_values
          t.metrics
          { feature_path; metric_name; values = [ span_since_push ] })
;;
...
let () =
  ...
  implement_rpc
    ~log:true
    (Post_check_in_features.one Hydra_worker.Action.feature_path)
...
let () =
  ...
  implement_rpc
    ...
    (fun t query -> User_info.alternate_names t.user_info ~which:(Query.action query))
;;
...
let () =
  implement_rpc
    ...
    (fun t query -> Metrics.get t.metrics (Query.action query))
;;
      ...
      then Metrics.add_values t.metrics (Query.action query)
        ...
        raise_s
          [%sexp
            ...
            , { user = (by : User_name.t); authorized_users : User_name.Set.t }]))
;;
    ...
    (fun t query ->
      ...
      then
        ...
        |> List.map ~f:(fun (feature_path, feature) ->
             { List_features.Reaction.feature_path
             ; feature_id = Feature.feature_id feature
             ; owners = Feature.owners feature
...
let () =
  ...
  implement_rpc
    ...
    (fun t query ->
      ...
      then
        ok_exn
          (Archived_features.list_feature_names
             t.archived_features
             ~descendants_of
             ~depth)
      else
  ...
  implement_rpc
    ...
    (fun t query ->
      ...
      let reaction feature =
        { List_feature_revisions.Reaction.feature_path = Feature.feature_path feature
        ; base = Feature.base feature
        ; tip = Feature.tip feature
        }
      in
      let subtree_features =
        List.map subtrees ~f:(fun feature_path ->
          ok_exn
            (Feature_forest.list
               t.features
               ~descendants_of:(Feature feature_path)
               ~depth:Int.max_value)
      ...
      let features =
        List.dedup_and_sort
          (List.concat (features :: subtree_features))
      ...
      | { feature_path; _ } :: _ ->
        ...
        List.iter features ~f:(fun { feature_path = feature_path2; _ } ->
          if not (Feature_name.equal root (Feature_path.root feature_path2))
          then
            raise_s
              [%sexp
                "features must be in the same tree but aren't"
                , ([ feature_path; feature_path2 ] : Feature_path.t list)]);
        let remote_repo_path =
          remote_repo_path t (find_feature_exn t feature_path) ~rev_zero
        in
        { List_feature_revisions.Reaction.remote_repo_path; features })
;;
...
let () =
  ...
  implement_rpc
    ~log:false
    Post_check_in_features.none
    (module List_root_features)
    (fun t query ->
      let () = Query.action query in
      list_of_iter (fun ~f ->
        Feature_forest.iteri_roots t.features ~f:(fun root_feature feature ->
          f
            { List_root_features.Reaction.root_feature
            ; remote_repo_path = remote_repo_path t feature
            ; tip = Feature.tip feature
            })))
;;
...
let () =
  ...
  implement_rpc
    ~log:true
    (Post_check_in_features.one Lock_feature.Action.feature_path)
    (module Lock_feature)
    (fun t query ->
      let { Lock_feature.Action.feature_path; for_; lock_names; reason; is_permanent } =
        Query.action query
      in
      User_info.ensure_user_exists t.user_info for_;
      let feature = find_feature_exn t feature_path in
...
let () =
  let module Find_features_by_partial_name = Iron_protocol.Find_features_by_partial_name
  in
  implement_rpc
    ~log:false
    Post_check_in_features.none
    ...
    (fun t query ->
      ...
      let feature_path =
        Option.try_with (fun () ->
          String.try_chop_suffix partial_name_prefix ~suffix:"/" |> Feature_path.of_string)
      in
      let find (mem, iteri) =
      ...
      in
      let ops_on_existing =
        Feature_forest.mem t.features, Feature_forest.iteri t.features
      in
      let ops_on_catch_up =
        Hashtbl2_pair.mem1 t.catch_up_managers, Hashtbl2_pair.iter1 t.catch_up_managers
      in
      let combine_ops (mem1, iteri1) (mem2, iteri2) =
        ( (fun feature_path -> mem1 feature_path || mem2 feature_path)
        , fun ~f ->
            iteri1 ~f:(fun p _ -> f p ());
            iteri2 ~f:(fun p _ -> f p ()) )
      in
      let sequence_ops (mem1, iteri1) (mem2, iteri2) =
        ...
        , fun ~f ->
            ...
            iteri1 ~f:(fun p _ ->
              hit := true;
              f p ());
            if not !hit then iteri2 ~f:(fun p _ -> f p ()) )
      in
      ...
      | `Existing_or_with_catch_up -> find (sequence_ops ops_on_existing ops_on_catch_up))
;;
...
let () =
  ...
  implement_rpc
    ...
    (fun t query ->
      let { Mark_fully_reviewed.Action.feature_path
          ; whom_to_mark
          ; reason
          ; create_catch_up_for_me
          ; base
      ...
      let check_rev name get_server_rev = function
        ...
        | Some provided_rev ->
          ...
          then
            raise_s
              [%sexp
                (sprintf "provided %s is different from what server knows" name : string)
                , { provided_rev : Rev.t; server_rev : Rev.t }]
      in
      ...
      iter_review_managers_of_feature
        ...
        | `Ok review_authorization ->
          Review_manager.mark_fully_reviewed
            review_manager
            query
...
let () =
  ...
  implement_rpc
    ...
    (fun t query ->
      let { May_modify_others_catch_up.Action.for_ } = Query.action query in
      check_catch_up_for_exn t ~for_ ~by:(Query.by query))
;;
      ...
      let check_review_manager _ review_manager =
        match
          review_authorization_may_skip_user_exn
            ...
            ~reason
            ~create_catch_up_for_me:false
            ~is_reviewing_for:whose_review
        with
        | `Error e -> Error.raise e
        | `Ok (_ : Review_manager.Review_authorization.t) -> ()
        | `Unauthorized_for_users_with_only_follow_lines _ -> ()
      in
      iter_review_managers_of_feature t feature ~of_:whose_review ~f:check_review_manager)
;;
...
let () =
  ...
  implement_rpc
    ...
    (fun t query ->
      let { Prepare_to_rebase.Action.feature_path = child_path
          ; rev_zero
      ...
      let may_rebase =
        Feature.is_owner child for_ || is_ok (find_review_manager t child for_)
      in
      if not may_rebase
      then
        if User_name.equal for_ (Query.by query)
      ...
      | Ok parent_path ->
               ...
               if not
                    (ok_exn
                       (Rev_facts.Is_conflict_free.check
                          new_base_facts.is_conflict_free
                          new_base))
               then
                 raise_s
                   [%sexp
                     "new base is not conflict free"
                     , (parent_path : Feature_path.t)
                     , (new_base : Rev.t)];
               if not
                    (ok_exn
                       (Rev_facts.Obligations_are_valid.check
                          new_base_facts.obligations_are_valid
                          new_base))
               then
                 raise_s
                   [%sexp
                     "new base obligations are invalid"
                     , (parent_path : Feature_path.t)
                     , (new_base : Rev.t)];
               if (not
                     (ok_exn
                        (Rev_facts.Is_cr_clean.check new_base_facts.is_cr_clean new_base)))
                  && not allow_non_cr_clean_new_base
               then
                 failwith
                   "new base is not cr clean -- use -allow-non-cr-clean-new-base to \
                    override";
               new_base)
        in
        (match Feature.tip_facts child with
         ...
         | Known child_tip_facts ->
           ...
           then
             raise_s [%sexp "feature is already rebased", (child_path : Feature_path.t)];
           if not
                (ok_exn
                   (Rev_facts.Is_conflict_free.check
                      old_tip_facts.is_conflict_free
                      child_tip))
           then
             raise_s
               [%sexp
                 "feature is not conflict free"
  ...
  implement_rpc
    ...
    (fun t query ->
      ...
      then
        ...
        raise_s
          [%sexp
            "feature has a bookmark", (Feature.feature_path feature : Feature_path.t)];
      { tip = Feature.tip feature
      ; remote_repo_path = remote_repo_path t feature ~rev_zero
      })
;;
...
let () =
  ...
  implement_rpc
    ~log:true
    (Post_check_in_features.create (fun t (action : Release.Action.t) ->
       let feature_path = action.feature_path in
       `run_after_reaction
         (fun () ->
           let released_feature_if_present =
             if is_ok (find_feature t feature_path) then [ feature_path ] else []
           in
           Which_features.these_features
             (match Feature_path.parent feature_path with
    ...
    (module Release)
    (fun t query ->
      let { Release.Action.feature_path; rev_zero; tagged_tip; for_ } =
        Query.action query
      in
      let by = Query.by query in
      let feature = find_feature_exn t feature_path in
      ...
      let send_release_email_to =
        Email_address.Set.union_list
          ...
          ; Option.value_map
              parent
              ~default:Email_address.Set.empty
      ...
      let disposition =
        ...
        | `Directly ->
          ...
          Feature.set_latest_release
            feature
            { released_feature = released_child; diff_from_base_to_tip };
      ...
      in
      { disposition; send_release_email_to })
;;
    ...
    let rec loop rev : unit =
      ...
      then
        ...
        then r.return true
          ...
          Hashtbl2_pair.find2_iter1 t.fully_reviewed_edges rev ~f:(fun from_ _ ->
            loop from_))
    in
    loop rev;
    false)
...
let () =
  ...
  implement_rpc
    ~log:true
    (Post_check_in_features.one Reviewed_diffs.Action.feature_path)
    (module Reviewed_diffs)
    (fun t query ->
      let { Reviewed_diffs.Action.feature_path
          ; for_
          ; reason
          ; review_session_id
          ; diff4_in_session_ids
...
let () =
  ...
  implement_rpc
    ...
    (fun t query ->
      let { Catch_up_diffs.Action.feature_path
          ; for_
          ; catch_up_session_id = id
          ; diff4_in_session_ids
          }
        =
        Query.action query
...
let () =
  ...
  implement_rpc
    ~log:true
    (Post_check_in_features.one Unsecond.Action.feature_path)
    (module Unsecond)
    (fun t query ->
...
let () =
  ...
  implement_rpc
    ~log:true
    (Post_check_in_features.one Second.Action.feature_path)
    (module Second)
    (fun t query ->
...
let () =
  ...
  implement_rpc
    ...
    (fun t query ->
      let { Set_brains_to_goal_if_edge.Action.feature_path } = Query.action query in
      let feature = find_feature_exn t feature_path in
    ...
    (fun t query ->
      ...
      let line_count_by_user =
        ...
        |> List.filter ~f:(fun (user, line_count) ->
             ...
             && Review_or_commit.count (Line_count.to_review_column_shown line_count) > 0)
      in
      let review_users = List.map line_count_by_user ~f:fst |> User_name.Set.of_list in
      let cr_summary = ok_exn (feature_cr_summary t feature) in
      let cr_users =
      ...
      let next_step_users =
        List.map (Feature.next_steps feature) ~f:(function
          ...
          | In_parent next_step_in_parent ->
            (match Feature_path.parent (Feature.feature_path feature) with
             | Error error ->
               raise_s
                 [%sexp
                   "Iron bug.  Root feature has next step [In_parent _]."
          ...
          | Wait_for_hydra | Wait_for_continuous_release -> User_name.Set.empty)
        |> User_name.Set.union_list
      in
      let active_users =
        User_name.Set.union_list
      ...
      let requested_active_users =
        ...
        | Some_active only_email ->
          ...
          then
            raise_s
              [%sexp
                "some requested users have nothing to do", (inactive : User_name.Set.t)];
          active
      in
      let users = Set.union requested_active_users owners in
      { Remind.Reaction.description = Feature.description feature
      ; line_count_by_user
...
let () =
  ...
  implement_rpc
    ...
    (fun t query ->
      let { Synchronize_state.Action.remote_repo_path; bookmarks } = Query.action query in
      ...
      Feature_forest.iter_descendants
        t.features
        (Feature.feature_path root)
        ~f:(fun feature ->
        let feature_path = Feature.feature_path feature in
        ...
        | Some
            { bookmark = _
            ; rev_info = { rev_author_or_error = _; first_12_of_rev }
            ; status
          ...
          Feature.set_has_bookmark
            feature
            query
            ~compilation_status:(`Update_with compilation_status);
          (match
             Feature.synchronize_with_hydra
               feature
          ...
          then
            add_span_since_push_value_to_metric_if_available
              t
              ~feature
              ~tip:first_12_of_rev
      ...
      (let bookmarks_without_feature =
         ...
         |> List.filter_map
              ~f:(fun
                   { bookmark
                   ; rev_info = { rev_author_or_error; first_12_of_rev }
                   ; status = _
              ...
              | Error _ -> None
              | Ok rev_author ->
                Some (rev_author, { Bookmark_without_feature.bookmark; first_12_of_rev }))
    ...
    (fun t query ->
      ...
      let features_not_owned_yet_assigned_to_release =
        ...
        List.iter all_owned_feature_ids ~f:(fun feature_id ->
          ...
          | My_owners ->
            Feature_forest.iter_children
              t.features
              (Feature.feature_path feature)
              ~f:(fun child ->
              if List.mem (Feature.next_steps child) Release ~equal:Next_step.equal
              then (
                match Assigned_release_or_rebase.create child ~parent:(Some feature) with
                | None -> ()
      ...
      let assigned =
        ...
        iter_review_managers_of_user t for_ ~f:(fun feature_id review_manager ->
          ...
          let line_count =
            ...
            | Some
              ...
              { Line_count.review = To_goal_via_session.maybe_partially_known review
              ; catch_up
              ; completed
              ; have_potentially_blocking_review_session_in_progress
              }
          ...
          let assigned_next_steps : Next_step.Assigned.t =
            ...
            then [ Ask_seconder ]
              ...
              let maybe_prepend_rebase t =
                if need_rebase then [ Next_step.Rebase; t ] else [ t ]
              in
              ...
              | Some next_step ->
                (match next_step with
                 ...
                 | Restore_base
                 | Restore_bookmark
                 | Widen_reviewing -> only_first_owner (maybe_prepend_rebase next_step)
                 | Fix_build | Fix_problems ->
                 ...
                 | Release ->
                   (match assigned_release_or_rebase t feature with
                    | None -> []
                    | Some { assignee; assigned } ->
                      if User_name.( <> ) for_ assignee then [] else assigned)
                 | Add_code
                 | Archive
                 | Ask_seconder
                 | Compress
        ...
        Hashtbl.iter
          features_not_owned_yet_assigned_to_release
          ~f:(fun (feature, assigned_next_steps) ->
          let assigned =
            { Todo.Assigned.feature_path = Feature.feature_path feature
            ; feature_path_exists = true
            ; review_is_enabled = Feature.review_is_enabled feature
      ...
      let make_feature_info feature_id =
        ...
        let rev_facts rev facts =
          Or_pending.map facts ~f:(fun (facts : Rev_facts.t) ->
            let one check fact =
              ...
              | Error _ -> Error ()
            in
            { Todo.Rev_facts.is_conflict_free =
        ...
        { Todo.Feature_info.feature_path
        ; num_crs = !num_crs
        ; num_xcrs = !num_xcrs
        ; num_reviewers_with_review_remaining = !num_reviewers_with_review_remaining
        ; base
        ; tip
        ; review_is_enabled = Feature.review_is_enabled feature
        ; next_steps = Feature.next_steps feature
      ...
      let can_be_hidden feature =
        let is_empty_umbrella_feature () =
          List.exists (Feature.next_steps feature) ~f:(function
            | Add_code -> true
            | _ -> false)
          && (Feature.is_permanent feature
             || Feature_forest.has_children_exn t.features (Feature.feature_path feature)
             )
        in
        let is_permanently_locked lock_name =
          Feature_locks.is_permanently_locked
            (Feature.locks feature)
            (Next_step.Lock_name.to_lock_name lock_name)
        in
        let is_permanently_locked_and_stable () =
          ...
          | _ -> false
        in
        is_empty_umbrella_feature () || is_permanently_locked_and_stable ()
      in
        ...
        List.iter Features_by_parties.Parties.all ~f:(function
          ...
          | Whole_feature_followers as parties ->
            ...
            |> List.iter ~f:(fun feature_id ->
                 if (not
                       (Hash_set.mem
                          feature_ids_already_shown_in_the_own_table
                          feature_id))
  ...
  implement_deferred_rpc
    ...
    (fun t query ->
      ...
      then
        raise_s
          [%sexp
            "a feature with that name already exists", (feature_path : Feature_path.t)];
      (match Feature_path.parent feature_path with
       | Error _ -> ()
       | Ok parent_path ->
         if is_error (find_feature t parent_path)
         then
           raise_s [%sexp "parent feature doesn't exist", (parent_path : Feature_path.t)]);
      let archived_feature =
        ok_exn (Archived_features.find_by_id t.archived_features feature_id)
      in
      ...
      { Unarchive_feature.Reaction.feature_tip = Feature.tip feature
      ; remote_repo_path = remote_repo_path t feature
      })
;;
...
let () =
  ...
  implement_rpc
    ...
    (fun t query ->
      let { Update_bookmark.Action.feature_path; info; feature_id; augment_worker_cache } =
        Query.action query
      in
      match find_feature t feature_path with
      | Error _ -> ()
...
let () =
  let module Notify_on_metric_updates = Iron_protocol.Notify_on_metric_updates in
  implement_pipe_rpc
    ~log:false
