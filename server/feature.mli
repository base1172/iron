...
val set_continuous_release_status : t -> Continuous_release_status.t -> unit

(** [expect_next_base_update_exn t ~for_ expected_base] is rejected if expected_base is
    equal to the current feature base. *)
val expect_next_base_update_exn : t -> for_:User_name.t -> Rev.t -> unit
...
val set_has_bookmark
  ...
  -> _ Query.t
  -> compilation_status:
       [ `Update_with of Hydra_compilation_status.t | `Keep_any_known_value ]
  -> unit

val set_properties : t -> _ Query.t -> Properties.t -> unit
val set_review_is_enabled : t -> _ Query.t -> bool -> unit
val set_crs_are_enabled : t -> _ Query.t -> bool -> unit
val set_crs_shown_in_todo_only_for_users_reviewing : t -> _ Query.t -> bool -> unit
val set_xcrs_shown_in_todo_only_for_users_reviewing : t -> _ Query.t -> bool -> unit
val set_seconder : t -> _ Query.t -> User_name.t option -> unit Or_error.t
val check_cached_attributes : t -> ignore_diffs_in_errors:bool -> unit Or_error.t
...
val set_latest_release : t -> Latest_release.t -> unit
...
val include_released_feature : t -> Released_feature.t -> unit
val clear_included_features : t -> unit
