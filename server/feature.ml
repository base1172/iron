module Stable = struct
  ...
  module Cr_soons_or_pending = struct
    module V1 = struct
      type t = Cr_soons.In_feature.V1.t Or_error.V1.t Or_pending.V1.t [@@deriving sexp]
    end
  end
  ...
  module Creation = struct
    module V1 = struct
      type t = Attributes.V1.t Query.V1.t [@@deriving sexp]
    end
  end
  ...
  module Remove_properties = struct
    module V1 = struct
      ...
      (* Remove_properties used to take Property.t list. The t_of_sexp function below
         ...
         duplicates. *)
      let t_of_sexp sexp =
        sexp |> [%of_sexp: Property.V1.t list] |> Unstable.Property.Set.of_list
      ;;
    end
  end
  ...
  module Diff_from_base_to_tip = struct
    module V2 = struct
      type t = Diff2s.V2.t Or_error.V1.t Or_pending.V1.t [@@deriving sexp]
    end
  end
end
...
module Diff_from_base_to_tip = struct
  ...
  let invariant t = Or_pending.invariant (Or_error.invariant Diff2s.invariant) t
end
...
module Persist = struct
  ...
  module Diff4s_file = struct
    module V2 = struct
      ...
      type t = Diff4.V2.t list Or_error.V1.t Or_pending.V1.t [@@deriving sexp]
    end
...
let check_cached_attributes t ~ignore_diffs_in_errors =
  ...
  |> List.map ~f:(fun (Cached.T cached, tag) ->
       Or_error.tag (Cached.check cached ~ignore_diffs_in_errors) ~tag)
...
let to_protocol
  ...
  =
  { Iron_protocol.Feature.feature_id = t.feature_id
...
let to_released_feature_exn t ~query ~tagged_tip =
      ...
      then
        raise_s
          [%sexp
            ...
            , { feature_tip = (t.tip : Rev.t); tagged_tip : Rev.t }];
      tagged_tip
  in
  let seconder =
  ...
  in
  { Released_feature.feature_id = t.feature_id
  ; feature_path = t.feature_path
  ; description = description t
  ; owners = t.owners
  ; whole_feature_followers = t.whole_feature_followers
...
let needs_bookmark_update t =
  ...
  | None -> false
  | Some hydra_master_state -> not (Rev.has_prefix t.tip hydra_master_state.tip)
;;
...
let invariant t =
  Invariant.invariant [%here] t [%sexp_of: t] (fun () ->
    ...
    Fields.iter
      ~feature_id:ignore
      ~feature_path:
        (check (fun feature_path ->
           Feature_path.invariant feature_path;
           [%test_result: bool]
             (is_some (remote_repo_path t))
      ...
      ~next_base_update_expected:
        (check
           (Option.iter ~f:(function
              | { Next_base_update_expected.update_expected =
                    { rev; by = _; expected_since = _ } as update_expected
                ; expiration
                }
              ->
      ...
      ~base_facts:
        (check
           (Or_pending.invariant (fun base_facts ->
      ...
      ~next_bookmark_update:
        (check (fun next_bookmark_update ->
           assert (
             Next_bookmark_update.same_variant
               next_bookmark_update
      ...
      ~tip_facts:
        (check
           (Or_pending.invariant (fun tip_facts ->
              Rev_facts.invariant tip_facts;
              assert (Rev.equal_node_hash t.tip (Rev_facts.rev tip_facts)))))
      ~base_is_ancestor_of_tip:
        (check
           (Or_pending.invariant (fun base_is_ancestor_of_tip ->
              assert (
                is_ok
                  (Rev_facts.Is_ancestor.check
                     base_is_ancestor_of_tip
                     ~ancestor:t.base
                     ~descendant:t.tip)))))
      ...
      ~queries:
        (check (fun queries -> List.iter queries ~f:(Query.invariant Action.invariant)))
      ...
      ~num_lines:
        (check
           (Or_pending.invariant
              (Or_error.invariant (fun num_lines -> assert (num_lines >= 0)))))
      ~properties:ignore
...
let set_diffs_internal t diff_from_base_to_tip diff4s =
  ...
  t.indexed_diff4s <- Or_pending.map diff4s ~f:(Or_error.map ~f:Indexed_diff4s.create);
  t.num_lines
    <- Or_pending.map
         diff_from_base_to_tip
         ~f:(Or_error.map ~f:(fun diff -> Diff2s.num_lines_for_feature_size diff))
;;
...
let set_diffs t diff_from_base_to_tip diff4s =
  ...
  Serializer.set_contents
    serializer
    ~file:diff_from_base_to_tip_file
    diff_from_base_to_tip
    (module Persist.Diff_from_base_to_tip);
  Serializer.set_contents serializer ~file:diff4s_file diff4s (module Persist.Diff4s_file)
;;
...
let set_allow_review_for t allow_review_for =
  ...
  Serializer.set_contents
    (serializer_exn t)
    ~file:allow_review_for_file
    allow_review_for
    (module Persist.Allow_review_for)
;;

let set_cr_soons_internal t cr_soons = t.cr_soons <- cr_soons
...
let set_cr_soons t cr_soons =
  ...
  Serializer.set_contents
    ...
    ~file:cr_soons_file
    cr_soons
    (module Persist.Cr_soons_or_pending)
;;

let set_latest_release_internal t latest_release = t.latest_release <- Some latest_release

let set_latest_release t latest_release =
  set_latest_release_internal t latest_release;
  Serializer.set_contents
    (serializer_exn t)
    ~file:latest_release_file
    latest_release
    (module Persist.Latest_release)
;;

let create_internal creation ~dynamic_upgrade_state ~serializer =
  let { Attributes.feature_id
      ; feature_path
      ; owners
      ; is_permanent
      ; description
...
let create_exn
  ...
  then failwith "a root feature must specify a remote repo path"
    ...
    let t =
      create_internal creation ~dynamic_upgrade_state ~serializer:(Some serializer)
    in
    ...
    Serializer.set_contents
      ...
      ~file:creation_file
      creation
      (module Persist.Creation);
    t)
;;

let review_goal t =
  match t.base_facts, t.tip_facts, t.base_is_ancestor_of_tip, t.diff_from_base_to_tip with
  | ( Known base_facts
    , Known tip_facts
    ...
    Or_error.bind diff_from_base_to_tip ~f:(fun diff_from_base_to_tip ->
      Review_goal.create
        ...
        ~tip_facts
        ~base_is_ancestor_of_tip)
  | _ -> error_string "no review goal because the feature is broken or pending"
;;
...
let include_released_feature t released_feature =
  Serializer.append_to
    (serializer_exn t)
    ~file:included_features_file
    released_feature
    (module Persist.Included_feature);
...
let expect_next_base_update_exn t ~for_ rev =
  ...
  then
    raise_s
      [%sexp
        "invalid expected next base.  This is already the feature's base", (rev : Rev.t)]
  else (
    set_next_base_update_internal
      t
      (Update_expected { rev; by = for_; expected_since = Time.now () });
    invalidate_dependents t)
;;
...
let set_lines_required_to_separate_ddiff_hunks
  t
  query
  lines_required_to_separate_ddiff_hunks
  =
  ...
  else if lines_required_to_separate_ddiff_hunks < 0
  then
    Or_error.error_s
      [%sexp
...
let set_inheritable_who_can_release_into_me t query who =
  ...
  Feature_inheritable_attributes.set_who_can_release_into_me t.inheritable_attributes who
;;

let set_inheritable_send_email_to t query whom =
  record t query (`Set_inheritable_send_email_to whom);
  Feature_inheritable_attributes.set_send_email_to t.inheritable_attributes whom
;;

let set_inheritable_send_email_upon t query upon =
  record t query (`Set_inheritable_send_email_upon upon);
  Feature_inheritable_attributes.set_send_email_upon t.inheritable_attributes upon
;;
...
let apply_bookmark_update t bookmark_update =
  let latest_bookmark_update =
    ...
    | Error _ as e -> e
    | Ok { Bookmark_update.base_facts; tip; tip_facts; base_is_ancestor_of_tip } ->
      t.bounded_hydra_retry <- Bounded_hydra_retry.empty;
  ...
  (match t.hydra_master_state with
   | None -> ()
   | Some
       { tip = last_known_hydra_tip; status = _; override_n_pending_status_if_needed = _ }
     ->
...
let update_bookmark t query =
  let { Iron_protocol.Update_bookmark.Action.info; _ } = Query.action query in
...
let synchronize_with_hydra t ~hydra_tip ~hydra_status =
  let hydra_status, override_n_pending_status_if_needed =
    ...
    | ( Some { tip; override_n_pending_status_if_needed = n; status = `Done }
      ...
      when Node_hash.First_12.equal tip hydra_tip && n > 0 -> `Done, n - 1
    | _ -> hydra_status, 0
  in
  let hydra_master_state : Hydra_master_state.t =
    { status = hydra_status; tip = hydra_tip; override_n_pending_status_if_needed }
  in
...
let remove_properties t query props =
  ...
  then (
    ...
    Ok ())
  else error "unknown properties" unknown_props [%sexp_of: Property.Set.t]
;;
...
let remove_inheritable_properties t query props =
  ...
  let unknown_props = Set.filter props ~f:(fun prop -> not (Map.mem properties prop)) in
  if Set.is_empty unknown_props
  then (
...
let lock t ~query ~for_ ~lock_name ~reason ~is_permanent =
  match
    ...
    | Create_child | Rebase | Release | Release_into | Rename -> Ok ()
    | Second ->
...
let unlock t ~query ~for_ ~lock_name ~even_if_permanent =
  Or_error.map
    (Feature_locks.unlock t.locks ~for_ ~lock_name ~even_if_permanent)
...
let apply_change_internal t query (action : Action.t) =
  ...
  | `Set_base rev ->
    set_base t query rev;
    Ok ()
  | `Set_feature_path feature_path ->
    set_feature_path t query feature_path;
    Ok ()
  | `Set_description description ->
    set_description t query description;
    Ok ()
  ...
  | `Set_whole_feature_reviewers users ->
    ...
    Ok ()
  | `Set_has_bookmark bool ->
    set_has_bookmark_internal t query bool;
    Ok ()
  | `Set_is_permanent bool ->
    set_is_permanent t query bool;
    Ok ()
  | `Set_owners users -> set_owners t query users
  | `Set_properties alist ->
    set_properties t query alist;
    Ok ()
  | `Set_review_is_enabled bool ->
    set_review_is_enabled t query bool;
    Ok ()
  | `Set_crs_are_enabled bool ->
    set_crs_are_enabled t query bool;
    Ok ()
  | `Set_crs_shown_in_todo_only_for_users_reviewing bool ->
    set_crs_shown_in_todo_only_for_users_reviewing t query bool;
    Ok ()
  | `Set_xcrs_shown_in_todo_only_for_users_reviewing bool ->
    set_xcrs_shown_in_todo_only_for_users_reviewing t query bool;
    Ok ()
  | `Set_seconder user_option -> set_seconder t query user_option
  | `Set_reviewing reviewing ->
    set_reviewing t query reviewing;
    Ok ()
  ...
  | `Set_inheritable_properties properties ->
    set_inheritable_properties t query properties;
    Ok ()
  | `Set_inheritable_owners owners ->
    set_inheritable_owners t query owners;
    Ok ()
  | `Set_inheritable_release_process how ->
    set_inheritable_release_process t query how;
    Ok ()
  | `Set_inheritable_whole_feature_followers followers ->
    set_inheritable_whole_feature_followers t query followers;
    Ok ()
  | `Set_inheritable_whole_feature_reviewers reviewers ->
    set_inheritable_whole_feature_reviewers t query reviewers;
    Ok ()
  | `Set_inheritable_crs_shown_in_todo_only_for_users_reviewing option ->
    set_inheritable_crs_shown_in_todo_only_for_users_reviewing t query option;
    Ok ()
  | `Set_inheritable_xcrs_shown_in_todo_only_for_users_reviewing option ->
    set_inheritable_xcrs_shown_in_todo_only_for_users_reviewing t query option;
    Ok ()
  | `Set_inheritable_who_can_release_into_me who ->
    set_inheritable_who_can_release_into_me t query who;
    Ok ()
  | `Set_inheritable_send_email_to whom ->
    set_inheritable_send_email_to t query whom;
    Ok ()
  | `Set_inheritable_send_email_upon upon ->
    set_inheritable_send_email_upon t query upon;
    Ok ()
  | `Set_lines_required_to_separate_ddiff_hunks lines_required_to_separate_ddiff_hunks ->
    set_lines_required_to_separate_ddiff_hunks
      ...
      query
      lines_required_to_separate_ddiff_hunks
  | `Set_release_process how ->
    set_release_process t query how;
    Ok ()
  | `Set_who_can_release_into_me who ->
    set_who_can_release_into_me t query who;
    Ok ()
  | `Set_send_release_email_to whom ->
    set_send_release_email_to t query whom;
    Ok ()
  | `Set_should_send_release_email bool ->
    set_should_send_release_email t query bool;
    Ok ()
  | `Set_send_email_to whom ->
    set_send_email_to t query whom;
    Ok ()
  | `Set_send_email_upon upon ->
    set_send_email_upon t query upon;
    Ok ()
;;
...
module Add_remove (T : sig
  type t [@@deriving sexp_of]

  include Comparable.S_plain with type t := t
end) : sig
  val add_list : T.t list -> T.t list -> (T.t list -> 'a) -> desc:string -> 'a Or_error.t
  ...
  val add : T.Set.t -> T.Set.t -> (T.Set.t -> 'a) -> desc:string -> 'a Or_error.t
  val remove : T.Set.t -> T.Set.t -> (T.Set.t -> 'a) -> desc:string -> 'a Or_error.t
end = struct
  ...
  let action_for (update : Change_feature.Update.t) =
    ...
    | `Remove_owners users ->
      User_names.remove_list
        ...
        (Set.to_list users)
        set_owners
        ~desc:"not currently an owner"
    | `Add_whole_feature_followers users ->
      User_names.add
        t.whole_feature_followers
        users
        set_whole_feature_followers
    ...
    | `Add_whole_feature_reviewers users ->
      User_names.add
        t.whole_feature_reviewers
        users
        set_whole_feature_reviewers
    ...
    | `Add_inheritable_owners owners ->
      User_names.add_list
        ...
        owners
        set_inheritable_owners
        ~desc:"is already inheritable owner"
    ...
    | `Add_inheritable_whole_feature_followers whole_feature_followers ->
      User_names.add
        (Feature_inheritable_attributes.whole_feature_followers t.inheritable_attributes)
        whole_feature_followers
        set_inheritable_whole_feature_followers
    ...
    | `Add_inheritable_whole_feature_reviewers whole_feature_reviewers ->
      User_names.add
        (Feature_inheritable_attributes.whole_feature_reviewers t.inheritable_attributes)
        whole_feature_reviewers
        set_inheritable_whole_feature_reviewers
    ...
    | `Add_inheritable_send_email_upon send_email_upon ->
      Send_email_upons.add
        ...
        send_email_upon
        set_inheritable_send_email_upon
    ...
    | ( `Remove_properties _
      ...
      | `Set_whole_feature_reviewers _ ) as update -> Ok (update :> Action.t)
  in
...
let deserializer ~dynamic_upgrade_state =
  Deserializer.with_serializer (fun serializer ->
    ...
    let%map_open () = return ()
    and cr_soons = one (module Persist.Cr_soons_or_pending) ~in_file:cr_soons_file
    and creation = one (module Persist.Creation) ~in_file:creation_file
