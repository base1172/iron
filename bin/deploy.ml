...
let fork_exec_wait ~prog ~args =
  Core.Unix.waitpid_exn (Core.Unix.fork_exec ~prog ~argv:(prog :: args) ())
;;
...
let deploy =
  Command.basic
    ...
    Command.Spec.(
      ...
      +> flag
           "-fe"
           (optional_with_default "self" file)
           ~doc:"(EXE|self|none) which executable to roll (defaults to self)"
      +> flag
           "-hgrc"
           (optional_with_default "default" file)
...
let deploy_check_obligations =
  Command.basic
    ...
    Command.Spec.(empty +> anon ("file" %: file) ++ generic_deploy_arguments)
