...
let server_state_invariant =
  Command.group
    ~summary:"check server-state invariant"
...
let internal =
  Command.group
    ~summary:"various commands for Iron developers"
    ...
    ; ( "clear-bookmarks-without-feature"
      , Iron_client.Cmd_clear_bookmarks_without_feature.command )
    ...
    ; "invariant", invariant
    ; ( "mark-fully-reviewed"
      , Iron_client.Cmd_has_moved.command
    ...
    ; ( "show-repo-for-hg-operations"
      , Iron_client.Cmd_workspace.repo_for_hg_operations_command )
...
let cmd_top =
  Command.group
    ~summary:"manage feature-based code development"
    [ "admin", Admin.command
