...
let browse_disk_quota =
  Command.basic'
    ~summary:"catalog browse the Iron disk quota"
    (let open Command.Let_syntax in
    let%map_open () = return () in
    fun () ->
      let prog = "catalog" in
      never_returns
        (Unix.exec () ~prog ~argv:[ prog; "browse"; Fe_config.catalog_browse_path ]))
;;

let build_info =
  Command.async'
    ~summary:"output Iron server executable's build info"
    (let open Command.Let_syntax in
    let%map_open () = return () in
    fun () -> Iron_client.Cmd_dump.dump Build_info)
;;

let last_hydra_error =
  Command.basic'
    ~summary:"show the last error hydra had talking to Iron server"
    (let open Command.Let_syntax in
    let%map_open () = return () in
    fun () ->
      let prog = "bash" in
...
let users =
  Command.group
    ~summary:"commands about the various sets of user names"
...
let version =
  Command.async'
    ~summary:"output Iron server executable's hg version info"
    (let open Command.Let_syntax in
    let%map_open () = return () in
    fun () -> Iron_client.Cmd_dump.dump Version)
;;

let command =
  Command.group
    ~summary:"control Iron server, deploy exe's, see disk usage, etc."
    ...
    ; ( "server"
      , Command.group
          ~summary:"commands to poke/prod/control the server"
