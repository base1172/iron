...
module type T = sig
  ...
  val feature_arg_type
    :  match_existing_feature:bool
    -> Feature_path.t Or_error.t Arg_type.t
  ...
  val feature_path_flagged : label:string -> doc:string -> Feature_path.t Or_error.t t
  ...
  val metric_name_regex_list_option : doc:string -> Regex.t list Or_error.t option t
  ...
  val rev : Raw_rev.t t
  val send_email_upon : switch:string -> doc:string -> Send_email_upon.Set.t option t
...
module type Iron_param = sig
  ...
  module Let_syntax : sig
    module Let_syntax : sig
      ...
      module Open_on_rhs : module type of struct
        include T
      end
    end
  end
