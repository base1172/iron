...
struct
  ...
  type t = ?connection:Connection.t -> Action.t -> Reaction.t Or_error.t Deferred.t
end
...
let command_rpc
  ...
  (module M : Iron_command_rpc.S with type action = action and type reaction = reaction)
  =
...
let get_feature_maybe_archived =
  rpc_to_server (module Iron_protocol.Get_feature.Maybe_archived)
;;
...
let notify_on_descendant_updates =
  pipe_rpc_to_server (module Iron_protocol.Notify_on_descendant_updates)
;;

let notify_on_feature_updates =
  pipe_rpc_to_server (module Iron_protocol.Notify_on_feature_updates)
;;
...
let revision_is_fully_reviewed =
  rpc_to_server (module Iron_protocol.Revision_is_fully_reviewed)
;;
...
let show_supported_iron_rpcs_command =
  Command.async'
    ~summary:"output a table of RPCs supported by Iron public lib"
    ...
    fun () ->
      ...
      let rpcs names rpcs =
        ...
        List.filter (force rpcs) ~f:(fun rpc -> Set.mem names (Rpc_description.name rpc))
        |> List.sort ~compare:Rpc_description.Compare_by_name_and_version.compare
      in
      ...
      in
      let command_rpcs = rpcs command_rpc_names Iron_command_rpc.rpc_descriptions in
      if as_sexp
      then
        [%sexp
