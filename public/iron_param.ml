...
module T = struct
  ...
  let path_in_repo_arg_type =
    Arg_type.map Filename_unix.arg_type ~f:(fun file ->
      match Repo_root.program_started_in with
      | Error _ -> Path_in_repo.of_string file
      | Ok repo_root ->
        Repo_root.relativize_exn
          repo_root
          (ok_exn
             (Abspath.simplify_dotdots_syntax
  ...
  let blang_arg_type_with_completion
    (type a)
    (module Blangable : Blangable with type t = a)
    get_universe
    =
    let complete _ ~part =
    ...
    in
    Arg_type.create ~complete (fun str ->
      Sexp.of_string_conv_exn str [%of_sexp: Blangable.t Blang.t])
  ;;
  ...
  let make_enum ~f switch ~doc m =
    ...
    let choices =
      enum
      |> List.map ~f:fst
      |> List.sort ~compare:String.alphabetic_compare
      |> String.concat ~sep:" | "
    in
    let doc =
  ...
  let no_arg_flag ?aliases switch ~doc =
    let doc =
      if String.length doc = 0 || Char.equal doc.[0] ' ' then doc else concat [ " "; doc ]
    in
  ...
  let create_catch_up_for_me =
    ...
    fun ~is_reviewing_for ->
      ...
      then
        Or_error.errorf
          "Cannot use [%s] when reviewing for other users only."
          Switch.create_catch_up_for_me
      else Ok create_catch_up_for_me
  ;;

  let resolved_file_path_arg_type =
    Arg_type.map Filename_unix.arg_type ~f:(fun string ->
  ...
  let rev_arg_type : Raw_rev.t Arg_type.t = Arg_type.create create_rev_from_local_repo
  ...
  let complete_or_error types _ ~part:prefix =
    (* If any problem, give up: no completions. *)
    match
      Thread_safe.block_on_async (fun () ->
        Iron_protocol.Complete.rpc_to_server_exn { types; prefix })
    with
    | Ok _ as completions -> completions
    | Error exn ->
      Or_error.error_s
        [%sexp
          "unless the server is down, please report this bug in Iron server's completion"
          , { types : Iron_protocol.Complete.Type.t list; prefix : string; exn : Exn.t }]
  ;;

  let complete types hmap ~part =
    match complete_or_error types hmap ~part with
    | Ok completions -> completions
    | Error err -> completion_problem err
  ;;

  let complete_in_sequence types umap ~part =
  ...
  let user_name =
    Arg_type.create User_name.of_string ~complete:(complete [ User_info Existing_user ])
  ;;
  ...
  let find_preferred_feature_path_heuristic ~matching_features ~partial_name_prefix
    ...
    | _ :: _ :: _ ->
      ...
      (match
         List.find_map [ strict_equality; strict_suffix; project ] ~f:(fun pred ->
  ...
  let%test_unit _ =
    ...
    in
    check [ "jane/oculus"; "jane/grass/oculus" ] "oculus" ~expect:"jane/oculus";
    check [ "jane/fe"; "jane/fe/a" ] "fe" ~expect:"jane/fe"
  ;;
  ...
  let feature_path_of_string_or_partial_name_internal_exn
    ?(parse_partial_name_if_no_match = false)
    partial_name_prefix
    ~namespace
    =
    let open Async in
    ...
    in
    let matching_features = List.sort matching_features ~compare:Feature_path.compare in
    match
      find_preferred_feature_path_heuristic ~matching_features ~partial_name_prefix
    with
    | No_match ->
      let fail () = raise_s [%sexp "no such feature", (partial_name_prefix : string)] in
    ...
    | Cannot_disambiguate ->
      (match Repo_root.program_started_in with
       | Error _ ->
         raise_s
           [%sexp
             "cannot disambiguate among features"
             , (matching_features : Feature_path.t list)]
       | Ok repo_root ->
         let%map desired_family =
         ...
         (match desired_family with
          | Error error ->
            raise_s
              [%sexp
                "cannot disambiguate among features, and cannot determine your repo \
                 family"
                , { matching_features : Feature_path.t list; error : Error.t }]
          | Ok desired_family ->
            ...
            (match
             ...
             | No_match ->
               raise_s
                 [%sexp
                   (sprintf
                      "cannot disambiguate among features, and none of them are in the \
                       %s repo"
                      (Feature_name.to_string desired_family)
                     : string)
             ...
             | Cannot_disambiguate ->
               raise_s
                 [%sexp
                   (sprintf
                      "cannot disambiguate among features in the %s repo"
                      (Feature_name.to_string desired_family)
                     : string)
                   , { matching_features_in_this_repo : Feature_path.t list
                     ; matching_features_in_other_repos : Feature_path.t list
  ...
  let feature_path_of_string_or_partial_name_exn partial_name_prefix ~namespace =
    ...
    then
      feature_path_of_string_or_partial_name_internal_exn partial_name_prefix ~namespace
  ...
  let feature_arg_type ~match_existing_feature =
    Arg_type.create
      ...
      ~complete:
        (complete
           [ (if match_existing_feature then Feature_path else Absolute_feature_path) ])
  ;;
  ...
  let root_feature_arg_type =
    Arg_type.create Feature_name.of_string ~complete:(complete [ Root_feature_path ])
  ;;
  ...
  let clone_of_root_feature_of ~doc =
    ...
    |> map ~f:(fun list ->
         ...
         |> Or_error.map ~f:(fun list ->
              list |> List.map ~f:Feature_path.root |> Feature_name.Set.of_list))
  ;;
  ...
  let feature_path_option_gen ~match_existing_feature =
    map
      (anon (maybe (feature_anons_gen ~match_existing_feature)))
      ~f:
        (function
         | Some (Error _ as e) -> e
         | Some (Ok p) -> Ok (Some p)
  ...
  let absolute_feature_path_option = feature_path_option_gen ~match_existing_feature:false
  ...
  let feature_path_option = feature_path_option_gen ~match_existing_feature:true
  ...
  let feature_path_flagged_listed ~label ~doc =
    map
      (flag label (listed (feature_arg_type ~match_existing_feature:true)) ~doc)
  ...
  let current_bookmark () =
    ...
    let result =
      ...
      else Deferred.Or_error.error_string "feature-path inference is disabled via [.ferc]"
    in
    match%map result with
    | Ok _ as x -> x
    | Error e -> error "could not determine feature you want to use" e [%sexp_of: Error.t]
  ;;

  let make_feature_path_or_current_bookmark feature_anons =
    map
      (anon (maybe feature_anons))
      ~f:
        (function
  ...
  module Maybe_archived_feature_spec : sig
    ...
    let complete_with_archived_flag_u_key =
      Univ_map.With_default.Key.create
        ~name:Switch.archived
        [%sexp_of: bool]
        ~default:false
    ;;
    ...
    let complete_with_existing_or_most_recently_archived_flag_u_key =
      Univ_map.With_default.Key.create
        ~name:Switch.existing_or_most_recently_archived
        [%sexp_of: bool]
        ~default:false
    ;;
    ...
    let complete_with_archive_flag hmap =
      ...
      else if Univ_map.With_default.find
                hmap
    ...
    let resolve_maybe_archived_feature_spec_exn
      { Maybe_archived_feature_spec.Command_line.feature_spec; namespace }
      =
      let open Async in
      match feature_spec with
      | (`Feature_id _ | `Feature_path _) as feature_spec ->
        return { Maybe_archived_feature_spec.feature_spec; namespace }
      | `Partial_name partial_name ->
        ...
        let%map feature_path =
          feature_path_of_string_or_partial_name_internal_exn
            ~parse_partial_name_if_no_match:true
            partial_name
            ~namespace
        in
        { Maybe_archived_feature_spec.feature_spec = `Feature_path feature_path
        ; namespace
        }
      | `Current_bookmark ->
        let%map feature_path = current_bookmark () in
        { Maybe_archived_feature_spec.feature_spec = `Feature_path (ok_exn feature_path)
        ; namespace
        }
    ;;
    ...
    let maybe_archived_feature =
      return
        (fun raw_feature_path archived_flag existing_or_most_recently_archived_flag ->
        Or_error.try_with (fun () ->
          ...
          let namespace =
            ...
            | true, true ->
              failwithf
                "The switches %s and %s are incompatible."
                Switch.archived
                Switch.existing_or_most_recently_archived
                ()
            | true, false -> `Archived
      ...
      <*> flag
            Switch.archived
            ~doc:" lookup only among archived features"
            (no_arg_register ~key:complete_with_archived_flag_u_key ~value:true)
      <*> flag
            Switch.existing_or_most_recently_archived
            ~doc:
              " try finding the most recently archived feature if the feature does not \
               exist"
            (no_arg_register
  ...
  let anon_feature_paths = map (anon (sequence feature_anons)) ~f:Or_error.combine_errors

  let included_features_order =
    let release_time_switch = "-order-included-features-by-release-time"
    ...
    and release_time_decreasing =
      no_arg_flag
        release_time_decreasing_switch
  ...
  let include_active_cr_soons =
    no_arg_flag
      Switch.include_active_cr_soons
  ...
  let interactive =
    map
      (bool_with
         "-interactive"
         ~default:!Async_interactive.interactive
  ...
  let description =
    flag "-description" (optional string) ~doc:"TEXT what the feature will accomplish"
  ;;

  let display_ascii =
    map
      ~f:(fun b -> b || Iron_options.display_ascii_always)
      (no_arg_flag
         "-display-ascii"
         ~doc:"display ASCII characters, with no ANSI color escapes or unicode characters")
  ;;

  let terminal_width = Unix_extended.terminal_width
  ...
  let even_if_some_files_are_already_reviewed =
    no_arg_flag
      Switch.even_if_some_files_are_already_reviewed
  ...
  let diff4_in_session_ids =
    flag "-diff-id" ~doc:"DIFF_ID diff id" (listed Diff4_in_session.Id.arg_type)
  ;;
  ...
  module Which_diffs = struct
    ...
    let param =
      ...
      <*> flag
            "-file"
            (listed path_in_repo_arg_type)
            ~doc:"FILE show only specified files"
  ...
  module Which_features = struct
    ...
    let features_list features ~rec_ =
      Which_features.Features
        (List.map features ~f:(fun feature_path ->
           { Which_features.Feature.feature_path; include_descendants = rec_ }))
    ;;
    ...
    let param
      ...
      Lazy.from_fun (fun () ->
        ...
        else if List.is_empty features
        then (
          let%map features =
        ...
        else return (features_list features ~rec_))
    ;;
  end
      ...
      | `Files files ->
        ...
        let elts =
          List.concat_map files ~f:(fun path_in_repo ->
            ...
            | Some elts -> elts
            | None ->
              files_not_found := path_in_repo :: !files_not_found;
              [])
        in
           ...
           raise_s
             [%sexp
               (sprintf
                  ...
                  from
                 : string)
               , (List.sort not_found ~compare:Path_in_repo.default_review_compare
                   : Path_in_repo.t list)])
    ;;
  end
  ...
  module Review_sort = struct
    ...
    let param =
      return
        (fun using_file by_increasing_review_lines by_decreasing_review_lines build_order
        ->
        match
          using_file, by_increasing_review_lines, by_decreasing_review_lines, build_order
        with
      ...
      <*> flag
            "-sort"
            (optional resolved_file_path_arg_type)
            ~doc:"FILE order the files to be reviewed using the list in this file."
      <*> no_arg_flag
            "-sort-by-increasing-lines"
            ~doc:
              " order the files to be reviewed by increasing number of lines to review."
      <*> no_arg_flag
            "-sort-by-decreasing-lines"
            ~doc:
              " order the files to be reviewed by increasing number of lines to review."
      <*> sort_build_order
    ;;
  ...
  let context ?(default = 12) () =
    map
      (flag
         "-context"
         (optional int)
         ~doc:(sprintf "N number of lines of context to show (default %d)" default))
      ~f:
        (function
  ...
  let lines_required_to_separate_ddiff_hunks_override =
    flag
      Switch.lines_required_to_separate_ddiff_hunks
      (optional int)
  ...
  let lines_required_to_separate_ddiff_hunks_with_default =
    ...
    flag
      ...
      ~doc:
        (sprintf "INT required spacing to separate two ddiff hunks (default %d)" default)
  ;;

  let email_address_list_arg_type =
    Arg_type.comma_separated
      (Arg_type.create Email_address.of_string)
  ...
  let comma_delim_list_arg_type of_string complete =
    Arg_type.comma_separated
      ...
      (Arg_type.create of_string ~complete:(fun univ_map ~part ->
         try complete univ_map ~part with
         | exn ->
           completion_problem
             (Error.create
                "please report this bug in Iron completion"
                exn
                [%sexp_of: exn])))
  ;;
  ...
  let enum_list switch ~doc enum =
    map
      (flag switch ~doc (optional (enum_list_arg_type enum)))
      ~f:
        (function
         | None -> []
         | Some list -> list)
  ...
  let email_addresses_option ~switch =
    map
      (flag switch ~doc:"EMAIL[,EMAIL...] " (optional email_address_list_arg_type))
  ...
  let user_list_option ~switch =
    flag switch ~doc:"USER[,USER...] " (optional user_list_arg_type)
  ;;

  let users_option ~switch =
    map (user_list_option ~switch) ~f:(Option.map ~f:User_name.Set.of_list)
  ;;

  let property_list_arg_type =
    Arg_type.comma_separated
      (Arg_type.create Property.of_string)
      ~strip_whitespace:true
      ~unique_values:true
  ;;

  let properties_option ~switch ~doc = flag switch (optional property_list_arg_type) ~doc
  ...
  let inheritable_properties_set_option ~switch =
    map
      (properties_option ~switch ~doc:"ATTR[,ATTR...] ")
      ~f:(Option.map ~f:Property.Set.of_list)
  ;;
  ...
  let property_values_flag ~switch ~doc =
    ...
    map
      (flag
         switch
         ~doc:(sprintf "%s %s" syntax doc)
         (listed
            (Arg_type.create (fun x ->
               match String.lsplit2 x ~on:separator with
               | None ->
                 raise_s
                   [%sexp
                     (sprintf "expected key value pair of the form %s but got" syntax
                       : string)
                     , (x : string)]
               | Some (key, value) -> key, Sexp.of_string value))))
      ~f:
        (function
         | [] -> None
  ...
  let remote_repo_path =
    flag
      ...
      ~doc:"URL location of the shared repo (e.g., ssh://...)"
      (optional
         (Arg_type.create
  ...
  let may_modify_local_repo =
    map
      ~f:not
      (no_arg_flag
         Switch.do_not_modify_local_repo
         ~doc:"don't run hg commands that modify the local repo")
  ;;

  let do_not_show_cr_soons =
    no_arg_flag Switch.do_not_show_cr_soons ~doc:"don't show cr soons"
  ;;

  let do_not_show_unclean_workspaces =
    no_arg_flag Switch.do_not_show_unclean_workspaces ~doc:"don't show unclean workspaces"
  ;;
  ...
  let may_repartition_crs =
    map
      ~f:not
      (no_arg_flag
         "-do-not-repartition-crs"
         ~doc:"do not repartition CRs after the change")
  ;;

  let update_local_repo =
    no_arg_flag Switch.update_local_repo ~doc:"update the local repo to the feature's tip"
  ;;

  let without_enough_whole_feature_reviewers =
    no_arg_flag
      Switch.without_enough_whole_feature_reviewers
      ~doc:"proceed with fewer than two whole-feature-reviewers"
  ;;

  let lock_reason = flag "-reason" ~doc:"REASON reason for locking" (required string)
  let lock_names = enum_no_args (module Lock_name) ~doc:(fun ~name _ -> name)

  let metric_values =
    flag
      "-values"
      ~doc:"FLOAT[,FLOAT..] value"
      (required
         (Arg_type.comma_separated float ~strip_whitespace:true ~unique_values:false))
  ;;
  ...
  let metric_name_anchored_regex_list_arg_type =
    comma_delim_list_arg_type
      (fun patt -> Regex.create ("^" ^ patt ^ "$"))
      (complete [ Metric_name ])
  ;;

  let metric_name_regex_list_option ~doc =
    flag "-metrics" ~doc (optional metric_name_anchored_regex_list_arg_type)
  ...
  let for_or_all_or_all_but_default_me =
    ...
    | Some _, Some _ ->
      Or_error.errorf
        "The flags [%s] and [%s] are mutually exclusive."
        Switch.for_
        Switch.for_all_but
  ;;
end
