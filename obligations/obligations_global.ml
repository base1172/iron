  ...
  Error_context.within ~file:obligations_global (fun e ->
    ...
    let process_declaration (declaration : Declaration.t) =
      ...
      | Define_scrutiny (name, scrutiny) ->
        (match Hashtbl.add scrutinies ~key:name ~data:(e, scrutiny) with
         | `Ok -> ()
         | `Duplicate ->
      ...
      | Obligations_version format ->
        (match !obligations_version with
         ...
         | Some _ ->
           Error_context.raise_f e "multiple Obligations_version specifications" ())
      | Cr_comment_format format ->
        (match !obligations_version with
         | Some _ ->
           Error_context.raise_f e "multiple Cr_comment_format specifications" ()
         | None ->
