...
let sorted_files_alist t =
  t.by_path
  |> Hashtbl.to_alist
  |> List.sort ~compare:(fun (p1, _) (p2, _) ->
       String.alphabetic_compare (Path_in_repo.to_string p1) (Path_in_repo.to_string p2))
;;

let sexp_of_t t =
  sorted_files_alist t |> [%sexp_of: (Path_in_repo.t * Review_attributes.t) list]
;;
...
let report_unused_definitions
  (type a)
  t
  label
  (module V : Validated_string.S with type t = a)
  of_obligations_repo
  of_review_attributes
  =
  match t.obligations_repo with
  | `Fake -> ()
  | `Actual obligations_repo ->
    let unused_definitions = V.Hash_set.of_list (of_obligations_repo obligations_repo) in
    ...
    then
      raise_s
        [%sexp
          ...
          , (unused_definitions |> Hash_set.to_list |> List.sort ~compare:V.compare
              : V.t list)]
;;

let report_unused_projections t =
  report_unused_definitions
    t
    "build projections"
    (module Build_projection_name)
...
let report_unused_tags
  t
  ~(defined_in_local_repo : Obligations_repo.Defined_in_local_repo.t)
  =
  report_unused_definitions
    t
    "tags"
  ...
  | `Actual obligations_repo ->
    Deferred.List.iter
      ...
      then return ()
        ...
        | false, false ->
          raise_s
            [%sexp "missing low-review file", (low_review_file_in_repo : Path_in_repo.t)]
        | false, true ->
          raise_s
            [%sexp
              "untracked low-review file", (low_review_file_in_repo : Path_in_repo.t)]
        | true, false ->
          raise_s
            [%sexp
        ...
        | true, true ->
          let%map stated_exceptions =
            Reader.load_sexps_exn
              (Path.to_string low_review_file)
...
let report_errors
  (type x)
  singular_message
  ?(plural_message = singular_message ^ "s")
  xs
  x
  =
  ...
  | _ ->
    let xs = List.sort xs ~compare:X.compare in
    raise_s [%sexp (plural_message : string), (xs : X.t list)]
;;

let create hg ?skip_full_repo_checks ~repo_root ~dirs ~manifest ~aliases () =
  let%bind obligations_repo_or_error = Obligations_repo.load hg repo_root ~aliases in
  let obligations_version_or_error =
    Or_error.map obligations_repo_or_error ~f:(fun (repo, _) -> repo.obligations_version)
  in
  let t_or_error_def =
    Deferred.Or_error.try_with ~extract_exn:true (fun () ->
      ...
      let%bind () =
        Deferred.List.iter
          ...
          if dot_fe_exists then untracked_dot_fes := dot_fe :: !untracked_dot_fes)
      in
      ...
      let load_dot_fe dot_fe =
        ...
        let declarations =
          ...
          then (
            ...
            match%map
              Reader.load_annotated_sexps
                (Path.to_string dot_fe_path)
                Dot_fe.Declaration.of_annotated_sexp
            with
            | Error error ->
              syntax_errors := error :: !syntax_errors;
              []
      ...
      let%bind declarations_by_dot_fe =
        Deferred.List.map
          ~how:(`Max_concurrent_jobs 30)
          (Hash_set.to_list dot_fes_to_load)
          ~f:load_dot_fe
      in
      ...
      let by_path =
        ...
        |> List.concat_map ~f:(fun (dir, (dot_fe, files_in_directory)) ->
             let scrutiny_names_of_files_affected_by_dot_fe =
               Hashtbl.find_or_add
                 scrutiny_names_of_files_affected_by_dot_fe
                 dot_fe
      ...
      Hashtbl.iteri
        ...
        Hashtbl.change
          ...
          dot_fe
          ~f:
            (Option.map ~f:(fun dot_fe_attributes ->
               let obligations_read_by =
                 List.map (Hash_set.to_list scrutiny_names) ~f:(fun scrutiny_name ->
                   ...
                   | None ->
                     raise_s
                       [%sexp
                         "This [.fe.sexp] file affects file of undefined scrutiny"
                         , [%here]
                         , { dot_fe : Path_in_repo.t; scrutiny_name : Scrutiny_name.t }])
               in
               let augmented_obligations =
                 Review_obligation.and_
                   ...
                   :: obligations_read_by)
               in
               Review_attributes.with_review_obligation
                 dot_fe_attributes
                 ~review_obligation:augmented_obligations)));
      let t = { obligations_repo = `Actual obligations_repo; by_path } in
      ...
      let%bind () =
        ...
        then return ()
          ...
          then
            report_errors
              "useless .fe.sexp file"
              (useless_dot_fes |> Hash_set.to_list)
              (module Path_in_repo);
...
module Stable = struct
  module V5 = struct
    ...
    let of_model { obligations_repo; by_path } =
      match obligations_repo with
      | `Fake ->
        failwith
          "Obligations.Stable.V4.of_model: unexpected serialization of fake obligations"
      | `Actual obligations_repo ->
        { Stable_format.obligations_repo; by_path = Hashtbl.to_alist by_path }
    ;;

    let to_model { Stable_format.obligations_repo; by_path } =
