    ...
    module Unshared = struct
      ...
      let hash t =
        ...
        Fields.fold
          ...
          ~build_projections:
            (hash (Build_projection_name.V1.Map.hash Build_projection.V2.hash))
...
module Defined_in_local_repo = struct
  type t = { tags : Tag.Set.t } [@@deriving sexp_of]
end
...
let check_duplicate_global e label sexp_of_a duplicate_set =
  ...
  then
    Error_context.raise_s
      ...
      [%sexp
        (sprintf
           "remove %s from .fe/obligations-repo.sexp that are also in \
            .fe/obligations-global.sexp (which comes from the scaffolded repo)"
           label
          : string)
        , (duplicate_set : a)]
;;
...
let eval
  ...
  ~aliases
  =
  let { Obligations_global.disallow_useless_dot_fe = global_disallow_useless_dot_fe
  ...
  Error_context.within ~file:obligations_repo (fun e ->
    ...
    List.iter declarations ~f:(fun declaration ->
      ...
      | Use_cr_spec ->
        Error_context.raise_s
          e
          [%sexp
            "Use_cr_spec is deprecated.  Old projections format is no longer supported"]
    ...
    let groups =
      ...
      check_duplicate_global
        e
        "groups"
        [%sexp_of: Group_name.Set.t]
        (Set.inter (Set.of_map_keys local_groups) (Set.of_map_keys global_groups));
    ...
    let scrutinies =
      ...
      Queue.iter define_scrutiny ~f:(fun (e, name, syntax) ->
        ...
        in
        match Hashtbl.add local_scrutinies ~key:name ~data:scrutiny with
        | `Ok -> ()
        | `Duplicate ->
          Error_context.raise_s
            e
      ...
      check_duplicate_global
        e
        "scrutinies"
        [%sexp_of: Scrutiny_name.Set.t]
        (Set.inter (Set.of_map_keys local_scrutinies) (Set.of_map_keys global_scrutinies));
      ...
      Queue.iter
        ...
        | Some default_scrutiny ->
          Hashtbl.add_exn
            ...
            ~key:name
            ~data:
              (Build_projection.create ~name ~default_scrutiny ~require_low_review_file));
    ...
    , { Defined_in_local_repo.tags = local_tags } ))
;;
...
let load (module Hg : Hg_dependency.S) repo_root ~aliases =
  ...
  let lose e file msg = Error (Error.tag_arg e msg file [%sexp_of: Abspath.t]) in
  ...
  match%bind
    Reader.load_annotated_sexps
      (Abspath.to_string obligations_repo)
  ...
  | Error err ->
    return
      (lose
         err
         repo_root_as_abspath
         (sprintf
            "Error loading %s file at repo root"
  ...
  | Ok repo_declarations ->
    ...
    let abspaths_to_look_for =
      List.map relpaths_to_look_for ~f:(fun relpath ->
        Abspath.append (Repo_root.to_abspath repo_root) relpath)
    in
      ...
      | None ->
        ...
        (match%map
         ...
         | `No_scaffold scaffold_sexp ->
           Or_error.error
             (sprintf
                !"Error finding %{Relpath}: looked for these files in turn, but none \
                  exist"
                obligations_global_path)
         ...
         | `Scaffold_exists (Error (_ as error)) ->
           Or_error.error
             (sprintf
                !"Error loading %{Relpath} via %{File_name}"
                obligations_global_path
                File_name.scaffold_sexp)
             error
             [%sexp_of: Error.t]
    ...
    let%bind gspec =
      ...
      Obligations_global.eval
        decls
        ~obligations_global:(Relpath obligations_global_path)
        ~aliases
    in
    eval gspec repo_declarations ~obligations_repo:(Abspath obligations_repo) ~aliases
;;
