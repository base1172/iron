...
let check_users t ~known_users =
  ...
  |> List.filter_map ~f:(fun (group, unknown_users) ->
       ...
       then None
       else Some [%sexp { group : Group_name.t; unknown_users : Unresolved_name.Set.t }])
  |> function
  | [] -> Ok ()
  | _ :: _ as errors ->
    Or_error.error "groups with unknown users" errors [%sexp_of: Sexp.t list]
;;

let eval_exn e t group =
  match Map.find t group with
  | Some users -> users
  | None -> Error_context.raise_s e [%sexp "no such group", (group : Group_name.t)]
;;
...
let get_users_exn t blang =
  ...
  then
    raise_s
      [%sexp
        (sprintf "undefined group%s" (if Set.length undefined_groups > 1 then "s" else "")
          : string)
  ...
  in
  let to_set group = Option.value (Map.find t group) ~default:Unresolved_name.Set.empty in
