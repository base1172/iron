...
module Partial_attributes = struct
  type t =
    ...
    ; unapplied_attributes : (Error_context.t[@sexp.opaque]) list
    }
         ...
         (match Map.find obligations_repo.scrutinies scrutiny_name with
          ...
          | Some
            ...
            let fewer_than_min_reviewers =
              num_reviewers_lower_bound < min_file_reviewers
            in
            let more_than_max_reviewers =
              num_reviewers_lower_bound > max_file_reviewers
            in
            ...
            then
              error
                (sprintf
                   "there are %s and only %d required, so must have \
...
let eval
  ...
  ~aliases
  =
  let { Obligations_repo.users = allowed_users
      ; groups = known_groups
      ; tags = known_tags
      ; scrutinies
      ; build_projections
  ...
  let rec eval_one (u : Declaration.u) e ~partial_attributes ~depth =
    ...
    | Apply_to file_set ->
      ...
      then
        Error_context.raise_s e [%sexp "attributes are applied to an empty list of files"];
      ...
      then
        Error_context.raise_s
          ...
          [%sexp
            (sprintf
               "undefined build %s"
               (one_or_more "projection" (List.length undefined))
              : string)
    ...
    | Tags tags ->
      (match List.map tags ~f:(fun t -> t, ()) |> Tag.Table.of_alist_report_all_dups with
       | `Ok _ -> ()
       | `Duplicate_keys ts ->
      ...
      then
        Error_context.raise_s
          e
          [%sexp
            (sprintf "undefined %s" (one_or_more "tag" (List.length undefined)) : string)
  ...
  Error_context.within ~file:dot_fe (fun e ->
    ...
    let alist =
      List.filter_map
        (Hashtbl.to_alist partial_attributes_by_file_name)
        ~f:(fun (file_name, partial_attributes) ->
        match
          Or_error.try_with_join (fun () ->
            Partial_attributes.to_attributes
              partial_attributes
              ~obligations_repo
              file_name)
        with
        ...
        | Error error ->
          problems := (file_name, error) :: !problems;
          None)
    in
      ...
      Error_context.raise_s
        ...
        [%sexp
          ...
          , (List.map !problems ~f:(fun (file, error) ->
               [%sexp { file : File_name.t; error : Error.t }])
              : Sexp.t list)];
