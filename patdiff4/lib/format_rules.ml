...
let outer_line_change ~style ~name text color =
  let pre = Rule.Annex.create ~styles:Style.[ Bold; Bg color; Fg Color.White ] text in
...
let outer_default =
  { inner_default with
    Rules.line_old = outer_line_change ~name:"line_old" ~style:[] "--" Color.Magenta
  ...
  let config =
    ...
    | Error as_new_config_exn ->
      ...
      | Error as_old_config_exn ->
        raise_s
          [%sexp
            "invalid config", { as_new_config_exn : Exn.t; as_old_config_exn : Exn.t }])
  in
  Patdiff_lib.Configuration.parse config |> fun t -> t.Patdiff_lib.Configuration.rules
;;
