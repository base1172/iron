...
let make_view_configuration () =
  default_view_configuration |> Map.to_alist |> Diff4_class.Table.of_alist_exn
;;
...
module Dot_patdiff = struct
  type t = { diff_config : Format_rules.t } [@@deriving fields]

  let default_diff_config = Format_rules.inner_default

  let get () =
    let diff_config =
      let module Patdiff_config = Patdiff.Configuration in
      let config =
        Option.try_with (fun () ->
          Option.bind (Core.Sys.getenv "HOME") ~f:(fun home ->
            match Sys_unix.file_exists (home ^/ dot_patdiff4) with
            | `Yes | `Unknown -> None
            | `No -> Patdiff_config.load ~quiet_errors:true (home ^/ dot_patdiff)))
        |> Option.join
      in
      match config with
...
let view_configuration t =
  t.view_configuration |> Hashtbl.to_alist |> Diff4_class.Map.of_alist_exn
;;
