...
module Index = struct
  ...
  let with_space_if_non_trivial t = if t.total <= 1 then "" else " " ^ to_string_hum t
end
...
module Nesting : sig
      ...
      | Group [ (Lines _ as lines); Group group ] ->
        (match group with
         ...
         | _ :: _ ->
           ...
           if List.for_all group ~f:(function
                | Group _ | Lines [] -> true
                | Lines (_ :: _) -> false)
           then Group (lines :: group)
           else Group [ lines; Group group ])
      | Group group -> Group (List.map group ~f:simplify_in_depth)
    in
        ...
        [%test_result: string]
          (to_strings
             (hunk
                ...
                ; view ~header:[ "feature-ddiff" ] [ block [] [ "some lines" ] ]
                ; view ~header:[ "conflict" ] [ block [] [ "some lines" ] ]
                ])
          |> String.concat ~sep:"\n")
          ~expect:
            "hello hunk\n\
  ...
  let to_strings_utf8 =
    let module Table_char = Ascii_table_kernel.Table_char in
    to_strings
  ...
  let to_strings =
    if Iron_options.display_ascii_always then to_strings_ascii else to_strings_utf8
  ;;
end
...
let nested_views ?hunk_name t =
  let block_to_lines ?(header = []) (block : Diff_algo.Block.t) =
    Nesting.block (List.concat [ header; block.hint; block.lines ])
  in
  let number_of_views = List.length t.views in
  let indexed_views =
    List.mapi t.views ~f:(fun i view ->
        ...
        Some
          [ Header.title
              (sprintf
                 "%sView%s : %s"
                 hunk_name
                 (Index.with_space_if_non_trivial index)
                 view_name)
          ])
      else None
    in
...
let align_alist alist =
  let max_len =
    List.fold alist ~init:0 ~f:(fun acc (label, _) -> max acc (String.length label))
  in
  List.map alist ~f:(fun (label, data) -> sprintf "%-*s = %s" max_len label data)
;;
...
let file_and_rev_names_information ~use_file_separator ~in_scope t =
  let shall_include_rev_names =
    match in_scope with
    | None -> true
    | Some scope -> not (Diamond.for_all2 ~f:String.equal scope.rev_names t.rev_names)
  in
  let shall_include_file_names =
    match in_scope with
    | None -> true
    | Some scope -> not (Diamond.for_all2 ~f:String.equal scope.file_names t.file_names)
  in
  ...
  let lines = if shall_include_file_names then filename :: lines else lines in
  let lines =
...
let list_to_lines hunks =
  ...
  let hunks =
    List.mapi hunks ~f:(fun current t -> { Index.current; total = number_of_hunks }, t)
  in
  let _in_scope, blocks =
    List.fold hunks ~init:(None, []) ~f:(fun (in_scope, acc) (index, t) ->
      ...
      in
      let file_separator =
        Nesting.lines (file_and_rev_names_information ~use_file_separator ~in_scope t)
      in
      let this_nesting = Nesting.group views in
      Some t, this_nesting :: file_separator :: acc)
  in
  blocks |> List.rev |> Nesting.group |> Nesting.to_strings
;;
