...
module T = struct
  type metadata = string [@@deriving sexp, compare, hash]
...
let is_simple_diff = function
  ...
  | `old_base_to_new_tip
  | `old_tip_to_new_tip
  | `new_base_to_new_tip -> true
  | `feature_ddiff | `base_ddiff | `story | `conflict_resolution | `metadata _ -> false
;;
...
let%test _ =
  ...
  in
  List.for_all referenced_by_patdiff4_el ~f:(fun (t, expect) ->
    String.equal (to_string t) expect)
;;
