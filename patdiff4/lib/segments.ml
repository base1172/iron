...
type t = Segment.t list [@@deriving sexp_of]
...
let%test_unit _ =
  ...
  let check expr expect = [%test_result: int list] expr ~expect in
  check (tail list 0) [];
  check (head list 0) [];
  check (tail list 1) [ 9 ];
  check (head list 1) [ 0 ];
...
let%test_module "split_around" =
  (module struct
    ...
    let%test _ = is_error (Result.try_with (fun () -> split_around [ 'a'; 'b'; 'c' ] 3))
...
let of_matches
  ...
  let enqueue_segment (segment : Segment.t) =
    ...
    then
      Debug.eprint_s
        [%sexp
          "of_matches: enqueue_segment"
          , [%here]
          , { segment : Segment.t; current_segment : (Segment.t * Segment.t) option ref }];
    ...
    | Some (seg1, last_of_seg1) ->
      ...
      let diff4_class_of_last_two =
        lazy
          ...
          then last_of_seg1.diff4_class
            ...
            Diamond.classify
              ~equal:equal_for_classify
              (Diamond.map2 last_of_seg1.slice seg2.slice ~f:(fun s1 s2 ->
                 List.concat [ s1.lines; context_between; s2.lines ] |> Array.of_list)))
      in
      ...
      then (
        ...
        then (
          let split_seg1 = if Segment.is_shown seg2 then split_at else context in
          let seg1 = Segment.append seg1 (head context_between split_seg1) in
          Queue.enqueue segments seg1);
        let split_seg2 = if Segment.is_shown seg1 then split_at else context in
    ...
    then (
      let slice =
        Diamond.map3
          ...
          match_lines
          source_lines
          ~f:(fun source line_number lines -> Slice.create ~source line_number lines)
      in
      let diff4_class =
        Diamond.classify
          ~equal:equal_for_classify
  ...
  let fold_matches (match_lines, source_lines) matches =
    ...
    let prev_lines, mid, next_lines =
      ...
      in
      Diamond.map ~f:fst3 split, Diamond.map ~f:snd3 split, Diamond.map ~f:trd3 split
    in
...
let of_files
  ?(verbose = false)
  ?(force_should_split_files_in_hunks_for_tests = false)
  ~rev_names
  ~context
  ~lines_required_to_separate_ddiff_hunks
  ~contents:diamond
  ...
  if verbose
  then Debug.eprint_s [%sexp "input:", [%here], (arrays : string Array.t Diamond.t)];
  ...
  then
    Debug.eprint_s
      [%sexp "files_diff4_class", [%here], (files_diff4_class : Diff4_class.t)];
  if not
       (Diff_algo.should_split_files_in_hunks files_diff4_class
       || force_should_split_files_in_hunks_for_tests)
  then
    [ { Segment.slice =
          Diamond.map2 file_names lines ~f:(fun source -> Slice.create ~source 0)
    ...
    in
    if verbose
    then Debug.eprint_s [%sexp "matches", [%here], (matches : (int * int) list)];
    let indexes_of_matches =
      List.map matches ~f:(fun (old_base_old_tip_idx, new_base_new_tip_idx) ->
        let _, old_base_idx, old_tip_idx = pcs_old_base_old_tip.(old_base_old_tip_idx) in
        let _, new_base_idx, new_tip_idx = pcs_new_base_new_tip.(new_base_new_tip_idx) in
        { Diamond.b1 = old_base_idx
        ; b2 = new_base_idx
        ; f1 = old_tip_idx
        ; f2 = new_tip_idx
        })
      ...
      Debug.amf
        ...
        (String.concat
           ...
           (List.map indexes_of_matches ~f:(fun d ->
              sprintf
                "%s %s"
                ([%sexp_of: int Diamond.t] d |> Sexp.to_string)
