...
let header_of_conflict_resolution ~context:_ to_range =
  let minus = { Header.Source.name = "conflict"; other_names = []; range = None } in
  Header.Diff2 { minus; plus = Range.to_header ~other_names:[] to_range }
;;
...
let header_of_change ~diff4_class ~full_diff change =
  ...
  { Header.minus = Range.to_header ~other_names:from_other_names from_range
...
let diff_of_change
  ...
  let module Patdiff_core = Patdiff.Patdiff_core in
  Patdiff_core.diff
    ~context
    ~line_big_enough:Patdiff_core.default_line_big_enough
    ~keep_ws:false
    ~prev:(Array.of_list minus)
    ~next:(Array.of_list plus)
  |> (if refined
     then
       Patdiff_core.refine
         ~output:Ansi
         ...
         ~interleave:false
         ~word_big_enough:Patdiff_core.default_word_big_enough
...
let resolve_diff_algo ~include_hunk_breaks ~diff4_class:_ ~context full_diff =
  let { Diamond.b1 = old_base; b2 = new_base; f1 = old_tip; f2 = new_tip } = full_diff in
  ...
  { View.id = `conflict_resolution; blocks = [ { Block.hint = []; lines } ] }
;;

let make_ddiff_algo_fun
  ~id
  ?(hint = [])
  a
  b
  ...
  let make_change (from, to_) =
    { Change.from = from, Diamond.get full_diff from
  ...
  let change = { Change.from = (), make_slice a_inner; to_ = (), make_slice b_inner } in
  ...
  let lines =
    ...
    diff_of_change
      ...
      change
  in
  { View.id; blocks = [ { Block.hint; lines } ] }
;;
...
let base_ddiff ?hint () = make_ddiff_algo ?hint `base_ddiff
let feature_ddiff ?hint () = make_ddiff_algo ?hint `feature_ddiff

(* Diff Algos *)

let build_diff_lines (from, to_) ~include_hunk_breaks ~diff4_class ~context full_diff =
  let change =
    { Change.from = from, Diamond.get full_diff from
    ; to_ = to_, Diamond.get full_diff to_
    }
...
let build_diff_algo_fun
  ...
  =
  let lines =
    build_diff_lines (from, to_) ~include_hunk_breaks ~diff4_class ~context full_diff
  in
  { View.id; blocks = [ { Block.hint; lines } ] }
;;
...
let conflict_resolution = `conflict_resolution, resolve_diff_algo
...
let should_split_files_in_hunks diff4_class =
  ...
  | Some diff4_class ->
    (match diff4_class with
     ...
     | `b1_f1 | `b1_f2 | `b2_f2 | `f1_f2 | `conflict -> true)
;;
...
let story ~dropped_hint ~dropped_elements ~in_favor_hint ~in_favor_elements =
  let fct ~include_hunk_breaks ~diff4_class ~context full_diff =
    let build_diff elements =
      build_diff_lines elements ~include_hunk_breaks ~diff4_class ~context full_diff
    in
    { View.id = `story
    ; blocks =
        [ { Block.hint = [ dropped_hint ]; lines = build_diff dropped_elements }
        ; { Block.hint = [ in_favor_hint ]; lines = build_diff in_favor_elements }
        ]
    }
  in
  `story, fct
...
let select_algos_for_review : Diff4_class.Shown_class.t -> t list = function
  ...
  | `b2_f2 ->
    ...
    ; story
        ~dropped_hint
        ~dropped_elements:(`old_base, `old_tip)
        ~in_favor_hint
        ~in_favor_elements:(`old_base, `new_base)
  ...
  | `f1_f2 ->
    ...
    ; story
        ~dropped_hint
        ~dropped_elements:(`old_base, `new_base)
        ~in_favor_hint
        ~in_favor_elements:(`old_base, `new_tip)
...
let display_forget ~context slices =
  ...
  apply (diff_algo :> t) ~include_hunk_breaks:true ~diff4_class:`conflict ~context slices
;;
  ...
  then None
    ...
    let diff =
      List.concat_map errors ~f:(fun error ->
        error |> Error.to_string_hum |> String.split_lines)
    in
    let error_block = { Block.hint = []; lines = Header.errors :: diff } in
    let view = { View.id = `metadata "errors"; blocks = [ error_block ] } in
    Some view)
;;

let lines t ~context slices =
  let view = apply t ~include_hunk_breaks:true ~diff4_class:`conflict ~context slices in
