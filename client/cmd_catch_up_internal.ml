...
let mark_id =
  Command.async'
    ...
    and diff4_in_session_ids = diff4_in_session_ids in
    fun () ->
      let feature_path = ok_exn feature_path in
      Catch_up_diffs.rpc_to_server_exn
        { for_; feature_path; catch_up_session_id; diff4_in_session_ids })
;;
...
let show_num_lines =
  Command.async'
    ...
    fun () ->
      ...
      let feature_path = ok_exn feature_path in
      let%bind status = Get_catch_up_session.rpc_to_server_exn { feature_path; for_ } in
      let lines =
        match status with
        | `Up_to_date -> 0
...
let command =
  Command.group
    ~summary:"various catch-up commands for Iron developers and tests"
