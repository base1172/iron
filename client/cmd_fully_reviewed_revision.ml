...
let add =
  Command.async'
    ~summary:"add a fully-reviewed revision"
...
let check =
  Command.async'
    ~summary:"check whether a revision is fully-reviewed"
    (let open Command.Let_syntax in
    let%map_open () = return ()
    and rev = rev in
    fun () ->
...
let remove =
  Command.async'
    ~summary:"remove a fully-reviewed revision"
    (let open Command.Let_syntax in
    let%map_open () = return ()
    and rev = rev in
    fun () ->
...
let internal =
  Command.group
    ~summary:"change the set of fully-reviewed revisions"
    [ "add", add; "remove", remove ]
;;

let tools =
  Command.group ~summary:"check the set of fully-reviewed revisions" [ "check", check ]
;;
