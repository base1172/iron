...
let dialog
    ...
    then return ()
      ...
      Async_interactive.printf
        ...
        (Ascii_table.to_string
           (Line_count_table.create
              ~show_completed_review:true
  ...
  let%bind b =
    Async_interactive.ask_yn
      ~default:false
      (sprintf "Do you want to second %s ?" (Feature_path.to_string feature_path))
  in
  if b then return () else failwith "did not second"
;;
...
let command =
  Command.async'
    ...
    ~readme:(fun () ->
      concat
        [ "The recommended workflow is to find a non-owner whole-feature reviewer to act \
           as the\n\
           seconder.  If you need to second even though you are an owner, use:\n\n\
          \  $ fe second "
        ; Switch.even_though_owner
        ; " [FEATURE]\n\n\
    ...
    fun () ->
      ...
      in
      Second.rpc_to_server_exn { feature_path; even_though_empty; even_though_owner })
;;
