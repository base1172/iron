...
let check_remote { Feature.feature_path; next_bookmark_update; tip; remote_repo_path; _ } =
...
let check_cleanliness_and_update
  ~feature_path
  ~remote_repo_path
  ~actually_update_if_update_requested
  = function
    ...
    (match waiting_on_current_bookmark with
     ...
     | Ok () ->
       ...
       then Deferred.unit
         ...
         let%bind () =
           Hg.pull ~repo_is_clean repo_root ~from:remote_repo_path (`Feature feature_path)
         in
         let%bind () =
           Hg.update
             repo_root
...
module Waiting_status = struct
  ...
  let now reason_for_waiting = { waiting_since = Time.now (); reason_for_waiting }
end

let main { Fe.Wait_for_hydra.Action.feature_path; rev_zero; timeout; whether_to_update } =
  let%bind feature =
    Iron_protocol.Get_feature.rpc_to_server_exn { feature_path; rev_zero }
  in
       ...
       let maybe_check_remote_again feature =
         don't_wait_for
           (let%map () = Clock.after (sec 5.) in
            if is_last_non_pending_feature feature && not (Pipe.is_closed pipe_writer)
            then Pipe.write_without_pushback pipe_writer (`Check_remote_again feature))
       in
       let process_event_queue event_queue =
         ...
         | `Updated (feature : Feature.t) ->
           ...
           (match%map wait_status feature with
            | `Expecting_bookmark_update _ as reason_to_wait -> `Wait reason_to_wait
            | `Finished _ as finished -> finished
            | `Different_tips _ as reason_to_wait ->
  ...
  | `Timeout ->
    raise_s
      [%sexp
        (sprintf
           "timed out after waiting %s for hydra to process %s"
...
let command =
  ...
  Command.async'
    ...
    ~readme:(fun () ->
      concat
        [ "While Iron is waiting for hydra to process a feature, it is shown as [pending].\n\
           ...
           There is a default timeout of "
        ; Time.Span.to_string timeout_default
        ; " (use ["
        ; timeout_switch
        ; "] to change it).\n\nIf the flag ["
        ; Switch.update_local_repo
        ; "] is supplied, the command will also\n\
           update the local repository to the tip of the feature just before returning.\n\n\
           One can also add the following line to one's [.ferc] to force the update by \
           default:\n\n\
          \  "
        ; line_in_ferc
        ; "\n\nWhen using the latter, the switch ["
    ...
    and timeout =
      flag
        timeout_switch
        (optional_with_default timeout_default time_span)
    ...
    and whether_to_update =
      ...
      and update_local_repo = update_local_repo in
      Lazy.from_fun (fun () ->
        ...
        | true, true ->
          failwithf
            "The switches [%s] and [%s] are mutually exclusive"
            Switch.do_not_modify_local_repo
            Switch.update_local_repo
            ())
    in
