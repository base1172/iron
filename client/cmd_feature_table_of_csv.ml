...
let array_safe_get array i = if Array.length array > i then Some array.(i) else None
...
let get_columns (headers : _ Fe.Feature_table_of_csv.Row.t) data =
  let feature_path_column =
    Ascii_table.Column.string
      ~header:headers.feature_path
      ~align:Left
  ...
  let other_columns =
    List.mapi (Array.to_list headers.other_columns) ~f:(fun i header ->
      Ascii_table.Column.string
        ~header
        ~align:(get_alignment data i)
        (Ascii_table.Column.cell (fun (_, data) ->
           Option.value
             ~default:""
...
let main'
  { Fe.Feature_table_of_csv.Action.headers
  ; data
...
let command =
  Command.async'
    ~summary:"print tabular data indexed by feature name"
    ~readme:(fun () ->
      "This commands reads comma separated values from stdin and outputs the data in the \
       form of\n\
       an Iron feature table on stdout.\n\n\
       The first line of input should be a header.  The first column should be feature \
       names.\n\n\
       Rows will be re-ordered by increasing feature name, but duplicates will be \
       preserved.")
    (let open Command.Let_syntax in
    let%map_open () = return ()
    and sep =
      flag "-sep" (optional char) ~doc:"CHAR delimits adjacent columns.  Default is ','"
    and max_output_columns = max_output_columns
    and display_ascii = display_ascii in
    fun () ->
      ...
      | car :: _ as rows ->
        let headers =
          Delimited.Read.Row.headers car
          |> Map.to_alist
        ...
        let data =
          List.mapi rows ~f:(fun i row ->
            let feature_path =
              try
                Delimited.Read.Row.nth_conv_exn row 0 [%here] Feature_path.of_string
              with
              | _ ->
                raise_s
                  [%sexp
                    (sprintf
                       "invalid row(%d): first column must be valid feature path"
                       (succ i)
                      : string)
                    , (Delimited.Read.Row.to_list row : string list)]
            in
            let other_columns =
              let all_columns = Delimited.Read.Row.to_array row in
