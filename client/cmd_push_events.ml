  ...
  Command.async'
    ...
    and what_to_show =
      ...
      Or_error.try_with (fun () ->
        ...
        | false, Some feature_id, None -> `Feature_id feature_id
        ...
        | _, _, _ ->
          failwithf
            ...
            ())
    in
    fun () -> Cmd_dump.dump (Push_events (ok_exn what_to_show)))
;;
...
let clear =
  Command.async'
    ...
    and what_to_clear =
      let%map revs =
        flag "-rev" (listed rev_arg_type) ~doc:"REV revision(s) to be cleared from state"
      ...
      and all = no_arg_flag all_switch ~doc:" clear the entire state" in
      Or_error.try_with (fun () ->
        ...
        | _, _, _, _ ->
          failwithf
            "One should explicitly specify what to clear or supply %s"
            all_switch
            ())
    in
    fun () ->
      ...
      let%bind what_to_clear =
        ...
        | `All -> return Push_events.Change.Action.Clear_all
        | `Features feature_ids ->
          return (Push_events.Change.Action.Clear_features feature_ids)
        | `Users user_names -> return (Push_events.Change.Action.Clear_users user_names)
        | `Revs revs ->
          let%map revs =
            Deferred.List.map revs ~how:(`Max_concurrent_jobs 4) ~f:(fun rev ->
...
let command =
  Command.group
    ...
    [ "clear", clear; "set-max-size-per-feature", set_max_size_per_feature; "show", show ]
;;
