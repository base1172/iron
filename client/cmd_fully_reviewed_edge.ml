...
let internal_add =
  Command.async'
    ~summary:"add a fully-reviewed edge"
    ~readme:(fun () ->
      concat
        [ "This command is unsound because it marks an arbitrary edge as \
           fully-reviewed.  This\n\
           requires admin privileges.  To mark as fully-reviewed the edge of a \
           releasable feature,\n\
    ...
    and to_ = flag "-to" (required rev_arg_type) ~doc:"REV" in
    fun () ->
      ...
      Add_fully_reviewed_edge.rpc_to_server_exn { rev_zero; edge = `From_to (from, to_) })
;;

let internal =
  Command.group
    ~summary:"deal with the set of fully-reviewed edges"
    [ "add", internal_add ]
;;

let tools_add =
  Command.async'
    ~summary:"add a fully-reviewed edge from a feature's base to tip"
    ~readme:(fun () ->
      concat
    ...
    and if_feature_is_empty =
      ...
      |> map ~f:(fun do_nothing_if_feature_is_empty ->
           if do_nothing_if_feature_is_empty then `Do_nothing else `Fail)
    in
    fun () ->
      let open! Deferred.Let_syntax in
...
let tools_check =
  Command.async'
    ~summary:"check whether a fully-reviewed edge exists"
    ~readme:(fun () -> concat [ "See:\n\n  $ fe tools fully-reviewed-edge -help\n" ])
    (let open Command.Let_syntax in
    let%map_open () = return ()
    and from = rev
    and to_ = rev in
...
let tools =
  Command.group
    ...
    ~readme:(fun () ->
      concat
        [ "Fully-reviewed edges are added lazily to the server state, typically each \
           time a feature\n\
           is released.  However, one may choose to add new edges manually using this \
           command, at any\n\
           point using the current base and tip of a releasable feature.\n\n\
           Fully-reviewed edges affect the behavior of Iron in various ways.  For \
           example, when\n\
           [fe create] is called with a [-base] and [-tip] matching an existing \
           fully-reviewed edge,\n\
           all users are automatically marked as reviewed with no catch-up.\n\n\
           To check whether an edge has been marked as fully-reviewed in the server \
           state, see:\n\n\
          \  $ fe tools fully-reviewed-edge check -help\n\n\
           To check whether there exists a path of fully-reviewed edges leading to a \
           particular\n\
           revision, see:\n\n\
          \  $ fe tools fully-reviewed-revision check -help\n"
        ])
    [ "add", tools_add; "check", tools_check ]
;;
