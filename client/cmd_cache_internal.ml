...
let force_set_cached_feature_attributes =
  Command.async'
    ...
    fun () ->
      ...
      let next_steps =
        try
          next_steps |> Sexp.of_string |> [%of_sexp: Next_step.For_command_line.t list]
        with
        | exn -> raise_s [%sexp "error while parsing next steps", (exn : Exn.t)]
      in
      Force_set_cached_feature_attributes.rpc_to_server_exn
        { feature_path; skip_post_RPC_check; next_steps })
;;
...
let errors_group =
  Command.group
    ~summary:"deal with cached-attribute errors"
    [ "clear", clear_cached_attributes_errors; "get", get_cached_attributes_errors ]
;;

let command =
  Command.group
    ~summary:"deal with cached attributes in features"
