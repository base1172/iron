...
let create list =
  ...
  let pointer = 0 in
  { check_code = !check_code; elements; pointer; deleted }
;;

let pointed t =
  if Array.is_empty t.elements
  then None
  else if t.deleted.(t.pointer)
  then None
  else
    Some
      { Elt.value = t.elements.(t.pointer); index = t.pointer; check_code = t.check_code }
;;
...
let values t = Array.to_list (Array.filteri t.elements ~f:(fun i _ -> not t.deleted.(i)))
let is_empty t = Array.for_all ~f:Fn.id t.deleted
let mem t elt = Int.( = ) t.check_code (Elt.check_code elt)

let delete t elt =
  if not (mem t elt) then failwith "Review_ring.delete. Not a element of that ring";
  let len = Array.length t.elements in
  let index = Elt.index elt in
  if index >= 0 && index < len
  then (
    t.deleted.(index) <- true;
    if t.pointer = index then goto_next t)
;;
