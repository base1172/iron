...
module Iron_command_rpc = Iron_public.Iron_command_rpc
...
module Command = struct
  ...
  module Param = Iron_public.Std.Iron.Param
  module Let_syntax = Param.Let_syntax
end

include struct
  open Iron_public.Std
  module Client_config = Iron.Client_config
  module Fe = Fe
end

module Iron = struct
  let (_ : [> `Use_fe_instead ]) = `Use_Fe_instead
end

let die message a sexp_of_a =
  eprintf
    "%s\n"
    (Sexp.to_string_hum (Error.create message a sexp_of_a |> [%sexp_of: Error.t]));
  Shutdown.exit 1
;;
...
let ensure_can_access_remote_repo ~for_root_of =
  let%bind { remote_repo_path; _ } =
    Get_feature_revs.rpc_to_server_exn
      { feature_path = Feature_path.root_path for_root_of; rev_zero = None }
  in
  Hg.ensure_can_access_remote_repo remote_repo_path
;;

let feature_reviewers feature_path ~sort =
  let%map feature = Get_feature.rpc_to_server_exn { feature_path; rev_zero = None } in
