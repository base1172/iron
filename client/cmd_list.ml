...
let table
  ...
  let rows =
    Feature_table.create
      (features : List_features.Reaction.t)
      ...
      (fun ~feature reaction_option ->
        ...
        | Some { next_steps; num_lines; review_is_enabled; feature_id; status; _ } ->
          let feature_id =
            match status with
  ...
  let columns =
    Ascii_table.Column.
      ...
      ; string
          ...
          (attr_cell Line.reason_for_archiving)
...
module Table = struct
  let main action = return (table action)
end
...
let command =
  Command.async'
    ...
    and use_archived = no_arg_flag Switch.archived ~doc:"list archived features"
    and sort_most_recently_archived_first =
      no_arg_flag
        "-sort-most-recently-archived-first"
        ~doc:
          (sprintf
             "show most recently archived features first.  Implies [%s]"
             Switch.archived)
    ...
    fun () ->
      ...
      then (
        ...
        in
        feature_paths
        |> List.sort ~compare:Feature_path.compare
        ...
        then (
          let features =
            let compare : List_features.Reaction.one -> List_features.Reaction.one -> int =
              ...
              then
                ...
                | Archived t1, Archived t2 -> Time.compare t2.archived_at t1.archived_at
              else
                fun (f1 : List_features.Reaction.one) f2 ->
                Feature_path.compare f1.feature_path f2.feature_path
            in
            List.sort features ~compare
          in
          if name_only
          then
            ...
            print_string
              (table
                 { features; preserve_input_ordering; display_ascii; max_output_columns })))))
;;
