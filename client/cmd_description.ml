...
let set_description_exn feature_path description =
  ...
  let%bind responses = Change_feature.rpc_to_server_exn { feature_path; updates } in
  ...
  Command.async'
    ...
    fun () ->
      ...
      let rec loop previous_description =
        let%bind new_description = Editor.invoke_editor previous_description in
        let new_description = ok_exn new_description |> String.strip in
        if String.equal new_description initial_description
        then (
          print_endline "You didn't change the description.";
          Deferred.unit)
        else (
          ...
          print_endline
            (Patdiff.Patdiff_core.patdiff
               ~rules
               ()
               ~prev:{ name = "old"; text = initial_description }
               ~next:{ name = "new"; text = new_description });
...
let command =
  Command.group
    ~summary:"manage the description of a feature"
    [ "edit", edit; "set", set; "show", show ]
;;
