...
let grep repo_root ~aliases =
  ...
  let cr_comment_format =
    obligations_version |> ok_exn |> Obligations_version.cr_comment_format
  in
  Cr_comment.grep
    repo_root
    cr_comment_format
...
let command =
  Command.async'
    ...
    and should_grep =
      no_arg_flag
        "-grep-current-dir-and-below"
        ~doc:"grep the current directory and below"
    and drop_content =
      no_arg_flag
        "-drop-content"
        ~doc:"drop the content of the CR and just show the position"
    ...
    and feature_owner =
      flag
        "-owner"
        (optional user_name)
        ~doc:"USER treat USER as the feature owner, for unassigned CRs"
    and show_summary_only = no_arg_flag "-summary" ~doc:"show counts of CRs in a table"
    ...
    and include_active_cr_soons = include_active_cr_soons in
    fun () ->
      ...
      then failwithf "%s is only meaningful with -soon" Switch.include_active_cr_soons ();
      ...
      | true ->
        ...
        (match bookmark_or_error with
         | Error _ -> ()
         | Ok bookmark ->
           if not (String.equal (Feature_path.to_string feature_path) bookmark)
           then
             failwithf
               "cannot grep for feature '%s' since current bookmark is '%s'"
        ...
        then (
          ...
          Option.iter (Cr_comment.Summary.to_ascii_table summary) ~f:(fun ascii_table ->
            print_string
              (Ascii_table.to_string ascii_table ~display_ascii ~max_output_columns)))
        else
          print_cr_comments
            ~crs:grepped_crs
            ~for_or_all
      ...
      | false ->
        if someday
        then
          failwith "[fe crs] is incompatible with -someday, use [fe crs -grep -someday]";
        (match soon with
         | true ->
           let%map cr_soons =
           ...
           let cr_soons =
             List.sort
               ...
               ~compare:Cr_soon_in_feature.For_sorted_output.compare
           in
           print_cr_soons_in_feature ~cr_soons ~drop_content
         | false ->
           if show_summary_only
           then (
             let%map summary = Get_cr_summary.rpc_to_server_exn { feature_path } in
             let summary = ok_exn summary in
             Option.iter
               (Cr_comment.Summary.to_ascii_table summary)
               ~f:(fun ascii_table ->
               print_string
                 (Ascii_table.to_string ascii_table ~display_ascii ~max_output_columns)))
           else (
             let%map crs = Get_crs.rpc_to_server_exn { feature_path; for_or_all } in
