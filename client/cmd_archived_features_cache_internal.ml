...
let clear =
  ...
  Command.async'
    ...
    let%map_open () = return ()
    and action =
      let%map all = no_arg_flag all_switch ~doc:"clear all the values cached"
      and id =
        flag
          id_switch
      ...
      Or_error.try_with (fun () ->
        ...
        | false, None -> failwithf "specify either [%s] or [%s]" all_switch id_switch ()
        | true, Some _ ->
          failwithf
...
let show =
  ...
  Command.async'
    ~summary:"show the items in the archived features cache"
    (let open Command.Let_syntax in
    let%map_open () = return ()
    and with_feature_paths =
      no_arg_flag
        feature_paths_switch
    ...
    fun () ->
      ...
      let what_to_dump =
        ...
        | true, Some _ ->
          failwithf
            "flags [%s] and [%s] are incompatible"
            feature_paths_switch
            feature_id_switch
            ()
      in
...
let command =
  Command.group
    ...
    ~readme:(fun () ->
      concat [ "Operations side affecting the cache require admin privileges." ])
    [ "clear", clear; "set-max-size", set_max_size; "show", show ]
;;
