...
let send_sigkill_if_pid_exists pid =
  ...
  match%map
    In_thread.syscall ~name:"send_signal" (fun () -> Signal_unix.send_i signal (`Pid pid))
    |> Clock.with_timeout (sec 0.2)
  with
  | `Result (Ok ()) -> Ok ()
  | `Result (Error exn) ->
    error_s
      [%sexp "sending signal failed", { pid : Pid.t; signal : Signal.t; exn : Exn.t }]
  | `Timeout ->
    error_s [%sexp "sending signal timed out", { pid : Pid.t; signal : Signal.t }]
;;
...
let process_run ~prog ~args ~timeout () =
  ...
  | Ok process ->
    (match%bind Clock.with_timeout timeout (Process.collect_stdout_and_wait process) with
     ...
     | `Timeout ->
       ...
       add_error
         (Error.create_s
            [%sexp "running process timed out", { prog : string; args : string list }]);
    ...
    ((* We use the [emacsclient] in the user's PATH rather than an absolute path -- this is
     ...
     match%map
       process_run
         ()
         ~prog:emacsclient
         ~args:[ "--alternate-editor=/bin/false"; "-e"; "Omake.Server.program" ]
         ~timeout:(sec 1.0)
     with
     | Error _ as e -> e
     | Ok stdout ->
       (match String.split_lines stdout with
        | ([] | _ :: _ :: _) as lines ->
          error
            (sprintf "strange output from %s" emacsclient)
            lines
            [%sexp_of: string list]
        | [ line ] ->
          let quote = "\"" in
...
module Jenga = struct
  ...
  let find_in_enclosing_repo_root enclosing_repo_root =
    let enclosing_repo_path =
      enclosing_repo_root |> Repo_root.to_abspath |> Abspath.to_string
    in
    let case1 () =
      let module Lock = struct
        type t = { pid : Pid.t } [@@deriving sexp] [@@sexp.allow_extra_fields]
      end
      in
      match%map
    ...
    in
    let case2 () =
      match%map Reader.load_sexp (enclosing_repo_path ^/ ".jenga/lock") Pid.t_of_sexp with
      | Error _ -> None
      | Ok pid -> Some pid
    in
...
module Project_id = struct
  include
    String_id.Make
      (struct
        let module_name = "Project_id"
      end)
      ()

  (** jane-elisp build-manager refers to running builds by their root directory. *)
  let of_jenga (jenga : Jenga.t) =
    of_string (Filename_unix.realpath (Repo_root.to_string jenga.enclosing_repo_root))
  ;;
end
