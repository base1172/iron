...
let available_views hunk =
  List.map (Hunk.views hunk) ~f:(fun view -> Diff_algo.View.id view, view)
;;

let default_view_configuration =
  lazy
    (let module Patdiff4_config = Pdiff4.Std.User_config in
    Patdiff4_config.get () |> Patdiff4_config.view_configuration)
;;
...
let find_view_configuration view_configuration diff4_class =
  Option.value ~default:[] (Map.find view_configuration diff4_class)
;;
...
let get_and_update_view_configuration view_configuration hunk =
  ...
  let update_configuration data = Map.set view_configuration ~key:diff4_class ~data in
...
let show_hunks ?view_configuration hunks =
  hunks_to_lines_with_configuration ?view_configuration hunks |> Review_util.show_lines
;;

let rec hunk_by_hunk
  : type a.
    ?view_configuration:view_configuration
    -> a t
    -> a file Review_ring.Elt.t
    -> result Deferred.t
  =
 fun ?view_configuration t file_elt ->
  let hunk_by_hunk ?view_configuration () = hunk_by_hunk ?view_configuration t file_elt in
  ...
  | Some hunk_elt ->
    ...
    in
    let choices =
      if M.may_commit_session then choices @ [ Choice.commit_session ] else choices
    in
    let choices =
...
and file_by_file : type a. a t -> result Deferred.t =
 fun t ->
  let module M = (val t.m : M with type t = a) in
  ...
  | Some file_elt ->
    ...
    then (
      ...
      let commit_session_option =
        if M.may_commit_session then [ Choice.commit_session ] else []
      in
      let question =
        let index = file.index_in_review in
        sprintf "[%d/%d] %s Mark as reviewed?" index t.number_of_files file.name
      in
      match%bind
        Async_interactive.ask_dispatch_with_help
          ...
          @ [ Choice.Mode.global_diff; Choice.Mode.selected_files; Choice.quit ])
      with
      | `Quit -> return `Quit
      | `Commit_session -> return `Commit_session
      | `Reviewed ->
...
and multiple_files
 ...
 fun t ~after_review_goto elements ->
  ...
  let%bind hunks = Deferred.List.all (List.map files ~f:(fun file -> file.hunks)) in
  ...
  then return `Reviewed
    ...
    let rec loop () =
      ...
      | `Quit -> return `Quit
      | `Reviewed ->
        let%bind () = reviewed t elements in
        after_review ()
...
and selected_files : type a. a t -> result Deferred.t =
 fun t ->
  ...
  | `File_by_file -> file_by_file t
  | `Global_diff -> global_diff t
  | `New_configuration files -> multiple_files t ~after_review_goto:`Selected_files files

and global_diff : type a. a t -> result Deferred.t =
 fun t ->
...
let files (type t) (module M : M with type t = t) files =
  ...
  let files =
    List.mapi files ~f:(fun index (file, hunks) ->
  ...
  in
  let t = { file_ring = Review_ring.create files; m = (module M); number_of_files } in
  let module Choice = Review_util.Choice in
  match%bind
    Async_interactive.ask_dispatch_with_help
      "How do you want to do this review?"
