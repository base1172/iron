...
module Dimension = struct
  ...
  let of_metric ~metric_name =
    if Metric_name.(
         equal hydra_synchronize_state_latency metric_name
...
module Values = struct
  ...
  let init (data_points : Metric.Data_point.t list) : t =
    data_points |> List.map ~f:Metric.Data_point.value |> Appendable_list.of_list
  ;;
...
module Stats = struct
  ...
  let compute (t : Values.t) =
    ...
    Array.sort array ~compare:Float.compare;
  ...
  let get_column t stat_column = List.Assoc.find t stat_column ~equal:Stat_column.equal
end
...
let add =
  Command.async'
    ...
    and values = metric_values in
    fun () ->
      let open! Deferred.Let_syntax in
      let feature_path = ok_exn feature_path in
      Metrics.Add_values.rpc_to_server_exn { feature_path; metric_name; values })
;;
  ...
  Command.async'
    ...
    and which_features =
      which_features
        ...
        ()
    and metric_name = metric_name in
    fun () ->
      let open! Deferred.Let_syntax in
      let%bind which_features = force which_features in
      Metrics.Clear.rpc_to_server_exn { metric_name; which_features })
;;
...
let increment_depth ~depth ~by =
  if Int.max_value - depth <= by then Int.max_value else depth + by
;;
...
let aggregate_by_feature ~feature_to_metrics_map ~descendants_of ~depth_to_aggregate =
  let shorten feature_path depth =
    ...
    then
      raise_s
        [%sexp
          "shorten requires depth > 0", { feature_path : Feature_path.t; depth : int }];
    List.take (Feature_path.parts feature_path) depth |> Feature_path.of_parts_exn
  in
  Map.fold
    feature_to_metrics_map
    ~init:Feature_path.Map.empty
    ~f:(fun ~key:descendant ~data:metrics acc ->
    let descendant =
      match (descendants_of : Which_ancestor.t) with
      | Any_root -> shorten descendant depth_to_aggregate
      | Feature ancestor ->
        shorten
          descendant
          (increment_depth
             ~depth:depth_to_aggregate
...
let metric_names feature_to_metrics_map =
  Map.fold
    feature_to_metrics_map
    ~init:Metric_name.Set.empty
    ~f:(fun ~key:_ ~data:metrics metric_names ->
    Map.fold metrics ~init:metric_names ~f:(fun ~key:metric_name ~data:_ metric_names ->
  ...
  let columns =
    Ascii_table.Column.(
      ...
      :: List.map which_stats ~f:(fun stat_column ->
           string
             ~header:(Enum.to_string_hum (module Stat_column) stat_column)
             ~align:Right
             (cell (fun (_, metric) ->
                match metric with
                | None -> ""
                | Some metric ->
                  (match Stats.get_column metric stat_column with
                   | None -> ""
                   | Some value ->
                     pp_value metric_name ~decimals ~stat_column:(Some stat_column) value)))))
  in
  let rows =
    ...
    | `By_feature feature_to_metrics_map ->
      let feature_to_metrics_map =
        Map.filter_map feature_to_metrics_map ~f:(fun metrics ->
          Map.find metrics metric_name)
      in
      Feature_table.create (Map.to_alist feature_to_metrics_map) fst (fun ~feature data ->
        match data with
        | None -> feature, None
        | Some (_, metric) -> feature, Some metric)
  in
  let rows =
    List.map rows ~f:(fun (row, snapshots) -> row, Option.map ~f:Stats.compute snapshots)
  in
...
let list =
  Command.async'
    ...
    (let open Command.Let_syntax in
    let%map_open () = return ()
    and descendants_of = descendants_of in
    fun () ->
      let open! Deferred.Let_syntax in
      let descendants_of = ok_exn descendants_of in
      let%map feature_to_metrics_map = Metrics.Get.rpc_to_server_exn { descendants_of } in
...
let get =
  Command.async'
    ...
    fun () ->
      ...
      let descendants_of = ok_exn descendants_of in
      let%map reaction =
        let%map features_map = Metrics.Get.rpc_to_server_exn { descendants_of } in
        let cut_depth =
      ...
      let print_table feature_path metric_name data_points =
        let columns =
          Ascii_table.Column.
            [ string
                ~header:"at"
                (cell (fun data_point -> Time.to_string (Metric.Data_point.at data_point)))
            ; string
                ~header:"value"
                (cell (fun data_point ->
      ...
      let include_metric_name =
        ...
        | Some metric_regex_list ->
          fun metric_name ->
            ...
            List.exists metric_regex_list ~f:(fun metric_regex ->
              Regex.matches metric_regex metric_name)
      in
...
let show =
  Command.async'
    ...
    ~readme:(fun () ->
      concat
        [ "Metrics are shown for all features in the requested subtree(s), aggregated by \
           -depth.\n"
        ])
    (let open Command.Let_syntax in
    let%map_open () = return ()
    ...
    and which_stats =
      let for_stats_doc =
        sprintf
          "STAT,[STAT...] select stats to show %s"
          (Stat_column.all
          |> List.sort ~compare:Stat_column.Compare_by_interest.compare
          |> List.map ~f:(Enum.to_string_hum (module Stat_column))
          |> String.concat ~sep:"|")
      in
      let%map for_stats_list =
        map
          (enum_list "-stats" ~doc:for_stats_doc (module Stat_column))
          ~f:Stat_column.Set.stable_dedup_list
      in
      if List.is_empty for_stats_list
      then Stat_column.(all |> List.sort ~compare:Compare_by_interest.compare)
      else for_stats_list
    and decimals =
      let default = 2 in
      flag
    ...
    fun () ->
      ...
      List.iter which_metrics ~f:(fun metric_name ->
        print_table
          ~name:(Metric_name.to_string metric_name)
          (by_metric_feature_table
             ~metric_name
             ~decimals
             ~depth_to_aggregate
             ~descendants_of
...
let subscribe =
  Command.async'
    ...
    and metric_name_option = metric_name_option in
    fun () ->
      let open! Deferred.Let_syntax in
      ...
      let (action : Notify_on_metric_updates.Action.t) =
        ...
        | None ->
          (match metric_name_option with
           ...
           | None ->
             failwith "A least one of a feature path and a metric name should be supplied")
      in
      ...
      let is_metric_of_interest =
        ...
        | Metric_name _, _ | Feature_path _, None -> const true
      ...
      Pipe.iter_without_pushback pipe ~f:(fun event ->
        ...
        then printf !"%{sexp#mach:Notify_on_metric_updates.Reaction.t}\n" event))
;;

let command =
  Command.group
    ~summary:"various commands for collecting data in features"
    [ "add", add
    ; "clear", clear
    ; "get", get
    ; "list", list
