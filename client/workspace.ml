    ...
    (match%bind find_root_feature feature_path with
     ...
     | Some { remote_repo_path; _ } ->
       ...
       let%map () =
         ...
         | Some feature_id ->
           Workspace_hgrc.save
             { repo_root; remote_repo_path; kind = `Feature { feature_id; feature_path } }
       in
       feature_id)
;;
...
module T : sig
  ...
  let center_repo_root_internal
    ...
    |> Repo_root.of_abspath
         ~human_readable:(concat [ Feature_path.to_string feature_path; "'s workspace" ])
  ;;

  let center_repo_root
    { feature_id = _
    ; feature_path
    ; enclosing_repo_root
    ; center_relative_to_enclosing_repo
  ...
  let create
    ...
    { feature_id; feature_path; enclosing_repo_root; center_relative_to_enclosing_repo }
  ;;
end
...
let enclosing_repo_root_abspath feature_path =
  Abspath.append
    (workspaces_basedir ())
    (Relpath.extend (Feature_path.to_relpath feature_path) share)
;;
...
let extract_feature_from_workspace_share_path_aux ~basedir repo_root =
  ...
  | Ok relpath ->
    ...
    then None
      ...
      Option.try_with (fun () ->
        List.take_while parts ~f:(fun name -> not (File_name.equal share name))
...
let share_or_clone_and_update
  ...
  let from_local_clone root_feature =
    ...
    let%map dst_repo_root = Repo_clone.create_share_exn clone ~dst_repo_root_abspath in
           ...
           raise_s
             [%sexp
               ...
               , { remote_repo_path : Remote_repo_path.t
                 ; known_by_iron =
                     (Map.data root_features : List_root_features.Reaction.t)
                 }])
  in
  let dst_repo_root =
  ...
  let%map () =
    Workspace_hgrc.save { repo_root = dst_repo_root; remote_repo_path; kind }
  in
  dst_repo_root
;;
...
let force { Workspace_hgrc.Feature.feature_id; feature_path } =
  ...
  let tmp_location =
    ...
    fun () ->
      ...
      Abspath.of_string
        (concat
           [ Abspath.to_string enclosing_repo_root_abspath; ".tmp"; Int.to_string !n ])
  in
  let%bind enclosing_repo_root =
    Workspace_util.create_repo_if_it_does_not_exist
      (`Share_of feature_path)
          ...
          let%bind () =
            Abspath.rename_exn
              ...
              ~dst__delete_if_exists:
                (Abspath.append tmp_enclosing_repo_root center_relative_to_enclosing_repo)
          in
          let%bind () =
        ...
        then Deferred.unit
          ...
          | true ->
            ...
            (* We allow this call to fail.  For example, we do not want to prevent the
                 ...
                 [fe create]. *)
            jenga_process |> ok_exn |> (ignore : Process.t -> unit))
      in
...
let unix_find_dirs_exn ~in_ args : Relpath.t list Deferred.t =
  ...
  | Ok lines ->
    List.map lines ~f:(fun line ->
      ...
      | Ok dir -> dir
      | Error err -> raise_s [%sexp "unexpected path returned by find", (err : Error.t)])
;;
    ...
    let%bind basedirs =
      unix_find_dirs_exn
        ...
        [ (* print but do not recur in directories ending in [+share+] *)
          "-name"
        ; File_name.to_string share
        ; "-prune"
        ; "-print" (* do not print or recur in directories ending in [+clone+] *)
        ; "-or"
        ; "("
        ; "-name"
        ; Char.to_string character_not_leading_feature_names ^ "*"
        ; "-or"
        ; "-name"
        ; ".hg"
        ; ")"
...
let find_repos t =
  ...
  List.map dirs ~f:(fun dir ->
    dir |> Abspath.append in_ |> Abspath.parent_exn |> Repo_root.of_abspath)
;;
...
let kill_build t =
  match%bind Build_manager.Jenga.find_in_enclosing_repo_root t.enclosing_repo_root with
  | None -> return ()
  | Some jenga ->
    (* Try to kill it via jane-elisp build manager *)
    let%bind build_manager_err =
      match%map Build_manager.kill_project (Build_manager.Project_id.of_jenga jenga) with
...
let removing t ~f =
  ...
  in
  let%bind () = f t in
  Deferred.repeat_until_finished deleted_repo_relpath (fun dir ->
...
let check_current_bookmark t =
  let%map result =
    Monitor.try_with ~extract_exn:true (fun () ->
      ...
      | Error err -> raise_s [%sexp "cannot determine current bookmark", (err : Error.t)]
      | Ok bookmark ->
...
module Log_in_dot_hg : sig
  ...
  let get_log =
    Memo.general ~hashable:Repo_root.Hash_by_path.hashable (fun repo_root ->
...
let unclean_status_throttle =
  Memo.unit (fun () ->
    let max_concurrent_jobs =
      Client_config.(
        get () |> Workspaces.unclean_workspaces_detection_max_concurrent_jobs)
    in
    ...
    Monitor.try_with ~extract_exn:true (fun () ->
      ...
      removing t ~f:(fun t ->
        Async_interactive.Job.run
          !"deleting share of %{Feature_path}"
          t.feature_path
  ...
  | false ->
    ...
    let%bind () =
      Workspace_hgrc.save
        { repo_root
        ; remote_repo_path
        ; kind = `Feature { feature_id; feature_path = dst_feature }
        }
    in
