...
let pause =
  Command.async'
    ...
    ~readme:(fun () -> "This command blocks until the file system is frozen.\n")
    ...
    in
    fun () -> Persistent_state_serializer.Pause.rpc_to_server_exn { with_timeout })
;;
...
let resume =
  Command.async'
    ...
    (let open Command.Let_syntax in
    let%map_open () = return () in
    fun () -> Persistent_state_serializer.Resume.rpc_to_server_exn ())
;;
...
let command =
  Command.group
    ~summary:"control Iron server's serializer of persistent state"
    ~readme:(fun () ->
      "Iron server maintains its state on the file system via a \"serializer\", which \
       has a queue\n\
       of pending changes to be made to the file system to reflect state changes.  \
       Whenever Iron\n\
       server commits to a state change, it enqueues a representation of the state \
       change in its\n\
       serializer.\n\n\
       ...
       persistent state.  The backup process temporarily [pause]s processing of the \
       serializer\n\
       queue, copies the state elsewere on disk, and then [resume]s processing of the \
       queue.\n")
    [ "pause", pause
    ; "resume", resume
