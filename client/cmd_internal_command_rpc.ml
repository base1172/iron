...
let supported_by_iron_lib =
  Command.async'
    ~summary:"output a table of command RPCs supported by Iron public lib"
    (let open Command.Let_syntax in
    let%map_open () = return ()
    and display_ascii = display_ascii
    and max_output_columns = max_output_columns
    and names_only = no_arg_flag "-names-only" ~doc:"output names only" in
    fun () ->
...
let supported_by_command =
  Command.async'
    ~summary:"output a table of command RPCs implemented by the binary"
    (let open Command.Let_syntax in
    let%map_open () = return ()
    and display_ascii = display_ascii
...
let referenced_by_fe_file =
  Command.async'
    ~summary:
      "output a table of command RPCs referenced by the [Fe] module in the Iron public \
       lib"
    (let open Command.Let_syntax in
...
let call =
  Command_rpc.Command.create
    ~summary:"spawn local command RPC server"
    Implement_public_lib.rpc_implementations
;;

let command =
  Command.group
    ~summary:"RPC access for fe client"
    [ "call", call
