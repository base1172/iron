...
let print_table ~table_name ~columns ~rows ~display_ascii ~max_output_columns =
  ...
  then (
    ...
    printf
      "%s%s"
      (if String.is_empty table_name then "" else table_name ^ ":\n")
...
let show_assigned assigned ~display_ascii ~max_output_columns =
  ...
  then (
    let rows =
      Feature_table.create assigned Assigned.feature_path (fun ~feature assigned_option ->
        feature, assigned_option)
    in
    let columns =
      Ascii_table.Column.(
        ...
        let review_lines ~header f =
          string
            ...
            ~show:`If_not_empty
            ~show_zero:false
            (if_assigned (fun a -> if not a.user_is_reviewing then "" else f a.line_count))
        in
        ...
        ; review_lines ~header:"review" (fun t ->
            Line_count.to_review_column_shown t |> Review_or_commit.to_string_hum)
        ; review_lines ~header:"follow" (fun t -> t.review.follow |> Int.to_string_hum)
        ; catch_up_lines ~header:"catch-up" (fun t ->
            Line_count.Catch_up.total t |> Int.to_string_hum)
        ; string
            ~header:"next step"
...
let show_unclean_workspaces
  ...
  Map.iteri unclean_workspaces_on_machine ~f:(fun ~key:machine ~data:unclean_workspaces ->
    let columns, rows =
      Cmd_workspace_unclean.unclean_workspaces_columns_and_rows unclean_workspaces
    in
    print_table
    ...
    Ascii_table.Column.(
      ...
      @ (let empty = [], "" in
         ...
         let rev_facts header field ~mention_crs =
           string
             ~header
             (attr_cell field (function
               | Pending_since _ -> [ `Yellow ], "pending"
               | Known
                   { Todo.Rev_facts.is_conflict_free; is_cr_clean; obligations_are_valid }
                 ->
         ...
         ; int_or_error
             ~header:"#left"
             (cell Feature_info.num_reviewers_with_review_remaining)
         ])
      @ [ string
            ~header:"next step"
  ...
  let columns =
    Ascii_table.Column.
      ...
      ; string
          ...
          (cell (fun cr_soon ->
             concat
               [ Relpath.to_string (Cr_soon_in_feature.path cr_soon)
               ; ":"
      ...
      ; string
          ...
          (cell (fun cr_soon ->
             Option.value_map
               (Cr_soon_in_feature.active_in cr_soon)
               ~default:""
               ~f:Feature_path.to_string))
      ]
  in
  let rows = List.sort cr_soons ~compare:Cr_soon_in_feature.For_sorted_output.compare in
  print_table
    ~table_name:cr_soons_table_name
    ~columns
    ~rows
    ~display_ascii
    ~max_output_columns
...
let show_bookmarks_without_feature
  ...
  let rows =
    List.concat_map
      (List.sort bookmarks_without_feature ~compare:(fun (r1, _) (r2, _) ->
         Remote_repo_path.compare r1 r2))
      ~f:(fun (r, bookmarks) ->
        match bookmarks with
        | [] -> []
        | b :: bookmarks ->
          (Remote_repo_path.to_string r, b) :: List.map bookmarks ~f:(fun b -> "", b))
  in
  let columns =
    Ascii_table.Column.
      ...
      ; string
          ~header:"bookmark"
          (cell (fun (_, b) -> Bookmark_without_feature.bookmark b))
      ]
  in
  print_table
    ~table_name:bookmarks_without_feature_name
    ~columns
    ~rows
    ~display_ascii
    ~max_output_columns
  ...
  Command.async'
    ...
    and owned_by_me_names =
      no_arg_flag
        ...
        ~doc:
          (sprintf
             "list features names, including features from the '%s' table"
             owned_table_name)
    ...
    and do_not_show_cr_soons = do_not_show_cr_soons
    and do_not_show_unclean_workspaces = do_not_show_unclean_workspaces
    and include_all_owned_features =
      no_arg_flag
        "-include-all-owned-features"
    ...
    and releasable = no_arg_flag "-releasable" ~doc:"show only releasable features" in
    fun () ->
      ...
      then
        failwithf
          "The flags [%s] and [%s] are mutually exclusive"
          Switch.do_not_show_unclean_workspaces
          switch_my_unclean_workspaces
          ();
      let client_config = Client_config.get () in
      let do_not_show_cr_soons =
        do_not_show_cr_soons || Client_config.Cmd.Todo.do_not_show_cr_soons client_config
      in
        ...
        | Some ancestor ->
          ...
          ; unclean_workspaces =
              Map.filter_map reaction.unclean_workspaces ~f:(fun unclean_workspaces ->
                match
                  List.filter unclean_workspaces ~f:(fun t ->
                    should_include t.feature_path)
                with
                | [] -> None
                | _ :: _ as unclean_workspaces -> Some unclean_workspaces)
          ; owned = List.filter reaction.owned ~f:(fun t -> should_include t.feature_path)
          ; watched =
      ...
      let { Todo.Reaction.assigned
        ...
        then reaction
          ...
          ; owned = List.filter reaction.owned ~f:(fun t -> should_include t.next_steps)
          ; watched =
      ...
      then
        ...
        then
          ...
          then
            show_unclean_workspaces unclean_workspaces ~display_ascii ~max_output_columns;
          ...
          then
            show_feature_info
              watched
              ~table_name:watched_table_name
              ~display_ascii
              ~max_output_columns;
          if my_cr_soons || (force_show && not do_not_show_cr_soons)
          then
            show_cr_soons
              (Cr_soon_multiset.to_list cr_soons)
              ~display_ascii
              ~max_output_columns;
          if force_show
          then
            show_bookmarks_without_feature
              bookmarks_without_feature
              ~display_ascii
              ~max_output_columns)))
;;
