...
let set_max_subscriptions_per_user =
  Command.async'
    ~summary:"set the maximum number of subscriptions per user"
...
let set_max_subscriptions_global =
  Command.async'
    ~summary:"set the maximum number of subscriptions globally"
    (let open Command.Let_syntax in
    let%map_open () = return ()
    and max = anon ("max" %: int) in
    fun () ->
...
let drop_all_by_users =
  Command.async'
    ~summary:"drop all subscriptions opened by a particular user or all users"
    (let open Command.Let_syntax in
    let%map_open () = return ()
    and user = for_or_all_required in
    fun () -> With_event_subscriptions.rpc_to_server_exn (Drop_all_by_user user))
;;

let show =
  Command.async'
    ~summary:"output the current limits and subscriptions"
    (let open Command.Let_syntax in
...
let command =
  Command.group
    ...
    ~readme:(fun () ->
      concat [ "Operations side effecting the subscriptions require admin privileges.\n" ])
