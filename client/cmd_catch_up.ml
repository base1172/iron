  ...
  Command.async'
    ...
    and only_those_reviewed_by =
      flag
        ...
        (optional string)
        ~doc:
          "USER_BLANG only clear those catch-up reviewed by a user satisfying the \
           specified boolean expression.  default is [true]."
    in
    fun () ->
      ...
      let only_those_reviewed_by =
        ...
        | None -> Blang.true_
        | Some str ->
          (try str |> Sexp.of_string |> [%of_sexp: Unresolved_name.t Blang.t] with
           | exn ->
             raise_s
               [%sexp
                 (concat
                    [ "invalid boolean expression provided with "
                    ; only_those_reviewed_by_switch
                    ]
                   : string)
                 , (str : string)
      ...
      Clear_catch_up_sessions.rpc_to_server_exn
        { feature_path; for_; ok_if_nothing_cleared; only_those_reviewed_by })
;;
...
let get_catch_up_session_exn feature_path for_ =
  ...
  | `Up_to_date ->
    failwithf
      !"no review to catch up on for %{User_name} in %{Feature_path}"
      for_
      feature_path
      ()
;;
...
let diff =
  Command.async'
    ...
    and which_files = which_files in
    fun () ->
      let open! Deferred.Let_syntax in
      let feature_path = ok_exn feature_path in
      let%bind repo_root =
      ...
      in
      let%bind { Get_catch_up_session.Catch_up_session.diff4s_to_catch_up
               ; catch_up_session_id
  ...
  Command.async'
    ...
    fun () ->
      ...
      | `Up_to_date ->
        raise_s
          [%sexp
            "catch up is up to date, cannot mark file"
      ...
      | `Catch_up_session
        ...
        let ids_to_mark, invalid_files =
          List.partition_map paths_in_repo ~f:(fun path_in_repo ->
            ...
            | Some diff4 -> First (Diff4_in_session.id diff4))
        in
        if not (List.is_empty invalid_files)
        then
          raise_s
            [%sexp
              "files not found in the current session"
              , (invalid_files : Path_in_repo.t list)];
        Catch_up_diffs.rpc_to_server_exn
          { for_; feature_path; catch_up_session_id; diff4_in_session_ids = ids_to_mark })
;;

module Attribute = struct
  module T = struct
    type t = Session_id [@@deriving enumerate, sexp]
  end
...
let show =
  Command.async'
    ...
    and attributes = Attribute.sequence in
    fun () ->
      let open! Deferred.Let_syntax in
      let feature_path = ok_exn feature_path in
      let%map ({ Get_catch_up_session.Catch_up_session.catch_up_session_id
        ...
        get_catch_up_session_exn feature_path for_
      in
      let diff4s_to_review = diff4s_to_catch_up |> List.map ~f:Diff4_to_review.catch_up in
      let get_attribute : Attribute.t -> Sexp.t = function
        | Session_id -> catch_up_session_id |> [%sexp_of: Session_id.t]
      in
      match attributes with
      | [] ->
        if not omit_header_and_description
        then Cmd_review.print_catch_up_header_and_description catch_up_session;
        if not omit_attribute_table
        then
          Cmd_review.print_catch_up_attribute_table
            catch_up_session
            ~display_ascii
            ~max_output_columns;
        Cmd_review.print_introduction_summary_for_review
          ...
          ~display_ascii
          ~max_output_columns
      | [ attribute ] -> print_endline (get_attribute attribute |> Sexp.to_string_hum)
      | attributes ->
        attributes
...
let command =
  Command.group
    ~summary:"manage your catch up duties"
    [ "clear", clear
    ; "diff", diff
    ; "is-needed", is_needed
