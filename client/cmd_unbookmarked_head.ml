...
let prune ~root_feature rev =
  ...
  then return ()
    ...
    match%map
      Async_interactive.Job.run !"Pruning head: %{Rev#hum}" rev ~f:(fun () ->
        Process.run
          ()
          ...
          ~args:
            ...
            ; "-repo"
            ; concat [ "iron-"; Feature_name.to_string root_feature ]
            ])
    with
    | Ok (_stdout : string) -> ()
    | Error (_ : Error.t) ->
...
let prune_command =
  Command.async'
    ~summary:"prune an unbookmarked head, so it is no longer a head"
    ...
    fun () ->
      ...
      let%bind repo_root =
        Cmd_workspace.repo_for_hg_operations_exn
          (Feature_path.of_root root_feature)
          ~use:`Clone
      in
...
let command =
  Command.group ~summary:"manage unbookmarked heads" [ "prune", prune_command ]
;;
