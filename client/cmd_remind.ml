...
let warn text ~subject users =
  ...
  in
  let%bind () =
    Deferred.List.iter (Set.to_list users) ~f:(fun u ->
...
let%test_module _ =
  (module struct
    let%test _ =
      ...
      String.equal subject "subject" && String.equal body "foo\nbar"
    ;;

    let%test _ =
      let text = "Subject: subject\n\nfoo" in
      let subject, body = parse_subject text in
      String.equal subject "subject" && String.equal body "\nfoo"
    ;;
...
let command =
  Command.async'
    ...
    ~readme:(fun () ->
      concat
        [ "When run interactively the command will show the remind message and give the \
           user the\n\
           opportunity to edit it.  Otherwise, the email is sent without requiring a \
           confirmation.\n"
        ])
    ...
    and just_print_recipients =
      no_arg_flag
        "-just-print-recipients-and-exit"
    ...
    fun () ->
      ...
      let recipients = Set.to_list users |> List.map ~f:User_name.to_string in
      let cc =
        Option.value_map cc ~default:[] ~f:Set.to_list |> List.map ~f:User_name.to_string
      in
      ...
      then (
        let recipients =
          if no_email_to_send
          then []
          else recipients @ cc |> List.sort ~cmp:String.compare
        in
      ...
      then (
          ...
          match%bind
            ...
            then return `Yes
              ...
              let%bind () =
                ...
                | No_update_expected_due_to_iron_bug _ | No_update_expected -> return ()
        ...
        let draft_msg =
          ...
          in
          String.concat
            [ "Subject: reminder for Iron feature "
            ; feature_path
            ; "\n"
