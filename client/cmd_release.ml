...
let main { Fe.Release.Action.feature_path; for_; included_features_order } =
      ...
      (match parent.release_process with
       ...
       | Direct ->
         ...
         then
           raise_s
             [%sexp
               "Failed to release feature"
               , (feature_path : Feature_path.t)
               , { feature_base : Node_hash.First_12.t
                 ; parent_tip : Node_hash.First_12.t
                 }])
  in
  ...
  let%bind () =
    ...
    | (`Released_and_cleared _ | `Released_and_archived) as disposition ->
      ...
      in
      if am_functional_testing || not (Set.mem feature.send_email_upon Release)
      then return ()
      else (
        ...
        Sendmail.send
          ...
          ~recipients:
            (List.map (Set.to_list send_release_email_to) ~f:Email_address.to_string))
  in
  let%bind () =
    match disposition with
    | `Not_released__push_to_hydra ->
      ...
      Deferred.unit
    | `Released_and_cleared _ -> Deferred.unit
    | `Released_and_archived -> Cmd_workspace.If_enabled.delete_workspace feature_path
  in
  let%bind () =
    Async_interactive.printf
      ...
      (match disposition with
       ...
       | `Released_and_cleared reasons ->
         concat
           [ "Released, but didn't archive -- "
           ; Release.Reasons_for_not_archiving.to_string_hum reasons
           ]
         ...
         | Some parent_workspace ->
           ...
           then Deferred.unit
             ...
             Async_interactive.Job.run
               "Updating parent workspace to new released tip"
               ~f:(fun () ->
