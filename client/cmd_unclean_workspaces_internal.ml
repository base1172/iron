...
let remove_user =
  Command.async'
    ...
    let%map_open () = return ()
    and user = anon ("USER" %: Arg_type.create User_name.of_string) in
    fun () -> With_unclean_workspaces.rpc_to_server_exn (Remove_user user))
;;
...
let command =
  Command.group
    ...
    [ ( "remove-machine"
      , Cmd_has_moved.command ~moved_to:[ "fe"; "workspace"; "unclean"; "remove-machine" ]
      )
