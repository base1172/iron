...
let command =
  ...
  Command.async'
    ...
    and in_place = flag in_place_switch no_arg ~doc:" update files in place"
    and max_concurrent_jobs =
      flag
        max_concurrent_jobs_switch
        (optional int)
        ~doc:
          (sprintf
             "NUM allow NUM jobs at once when updating files in place.  default is %d"
             default_max_concurrent_jobs)
    and extra_cr_comment_headers =
      flag
        "-extra-cr-comment-header"
        (listed string)
        ~doc:"STR additional header to recognize as a CR"
    and files =
      map
        (anon (non_empty_sequence_as_pair ("FILE" %: resolved_file_path_arg_type)))
        ~f:(fun (x, l) -> x :: l)
    in
    fun () ->
      ...
      let max_concurrent_jobs =
        ...
        | Some value ->
          ...
          then
            failwithf
              "Switch [%s] may be passed only when [%s] is supplied."
              max_concurrent_jobs_switch
              in_place_switch
              ();
          value
      ...
      in
      let how =
        if in_place then `Max_concurrent_jobs max_concurrent_jobs else `Sequential
      in
      Deferred.List.iter files ~how ~f:(fun file ->
        let%bind contents =
          strip_crs_exn
            Cr_comment_format.latest
            ~replace_with
            ~extra_cr_comment_headers
            file
        in
        if in_place
        then Writer.save (Abspath.to_string file) ~contents
        else (
