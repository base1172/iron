...
let compute_unclean_workspace_and_update_server ~repo_root =
  ...
  | Some current_feature ->
    Cmd_workspace_unclean.compute_and_update_server_exn
      ~for_:User_name.unix_login
...
let main ~repo_root ~hook_name =
  ...
  in
  Deferred.all_unit [ compute_unclean_workspace ~repo_root; maybe_send_push_event ]
;;
      ...
      concat
        ...
        ; "\n\n\
          ...
          \  $(hg root)/.hg/iron-hooks/${HOOK-NAME}/${TMP_RUNDIR}\n\n\
           Upon completion of the background hook, if there is no error the temporary \
           directory\n\
           is deleted.  In case of an error, the directory is kept untouched for \
           inspection.\n"
        ])
    (let open Command.Let_syntax in
    let%map_open () = return ()
    and hook_name = enum_anon "HOOK-NAME" (module Hook_name)
    and fg = no_arg_flag "-fg" ~doc:" run in foreground, don't daemonize" in
    fun () ->
