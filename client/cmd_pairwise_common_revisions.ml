  ...
  Command.async'
    ...
    and subtrees =
      feature_path_flagged_listed
        ~label:"subtree"
        ~doc:"FEATURE check descendants of this feature"
    and features = anon_feature_paths in
    fun () ->
      let open! Deferred.Let_syntax in
      let subtrees = ok_exn subtrees in
      let features = ok_exn features in
      let%bind repo_root = Cmd_workspace.repo_for_hg_operations_use_clone_exn features in
      ...
      let features =
        ...
        List.sort features ~compare:(fun f1 f2 ->
      ...
      let%map revs =
        Deferred.List.map
          ...
          let%map revs = Hg.create_revs repo_root (Revset.for_feature ~base ~tip) in
