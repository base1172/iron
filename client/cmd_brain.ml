...
let diff =
  Command.async'
    ~summary:"show the diff a reviewer knows for a feature"
...
let show_brain
  ...
  ~sort_build_order
  =
  let%bind { brain; _ } = Get_brain.rpc_to_server_exn { feature_path; for_ } in
  let brain = List.map brain ~f:(fun d -> d.diff2) in
  let brain =
  ...
  let%map brain =
    ...
    then return brain
    else Build_order.sort repo_root_and_kind_or_error brain Diff2.path_in_repo_at_tip
  in
  let brain = List.map brain ~f:Diff4.create_from_scratch_to_diff2 in
  print_string
    (Ascii_table.to_string
       (Diff4.summary brain ~sort:(not sort_build_order))
       ~display_ascii
       ~max_output_columns);
  brain
;;

let show_brain_for_all_or_user
  feature_path
  ~for_
  ~what_to_show
  ~display_ascii
...
let forget =
  Command.async'
    ~summary:"remove knowledge from a brain"
    (let open Command.Let_syntax in
    let%map_open () = return ()
    and feature_path = feature_path_or_current_bookmark
    and for_ = for_
          ...
          then
            ...
            match%bind
              Async_interactive.ask_yn
                ~default:false
                (sprintf
                   "Really forget above diffs for [%s]?"
...
let show =
  Command.async'
    ~summary:"show what a reviewer knows about a feature"
    (let open Command.Let_syntax in
    let%map_open () = return ()
    and display_ascii = display_ascii
    ...
    and what_to_show = which_files in
    fun () ->
      let open! Deferred.Let_syntax in
      let feature_path = ok_exn feature_path in
      let%bind repo_root_and_kind_or_error =
...
let command =
  Command.group
    ~summary:"manage and inspect a reviewer's brain for a feature"
    [ "diff", diff; "forget", forget; "show", show ]
;;
