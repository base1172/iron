...
include struct
  ...
  let idempotent =
    no_arg_flag "-idempotent" ~doc:"do not fail if setting is already as requested"
  ;;
end
  ...
  | `Using_locked_sessions ->
    concat
      [ "Some users may opt in here to try out an experimental feature:\n\n\
         When they run [fe review] or [fe session diff] their session is automatically \
         locked,\n\
         as if they had run [fe session lock].\n\n\
         If the experience is positive, this may become the new behavior implemented for \
         everyone.\n"
      ]
;;
...
let make_command user_set =
  let change_command change ~summary =
    Command.async'
      ...
      and user_names = user_names in
      fun () ->
        if Set.is_empty user_names then failwith "One must supply at least one user.";
        User_set.Change.rpc_to_server_exn { user_set; user_names; change; idempotent })
  in
  ...
  let get_command =
    Command.async'
      ...
      fun () ->
        ...
        Set.iter users ~f:(fun user -> print_endline (User_name.to_string user)))
  in
  Command.group
    ~summary:""
    ~readme:(readme user_set)
    [ "add", add_command; "remove", remove_command; "get", get_command ]
;;
