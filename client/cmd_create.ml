...
let default_description feature_path =
  let default () =
    sprintf !"WRITE ME (replace with a description of %{Feature_path})" feature_path
  in
  ...
  then
    ...
    | Ok from_template ->
      ...
      let subst = function
        ...
        | s -> s
      in
      Caml.Buffer.add_substitute buffer subst from_template;
      Buffer.contents buffer)
;;

let main
  { Fe.Create.Action.feature_path
  ; base
  ; tip
  ; description
  ; owners
  ...
  (match Feature_path.is_root feature_path, remote_repo_path with
   | true, None ->
     failwithf "must supply %s when creating a root feature" Switch.remote_repo_path ()
   | false, Some _ ->
     failwithf
       "cannot supply %s when creating a non-root feature"
  ...
  let%bind repo_is_clean =
    ...
    then return None
      ...
      status_cleanliness |> ok_exn |> Option.some)
  in
  ...
  let%bind () =
    if_bookmark (fun () ->
      ...
      then
        failwithf
          "repository already contains a bookmark named [%s]"
  ...
  let%bind () =
    if_bookmark (fun () ->
      ...
      then
        Hg.create_bookmark_and_update_to_it
          ?repo_is_clean
          repo_root
          remote_repo_path
          feature_path
...
let command =
  Command.async'
    ...
    fun () ->
      let reviewing =
        if set_reviewing_whole_feature_only && set_reviewing_first_owner_only
        then
          failwithf
            ...
            ()
        else if set_reviewing_whole_feature_only
        ...
        then `First_owner
          ...
          | Some value -> value
          | None ->
            if am_functional_testing then `First_owner else `Whole_feature_reviewers)
      in
      main
        { feature_path = ok_exn feature_path
        ; base
        ; tip
        ; description
        ; owners
