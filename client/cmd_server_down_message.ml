...
let set_message t =
  ...
  Editor.with_temp_file (fun file ->
    ...
    let%bind () = Server_down_message.save_exn ~perm:0o644 t (Abspath.of_string file) in
    let%map output =
      Process.run
        ~prog:"sink"
        ~args:
...
let rec edit_message old_message ~f =
  ...
  print_endline
    (Patdiff.Patdiff_core.patdiff
       ~rules
       ()
       ~prev:{ name = "old"; text = old_message }
       ~next:{ name = "new"; text = new_message });
...
let roll_start_command =
  Command.async'
    ...
    fun () ->
      ...
      in
      edit_message old_message ~f:(fun message ->
  ...
  Command.async'
    ...
    fun () ->
      ...
      | Some temporary_message ->
        ...
        (match%bind
           Async_interactive.ask_dispatch_with_help
             "Clear this temporary roll message ?"
...
let roll_command =
  Command.group
    ~summary:"roll message commands"
    [ "start", roll_start_command; "end", roll_end_command ]
;;

let command =
  Command.group
    ~summary:"server-down message commands"
    [ "edit", edit_command; "show", show_command; "roll", roll_command ]
;;
