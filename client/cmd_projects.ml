...
module Tagged_file = struct
  ...
  let create ~file_name ~tags ~lines = { file_name; tags; lines }
...
module Project = struct
  ...
  let create ~enclosing_dir ~tagged_files =
    { enclosing_dir
    ; tagged_files = List.sort tagged_files ~compare:Tagged_file.compare_by_file_name
...
let load repo_root : t Deferred.t =
  ...
  let%bind obligations =
    Cmd_obligations.create_obligations
      repo_root
      ~dirs:(`Below below)
  ...
  Hashtbl.iteri obligations.by_path ~f:(fun ~key:path_in_repo ~data:review_attributes ->
    ...
    then (
      ...
      Hashtbl.add_multi
        ...
        ~key:dir
        ~data:
          (Throttle.enqueue read_throttle (fun () ->
             let%map lines =
               Reader.file_lines
  ...
  in
  { projects_by_dir = Path_in_repo.Table.of_alist_exn alist; obligations }
;;
...
let list =
  ...
  Command.async'
    ...
    and directory_only =
      no_arg_flag directory_only_switch ~doc:"only output the directories of the projects"
    and show_readme =
      no_arg_flag show_readme_switch ~doc:"include the entire README file in the output"
    and tags =
      flag
        "-tag"
        (optional tag_blang_arg)
    ...
    fun () ->
      ...
      let projects =
        ...
        |> List.sort ~compare:Project.alphabetic_compare
      ...
      List.iter projects ~f:(fun (name, project) ->
        if show_readme
        then
          List.iter project.tagged_files ~f:(fun tagged_file ->
            let title =
              sprintf "%s/%s:" name (File_name.to_string tagged_file.file_name)
            in
            print_endline
              (sprintf
                 "%s (tags %s)\n%s\n%s\n"
...
let search =
  Command.async'
    ...
    (let open Command.Let_syntax in
    let%map_open () = return ()
    and regex = anon ("REGEXP" %: Arg_type.create Regex.create_exn) in
    fun () ->
      let open! Deferred.Let_syntax in
      let repo_root = ok_exn Repo_root.program_started_in in
      ...
      let matching_files =
        ...
        |> List.concat_map ~f:(fun project ->
             List.filter_map project.tagged_files ~f:(fun tagged_file ->
               if Set.exists tagged_file.tags ~f:(fun tag ->
                    Regex.matches regex (Tag.to_string tag))
        ...
        |> List.sort ~compare:Path_in_repo.default_review_compare
      in
...
let command =
  Command.group
    ~summary:"handle tagged files and projects directories"
    [ "list", list; "list-tags", list_tags; "search", search ]
;;
