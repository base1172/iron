  ...
  Command.async'
    ...
    and subtrees =
      feature_path_flagged_listed
        ~label:"subtree"
        ~doc:"FEATURE show this feature and its descendants"
    and features = anon_feature_paths
    and args = dash_dash_rest ~doc:" Give remaining arguments to hg glog" in
    fun () ->
      ...
      in
      let revs =
        List.concat_map features ~f:(fun { feature_path = _; base; tip } -> [ base; tip ])
      in
      let%bind () =
        if do_not_pull
        then return ()
      ...
      let%map log =
        Hg.log
          ~args:("-G" :: args)
          repo_root
          (Hg.Revset.or_ (List.map ~f:Hg.Revset.of_rev revs))
      in
      log |> ok_exn |> print_endline)
;;
