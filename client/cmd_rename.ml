...
let main { Fe.Rename.Action.from; to_; skip_gca_check } =
  ...
  let%bind () =
    ...
    | false ->
      ...
      then
        raise_s
          [%sexp
            "greatest common ancestor of feature and its new parent is not equal to the \
             feature's base"
...
let command =
  ...
  Command.async'
    ...
    ~readme:(fun () ->
      concat
        [ "The target feature is not restricted to be a sibling of the original feature, \
           i.e.\n\
           renaming [jane/a/b] to [jane/c] (or vice versa) is permissible.\n\n\
           Using [-skip-gca-check] skips the check that the greatest common ancestor of \
           the current\n\
           tip and the new base is the current base.  Skipping the check may mean future \
           rebases and\n\
           renames will be rejected.\n\n\
           When the basename of the feature is not meant to change, one can use ["
        ; new_parent_switch
        ; "].\n\
           For example, to rename [jane/a/keep-this-basename] into \
           [jane/b/keep-this-basename]:\n\n\
    ...
    and to_ =
      ...
      and rename_to_child_of =
        flag
          new_parent_switch
          (optional (feature_arg_type ~match_existing_feature:true))
          ~doc:
            "FEATURE Keep the same basename for the feature, rename it as a child of the \
             supplied parent feature."
      in
      Or_error.try_with (fun () ->
        ...
        | None, Some new_parent -> `Rename_to_child_of (ok_exn new_parent)
        | None, None -> failwith "The new feature-path needs to be specified."
        | Some _, Some _ ->
          failwithf
            "One cannot supply %s when the target FEATURE name is supplied."
            new_parent_switch
            ())
    and skip_gca_check = flag "-skip-gca-check" no_arg ~doc:" <see above>"
    and () = even_if_locked in
    fun () ->
