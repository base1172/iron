...
let create ~show_completed_review (line_count_by_user : (User_name.t * Line_count.t) list)
  =
  let rows =
    line_count_by_user
  ...
  let columns =
    Ascii_table.Column.(
      ...
      ; lines ~header:"follow" (f (fun line_count -> line_count.review.follow))
      ; lines
          ~header:"catch-up"
          (f (fun line_count -> Line_count.Catch_up.total line_count.catch_up))
      ; lines
          ~header:"completed"
          (f' (fun line_count ->
