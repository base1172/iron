...
module Kind = struct
  ...
  let to_string_hum = function
    | Catch_up catch_up_kind ->
      concat [ "Catch-up.  "; Catch_up_kind.to_string_hum catch_up_kind ]
...
let review { Diff4_in_session.And_review_kind.diff4_in_session; review_kind } =
  { diff4_in_session; kind = Review review_kind }
;;
...
let by_kind_of_decreasing_priority ts =
  ts
  |> List.stable_sort ~compare:compare_to_sort_by_decreasing_priority
