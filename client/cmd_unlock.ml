...
let main_internal
  ~verbose
  { Fe.Unlock.Action.feature_path; for_; lock_names; even_if_permanent }
  =
  ...
  Cmd_change.apply_updates_exn
    ...
    Unlock_feature.rpc_to_server_exn { feature_path; for_; lock_names; even_if_permanent })
;;
...
let command =
  Command.async'
    ...
    ~readme:(fun () ->
      concat
        [ "[fe unlock] succeeds only if the supplied locks are locked and the user \
           requesting to\n\
           unlock is the one who locked them.  You must supply at least one lock.  \
           Unlocking for\n\
           someone else is possible using [-for].  If the lock is permanent, it is \
           required to\n\
           supply the switch ["
    ...
    fun () ->
      ...
      main_internal ~verbose { feature_path; for_; lock_names; even_if_permanent })
;;
