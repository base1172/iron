...
let bookmarks_without_feature =
  Command.async'
    ~summary:"output bookmarks that don't have a feature"
    (let open Command.Let_syntax in
    let%map_open () = return ()
    and remote_repo_path = remote_repo_path
    and user = for_or_all_default_me in
    fun () -> dump (Bookmarks_without_feature (remote_repo_path, user)))
;;

let feature =
  Command.async'
    ~summary:"output a feature"
    (let open Command.Let_syntax in
    let%map_open () = return ()
    and feature_path = feature_path_or_current_bookmark in
    fun () -> dump (Feature (ok_exn feature_path)))
;;
...
let hash_consing_cache =
  ...
  Command.async'
    ~summary:"output stats or values of the hash-consing cache"
    (let open Command.Let_syntax in
    let%map_open () = return ()
    and what_to_dump =
      let%map values =
        no_arg_flag values_switch ~doc:"output the whole values instead of the stats"
      and module_values =
        flag
          module_values_switch
          (optional string)
      ...
      Or_error.try_with (fun () ->
        ...
        | _ ->
          failwithf
            ...
            ())
    in
    fun () -> dump (Hash_consing_cache (ok_exn what_to_dump)))
;;

let review_analysis =
  Command.async'
    ~summary:"output the review analysis of a feature"
    (let open Command.Let_syntax in
    let%map_open () = return ()
    and feature_path = feature_path_or_current_bookmark in
    fun () -> dump (Review_analysis (ok_exn feature_path)))
;;

let review_lines =
  Command.async'
    ~summary:"output details of review lines"
    (let open Command.Let_syntax in
    let%map_open () = return ()
    and feature_path = feature_path_or_current_bookmark
    and for_or_all = for_or_all_default_me in
    fun () -> dump (Review_lines (ok_exn feature_path, for_or_all)))
;;

let review_manager =
  Command.async'
    ~summary:"output a review manager"
    (let open Command.Let_syntax in
    let%map_open () = return ()
    and feature_path = feature_path_or_current_bookmark
    and for_or_all = for_or_all_default_me in
    fun () -> dump (Review_manager (ok_exn feature_path, for_or_all)))
;;

let state =
  Command.async'
    ~summary:"output the entire server state"
    (let open Command.Let_syntax in
...
let user_info =
  Command.group
    ~summary:"output user info"
    (List.map
       ~f:(fun (name, which, description) ->
         ( name
         , Command.async'
             ~summary:(sprintf "output %s" description)
             (let open Command.Let_syntax in
             let%map_open () = return () in
...
let command =
  Command.group
    ~summary:"output part of server internal state as a sexp"
