...
let command =
  ...
  Command.async'
    ...
    ~readme:(fun () ->
      concat
        [ "The command synthesizes a review session containing all the requested \
           changes, and\n\
           starts the interactive review client loop in the terminal. It has two uses:\n\n\
           - debugging, for example in an attempt to re-run a review client for a \
           particular revs\n\
          \  diamond even though no such session exists on the server anymore\n\n\
           - trying to understand a merge, allowing to see a diff the way fe would have \
           shown it\n\n\
           Because obligations are not taken into account, the ddiff shown can differ \
           from what fe\n\
           would show.  On the plus side, you can see ddiffs despite obligations being \
           broken,\n\
    ...
    and read_diff4s_from_sexp_file =
      flag
        ...
        (optional file)
        ~doc:
          "/path/to/diff4s.sexp Read diff4s from a sexp file instead of computing them \
           based on some given revisions"
    and ddiff_diamond =
      ...
      |> map ~f:(fun d ->
           lazy
             ...
             then None
               ...
               Some
                 (Diamond.map d ~f:(function
                   | Some rev -> rev
                   | None ->
                     failwith
                       "either supply all 4 revisions of the diamond \
                        -{old,new}-{base,tip} or none of them"))))
    and diff_diamond =
      let%map base =
        flag "-base" (optional string) ~doc:"rev the hg revision for the base"
      and tip = flag "-tip" (optional string) ~doc:"rev the hg revision for the tip" in
      lazy
        (match base, tip with
         | None, None -> None
    ...
    fun () ->
      ...
      let what_to_diff =
        ...
        | _, _, _ ->
          failwith
            (concat
               [ "specify what to review, using exactly one of:\n"
      ...
      let%bind diff4s =
        ...
        | `Read_diff4s_from_sexp_file sexp_file ->
          ...
          Sexp.of_string_conv_exn stdin [%of_sexp: Diff4.Stable.V2.t list]
