...
let header feature_path =
  ...
  then sprintf "%s" (underline '=' feature_path_string)
    ...
    let path_as_tree =
      List.foldi (Feature_path.parts feature_path) ~init:[] ~f:(fun i ac feature_name ->
        "\n" :: Feature_name.to_string feature_name :: String.make (2 * i) ' ' :: ac)
      |> List.rev
      |> String.concat
    in
    sprintf
...
let render_included_features
  feature
  ~display_ascii
  ~max_output_columns
  ~show_attribute_table
  ~show_description
  ~show_included_feature_details
    ...
    concat
      ...
      @ maybe show_included_feature_details (fun () ->
          List.concat_map included_features ~f:(fun (r : Released_feature.t) ->
            ...
            @ maybe show_description (fun () -> [ sprintf "%s\n" r.description ])
            @ maybe show_attribute_table (fun () ->
                [ sprintf
                    "\n%s"
                    (Ascii_table.to_string
                       (Released_feature.attribute_table r)
                       ~display_ascii
                       ~max_output_columns)
                ]))))
;;
...
module Attribute = struct
  ...
  let args =
    ...
    and user_defined = properties_list_option ~switch:"property" ~verb:"show" in
    let user_defined =
  ...
  let get t (feature : Feature.t) ~included_features_order =
    match t with
    | Allow_review_for -> feature.allow_review_for |> [%sexp_of: Allow_review_for.t]
    | Base -> Rev.node_hash feature.base |> [%sexp_of: Node_hash.t]
    | Compilation_status -> feature.compilation_status |> [%sexp_of: Compilation_status.t]
    | Crs_are_enabled -> feature.crs_are_enabled |> [%sexp_of: bool]
    | Crs_shown_in_todo_only_for_users_reviewing ->
      feature.crs_shown_in_todo_only_for_users_reviewing |> [%sexp_of: bool]
    ...
    | Next_base_update -> feature.next_base_update |> [%sexp_of: Next_base_update.t]
    | Next_bookmark_update ->
    ...
    | Release_process -> feature.release_process |> [%sexp_of: Release_process.t]
    | Remote_repo_path -> feature.remote_repo_path |> [%sexp_of: Remote_repo_path.t]
    | Rev_zero -> Rev.to_string_40 feature.rev_zero |> [%sexp_of: string]
    | Review_is_enabled -> feature.review_is_enabled |> [%sexp_of: bool]
    ...
    | Send_email_to -> feature.send_email_to |> [%sexp_of: Email_address.Set.t]
    | Send_email_upon -> feature.send_email_upon |> [%sexp_of: Send_email_upon.Set.t]
    ...
    | Users_with_unclean_workspaces ->
      feature.users_with_unclean_workspaces |> Map.keys |> users
    | Xcrs_shown_in_todo_only_for_users_reviewing ->
      feature.xcrs_shown_in_todo_only_for_users_reviewing |> [%sexp_of: bool]
  ;;
end
...
let bool_or_error attribute = function
  ...
  | Ok bool -> if bool then [] else [ attribute, ([ `Red ], Bool.to_string bool) ]
;;
  ...
  fun ?last_known_at name rev rev_facts_or_pending ->
    ...
    @ maybe rev_facts_or_pending (fun rev_facts_or_pending ->
        or_pending
          ~is_archived
          (concat [ name; " facts" ])
          rev_facts_or_pending
          (fun rev_facts ->
            match Rev_facts.check rev_facts rev with
            | Ok true -> []
            | Ok false | Error _ ->
              let { Rev_facts.is_conflict_free; is_cr_clean; obligations_are_valid } =
                rev_facts
              in
              ...
              List.concat
                ...
                ; fact "is cr clean" is_cr_clean Rev_facts.Is_cr_clean.check
                ]))
;;
...
let maybe_empty_set set ~f = if Set.is_empty set then [] else f set
let maybe_empty_list list ~f = if List.is_empty list then [] else f list

let set_to_row name ~f:to_string ?(attrs = []) set =
  [ name, (attrs, set |> Set.to_list |> List.map ~f:to_string |> String.concat ~sep:", ")
  ]
;;
...
let release_process_row release_process =
  [ "release process", ([], Release_process.to_string_hum release_process) ]
;;

let who_can_release_into_me_row who_can_release_into_me =
  [ ( "who can release"
    , ([], Who_can_release_into_me.to_string_hum_as_parent who_can_release_into_me) )
  ]
;;
...
let attribute_table_with_fields
  ~display_ascii
  ~max_output_columns
  ~next_steps
  ...
  let rows =
    ...
    let in_todo_for crs user_reviewing_only =
      [ ( crs ^ " shown in todo for"
        , ([], if user_reviewing_only then "users reviewing only" else "all") )
      ]
    in
    ...
    List.concat
      ...
      ; maybe crs_are_enabled (fun crs_are_enabled ->
          if (not (Option.value review_is_enabled ~default:false)) || not crs_are_enabled
      ...
      ; maybe next_bookmark_update (fun next_bookmark_update ->
          ...
          | Update_expected_since since ->
            if is_known_to_be_archived then [] else show `Yellow (pending_for ~since))
      ...
      ; maybe3 base tip base_is_ancestor_of_tip (fun base tip base_is_ancestor_of_tip ->
          or_pending "base is ancestor of tip" base_is_ancestor_of_tip (fun is_ancestor ->
            bool_or_error
              "base is ancestor of tip"
              (Rev_facts.Is_ancestor.check is_ancestor ~ancestor:base ~descendant:tip)))
      ; block
          "release into me"
             ...
             |> List.sort
                  ~cmp:
                    (Comparable.lexicographic
                       [ (fun (lock_name1, _) (lock_name2, _) ->
                           String.compare lock_name1 lock_name2)
                       ; (fun (_, (_, user1)) (_, (_, user2)) ->
             ...
             then []
               ...
               List.concat
                 [ maybe
                     crs_shown_in_todo_only_for_users_reviewing
                     (fun crs_shown_in_todo_only_for_users_reviewing ->
                     in_todo_for "CRs" crs_shown_in_todo_only_for_users_reviewing)
                 ; maybe
                     xcrs_shown_in_todo_only_for_users_reviewing
                     (fun xcrs_shown_in_todo_only_for_users_reviewing ->
                     in_todo_for "XCRs" xcrs_shown_in_todo_only_for_users_reviewing)
                 ...
                 ; maybe_empty_set
                     ~f:(user_set "whole feature follower")
                     whole_feature_followers
                 ; maybe_empty_set
                     ~f:(user_set "whole feature reviewer")
                     whole_feature_reviewers
                 ]))
      ]
  in
  let columns =
    Ascii_table.Column.
      [ string ~header:"attribute" (cell fst); string ~header:"value" (attr_cell snd) ]
  in
  Ascii_table.to_string
...
let attribute_table
  ...
  =
  attribute_table_with_fields
    ~display_ascii
    ~max_output_columns
    ?feature_id:
      (Option.some_if (show_feature_id || Is_archived.to_bool is_archived) feature_id)
    ...
    ?compilation_status_to_display:
      ...
      then
        Some
          (Compilation_status_to_display.of_compilation_status
             compilation_status
             feature_path
             ~feature_tip:tip
             ~show_full_compilation_status)
...
let print_line_count_table
  ...
  =
  print_table
    (Line_count_table.create line_count_by_user ~show_completed_review)
    ~display_ascii
    ~max_output_columns
;;
...
let review_sessions_in_progress_table user_set =
  ...
  let columns =
    Ascii_table.Column.
      [ of_to_string
          ~header:"review sessions in progress"
          User_name.to_string
          (cell (fun name -> name))
      ]
  in
...
let print_review_sessions_in_progress_table ~display_ascii ~max_output_columns user_set =
  print_table
    (review_sessions_in_progress_table user_set)
    ~display_ascii
    ~max_output_columns
;;
...
let print_unclean_workspaces_table ~display_ascii ~max_output_columns users =
  print_table (unclean_workspaces_table users) ~display_ascii ~max_output_columns
;;
...
let show_whole_feature
  ...
  then (
    ...
    | Ok users ->
      ...
      then
        print_review_sessions_in_progress_table users ~display_ascii ~max_output_columns
    | Error _ -> ());
  if show_unclean_workspaces_table
  then
    print_unclean_workspaces_table
      feature.users_with_unclean_workspaces
      ~display_ascii
      ~max_output_columns;
  printf
    "%s"
    (render_included_features
       feature
       ~display_ascii
       ~max_output_columns
       ~show_attribute_table
...
let display_description_org_mode ~description =
  print_string
    (concat
       (List.map (String.split_lines description) ~f:(fun line ->
          concat [ ": "; line; "\n" ])))
;;

let display_included_features_org_mode
  (feature : Feature.t)
  ~depth
  ~show_description
  ~show_diff_stat
  ...
  then return ()
    ...
    then return ()
      ...
      Deferred.List.iter included_features ~f:(fun (r : Released_feature.t) ->
        let depth =
          depth
          + (Feature_path.num_parts r.feature_path
  ...
  then (
    ...
    print_string
      (attribute_table
         ...
         ~show_next_steps
         ~show_full_compilation_status));
  display_included_features_org_mode
    feature
    ~depth
    ~show_description
    ~show_diff_stat
...
let print_user_list users =
  ...
  then users |> List.map ~f:User_name.to_string |> concat ~sep:"\n" |> print_endline
;;
...
let command =
  Command.async'
    ...
    and omit_review_sessions_in_progress_table =
      no_arg_flag
        "-omit-review-sessions-in-progress-table"
        ~doc:"don't include review sessions in progress"
    and omit_unclean_workspaces_table =
      no_arg_flag
        "-omit-unclean-workspaces-table"
        ~doc:"don't include the unclean workspaces table"
    and org_mode = no_arg_flag "-org-mode" ~doc:"produce output in org-mode format"
    and show_diff_stat =
      no_arg_flag
    ...
    and show_included_feature_details =
      no_arg_flag
        "-show-included-feature-details"
        ~doc:"show included-feature descriptions and attributes"
    and print_attribute =
      no_arg_flag "-print-attribute" ~doc:"print all required attributes and exit"
    ...
    and show_inheritable_attributes =
      no_arg_flag
        "-show-inheritable-attributes"
        ~doc:"show inheritable attributes in attributes table"
    and sexp = no_arg_flag "-sexp" ~doc:"print [Feature.t] as a sexp and exit"
    and show_feature_id = no_arg_flag "-show-feature-id" ~doc:"show feature id as well"
    and show_lock_reasons =
      no_arg_flag "-show-lock-reasons" ~doc:"show lock reasons as well"
    and show_full_compilation_status =
      no_arg_flag
        "-show-full-compilation-status"
    ...
    fun () ->
      ...
      then (
        ...
        in
        let%bind feature =
          Get_feature.Maybe_archived.rpc_to_server_exn { what_feature; what_diff = None }
        in
        if sexp
        then (
          feature |> [%sexp_of: Feature.t] |> Sexp.to_string_hum |> print_endline;
          return ())
        else (
          let show_diff_stat =
            ...
            then return None
              ...
              let not_in_clone () =
                failwith
                  (sprintf
                     "%s requires being in a clone of %s"
                     Switch.show_diff_stat
...
let show_lines_required_to_separate_ddiff_hunks =
  Command.async'
    ...
    let%map_open () = return ()
    and display_ascii = display_ascii
    and max_output_columns = max_output_columns in
    fun () ->
      let open! Deferred.Let_syntax in
      let%map rows =
        Get_lines_required_to_separate_ddiff_hunks.rpc_to_server_exn () >>| Map.to_alist
      in
      let columns =
        Ascii_table.Column.
          [ string
