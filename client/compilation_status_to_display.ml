...
type pending = Time.Span.t * rev [@@deriving sexp_of]
...
let to_status tip { Compilation_status.finished; pending } =
  { pending =
      (match
         List.find pending ~f:(fun t ->
           Node_hash.First_12.equal t.first_12_of_rev (Rev.to_first_12 tip))
       with
...
let status_row { pending; finished } =
  ...
  | None ->
    (match finished with
     | None -> show `Red (Next_step.to_string_hum Report_iron_bug)
  ...
  | Some (pending_for, Tip) ->
    (* tip is pending; normal situation during development *)
    let for_ = " for " ^ span_to_string pending_for in
    (match finished with
     | None -> show `Yellow ("tip pending" ^ for_)
  ...
  | Some (pending_for, Non_tip pending_rev) ->
    (* bookmark update is delayed *)
