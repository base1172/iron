...
let forget_hunks_of_diff4
  ...
  | true ->
    ...
    let lines =
      { Diamond.b1 = String.split_lines contents.b1
    ...
    let slices =
      ...
      Diamond.map2 file_names lines ~f:(fun source lines -> Slice.create ~source 0 lines)
    in
    let hunk =
      { Hunk.header_file_name
      ; scrutiny
      ; rev_names
      ; file_names
...
let code_change_hunks_of_diff4
  ~reviewer
  ~context
    ...
    if (not (Diff4_class.is_shown files_diff4_class))
       ...
       | `Reviewer reviewer ->
         (match Diff4.may_review diff4 reviewer with
          | `Dropped_from_follow | `Dropped_from_review | `Review_ownership_change -> true
          | `Nothing_to_review_or_follow | `Follow_lines | `Review_lines -> false)
    then return []
    else (
      let contents = fast_cat_or_empty fast_hg_cat_table in
      let%bind contents = Diamond.Deferred.all (Diamond.map files ~f:contents) in
...
let attributes_contents ~show_file_followers (attributed_file : Attributed_file.t) =
  ...
  | `Absent -> "<absent>"
  | `Present attributes ->
    let { Attributed_file.Attributes.hash = _
        ; owner
        ; review_obligation
...
let hunks_of_diff4
  ...
  let files = Diff4.diamond diff4 in
  let rev_names =
    Diamond.map files ~f:(fun file -> Attributed_file.rev file |> Rev.to_string_12)
  in
  let file_names =
    Diamond.map files ~f:(fun file ->
  ...
  List.concat [ attributes; errors; code ]
;;
...
let pull_and_update_for_catch_up_on_archived_feature
  ...
  let%bind () =
    Hg.pull
      ...
      ~even_if_unclean:true
      (`Rev review_session_tip)
  in
  let%bind () =
...
let pull_and_update
  ...
  let%bind local_feature_tip =
    ...
    | `Is_not_ancestor | `Unknown | `No_local_feature ->
      (match%bind Lazy_deferred.force_exn is_clean with
       ...
       | Ok repo_is_clean ->
         let%bind () =
           Hg.pull ~repo_is_clean repo_root (`Feature feature_path) ~from:remote_repo_path
         in
    ...
    | `Is_not_ancestor | `Unknown | `No_local_feature ->
      let%map review_session_tip_is_ancestor_of_local_bookmark =
        is_known_and_is_ancestor
          ~ancestor:review_session_tip
          ~descendant:local_feature_tip
      in
      (match review_session_tip_is_ancestor_of_local_bookmark with
       ...
       | (`Unknown | `Is_not_ancestor) as status ->
         let ancestor_error =
           ...
           | `Unknown ->
             Error.create
               "unknown review session tip.  Consider reviewing to tip"
               review_session_tip
               [%sexp_of: Rev.t]
           | `Is_not_ancestor ->
  ...
  let%bind () =
    ...
    then Deferred.unit
      ...
      | Error cleanliness_error ->
        raise_s
          [%sexp "Iron needs to [hg update] but won't", (cleanliness_error : Error.t)]
      | Ok repo_is_clean ->
        Hg.update
          repo_root
...
module Account_for_reviewer_warnings = struct
  type t =
    ...
    ; line_count_to_goal :
        Line_count.Review.t Or_error.t Or_pending.t
...
let print_introduction_summary_for_review
  ...
  (match (warn_reviewer : Account_for_reviewer_warnings.t option) with
   | None -> ()
   | Some { reviewer_in_feature; line_count_to_finish_session; line_count_to_goal } ->
     if not (Reviewer.equal reviewer_in_session reviewer_in_feature)
     then (
       Print.printf
         "Warning: what you need to review may have changed since this session was \
          created.\n\
     ...
     then (
       ...
       | ( Known (Ok line_count_from_session_end_to_goal)
         ...
         then (
           Print.printf
             "Warning: the feature has changed since this session was created.  It may \
              be more suitable\n\
              to review the feature to its most recent tip.  Consider committing your \
              session:\n";
           let table =
             let columns =
               Ascii_table.Column.(
  ...
  let int_size =
    List.fold diff4s_to_review ~init:0 ~f:(fun acc diff4_to_review ->
      num_lines_in_diff diff4_to_review |> Int.to_string_hum |> String.length |> max acc)
  in
  let int_to_string_padding int = sprintf "%*s" int_size (Int.to_string_hum int) in
  Print.printf
    "%d files to review%s: %d lines total\n"
    (count_total - count_reviewed)
    (if count_reviewed > 0 then sprintf " (%d already reviewed)" count_reviewed else "")
  ...
  List.iter diff4s_to_review ~f:(fun (kind, diff4s_to_review) ->
    ...
    List.iter diff4s_to_review ~f:(fun diff4_to_review ->
      ...
      Print.printf
        "   [%c] %s %s\n"
        (if Diff4_to_review.is_reviewed diff4_to_review then 'X' else ' ')
...
let create_files_for_review
  ...
  let%map paths_in_repo_by_rev =
    Deferred.List.map
      ~how:`Parallel
      (Hashtbl.to_alist paths_in_repo_by_rev)
      ~f:(fun (rev, paths) ->
      let%map map =
        Hg.cat
          repo_root
...
let confirm_review_session_id_exn
  repo_root
  ~action
  ~feature_path
  ~for_
  ...
  | `Review_session
    ...
    let diff4s_to_review =
      diff4s_in_session |> Array.map ~f:Diff4_to_review.review |> Array.to_list
    in
    let confirmed =
      if not !Async_interactive.interactive
      then return true
        ...
        print_introduction_summary_for_review
          ...
          ~warn_reviewer:
            (Some
               { reviewer_in_feature; line_count_to_finish_session; line_count_to_goal })
          ~diff4s_to_review
          ~display_ascii
          ~max_output_columns;
        Async_interactive.ask_yn
          ~default:false
          (sprintf
             !"Really %s this %{Feature_path} session%s?"
             action
             feature_path
...
let sort_review_files_using_list diff4s_to_review ~by =
  ...
  Array.stable_sort ~compare:(fun (i1, _d1) (i2, _d2) -> Int.compare i1 i2) keyed_diffs;
...
let sort_review_files_by_lines diff4s_to_review ~reviewer direction =
  ...
  Array.stable_sort diff4s_to_review ~compare;
  diff4s_to_review
;;
...
let sort_review_files repo_root_and_kind_or_error diff4s_to_review ~reviewer review_sort =
  ...
  | (`By_increasing_review_lines | `By_decreasing_review_lines) as review_sort ->
    sort_review_files_by_lines diff4s_to_review ~reviewer review_sort |> return
  | `Build_order ->
    let%map files =
      Build_order.sort
        repo_root_and_kind_or_error
        (Array.to_list diff4s_to_review)
...
let review_or_catch_up
  ...
  let%bind () =
    ...
    then return ()
    else if is_catch_up_on_archived_feature
    then
  ...
  let diff4s_to_review =
    ...
    Array.stable_sort
      diff4s_to_review
      ~compare:Diff4_to_review.compare_by_path_in_repo_at_f2_for_review;
    diff4s_to_review
  in
...
let may_modify_others_review_exn feature_path ~reason ~whose_review =
  ...
  if can_short_circuit
     || is_some
          (Sys.getenv "IRON_FUNCTIONAL_TESTING_CLIENT_DOES_NOT_CHECK_REVIEW_PERMISSIONS")
  then return ()
    ...
    fun () ->
      ...
      let maybe_sort_review_files =
        Option.first_some
          maybe_sort_review_files
          (Option.some_if
             (Client_config.Cmd.Review.sort_build_order client_config)
             `Build_order)
      in
      { emacs
      ; for_
...
let print_catch_up_header_and_description
  { Get_catch_up_session.Catch_up_session.feature_path; description; _ }
  =
  printf "%s\n" (Cmd_show.header_and_description feature_path ~description)
;;

let print_catch_up_attribute_table
  { Get_catch_up_session.Catch_up_session.whole_feature_reviewers
  ; owners
  ; base
  ; tip
  ; is_permanent
  ...
  =
  print_string
    (Cmd_show.attribute_table_with_fields
       ~display_ascii
       ~max_output_columns
...
let catch_up_review_loop
  ...
  Deferred.repeat_until_finished true (fun first_session ->
    let%bind reaction = Get_catch_up_session.rpc_to_server_exn { feature_path; for_ } in
    if raw
    then (
      reaction
      ...
      | `Up_to_date ->
        ...
        then
          printf "No review to catch up on in %s.\n" (Feature_path.to_string feature_path);
        return (`Finished ())
      | `Catch_up_session
        ...
        in
        let diff4s_to_review =
          diff4s_to_catch_up |> Array.of_list |> Array.map ~f:Diff4_to_review.catch_up
        in
        ...
        let mark_as_reviewed diff4s_to_review =
          Catch_up_diffs.rpc_to_server_exn
            ...
            ; feature_path
            ; catch_up_session_id
            ; diff4_in_session_ids = List.map diff4s_to_review ~f:Diff4_to_review.id
            }
        in
        print_catch_up_header_and_description catch_up_session;
        print_catch_up_attribute_table catch_up_session ~display_ascii ~max_output_columns;
        let%map result =
          review_or_catch_up
...
let review_loop
  ...
  printf "\n";
  let only_print_session = raw || (display_ascii && not !Async_interactive.interactive) in
  Deferred.repeat_until_finished true (fun first_session ->
    match%bind
    ...
    with
    | Error e -> if first_session then Error.raise e else return (`Finished ())
    | Ok
        ({ status; feature_tip; remote_rev_zero = _; remote_repo_path; may_second } as
        reaction) ->
      ...
      then (
        ...
        | `Review_session
          ...
          let diff4s_to_review =
            diff4s_in_session |> Array.map ~f:Diff4_to_review.review
          in
          let diff4s_to_review =
            ...
            | `Files files ->
              ...
              let diff4s_to_review =
                Array.filter diff4s_to_review ~f:(fun d ->
              ...
              then (
                ...
                then
                  print_endline
                    "Nothing to review in the specified files in the current session";
                None)
...
let catch_up_review_command =
  Command.async'
    ...
    and review_params = Review_params.review_params in
    fun () ->
      let open! Deferred.Let_syntax in
      if am_functional_testing
      then
        failwith
          "This command is deprecated.  Tests should be updated to use [fe review] \
           instead";
      let feature_path = ok_exn feature_path in
      let review_params = review_params () in
      let%bind repo_root, repo_root_kind =
        Cmd_workspace.repo_for_hg_operations_and_kind_exn
          feature_path
          ~use:`Share (* If the feature is archived, this defaults to the clone *)
      in
      catch_up_review_loop
        ~warn_if_no_session:true
        ~repo_root
        ~repo_root_kind
        ~feature_path
        review_params)
...
let command =
  ...
  Command.async'
    ...
    ~readme:(fun () ->
      concat
        [ "[fe review] can be used to see lines to review in a feature and mark them as \
           reviewed.\n\n\
           When there are both [review] and [catch-up] lines to review, the latter are \
           prioritized\n\
        ...
        ; "\n\n\
           Attending someone else's review may be done if authorized, and requires the \
           reviewer\n\
           to supply a brief reason.  Example:\n\n\
          \  $ fe review -for USER1 -reason 'you are out sick today'\n\n\
           By default, reviewing for someone else skips the catch-up review.  Attending \
           someone\n\
    ...
    and only_catch_up_review =
      no_arg_flag only_catch_up_review_switch ~doc:" only attend the catch-up review"
    ...
    fun () ->
      ...
      let () =
        ...
        then
          failwithf
            "At most one of the switches %s and %s may be supplied."
            only_catch_up_review_switch
      ...
      let%bind () =
        ...
        then return ()
          ...
          catch_up_review_loop
            ~warn_if_no_session:only_catch_up_review
            ~repo_root
            ~repo_root_kind
            ~feature_path
            review_params
