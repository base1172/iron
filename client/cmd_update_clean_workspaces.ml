...
let command =
  Command.async'
    ...
    ~readme:(fun () ->
      concat
        [ "A workspace is considered clean if it has no uncommitted changes, not \
           outgoing changes,\n\
           and no error occurred while trying to get this very information.\n\n\
           This command walks all of one's clean workspaces, and update their hg share \
           to the tip\n\
           of the feature's remote bookmark.  It also updates the satellites of scaffold \
           repos when\n\
           applicable.\n\n\
        ...
        ; " is supplied, the command will\n\
          ...
          \  ))\n\n\
           ...
           It is also possible to define a custom exception list of workspaces that \
           should never\n\
    ...
    fun () ->
      ...
      if do_nothing_if_not_enabled
         && not
              (Client_config.Workspaces.auto_update_clean_workspaces_is_enabled
                 client_config)
      then
        Async_interactive.printf
          "Auto update of clean workspaces is not enabled, and the switch %s was supplied.\n\
           Exiting with code 0\n"
          Switch.do_nothing_if_not_enabled
      else (
        ...
        in
        Cmd_workspace.run_concurrent_actions_exn
