...
module Project_root : sig
  ...
  let find repo_root =
    let rec loop abspath arcs =
      ...
      | true -> return (Ok (of_abspath abspath, Path_in_project.of_list (List.rev arcs)))
      | false ->
  ...
  let create project_root ~dirs ~additional_dependencies libmap =
    ...
    let rec compute_dir_dependencies path_in_project =
      ...
      then return ()
              ...
              let%bind depends_on =
                ...
                then return []
                  ...
                  Reader.load_sexps_exn
                    (Abspath.to_string libdeps_file)
                    [%of_sexp: Libname.t])
              in
              let depends_on =
                List.filter_map depends_on ~f:(fun libname ->
                  ...
                  | Error _ ->
                    ...
                    None)
                @ Option.value
                    (Hashtbl.find additional_dependencies path_in_project)
                    ~default:[]
              in
              let%bind () =
        ...
        return ())
    in
    let%bind () = Deferred.List.iter ~how:`Parallel dirs ~f:compute_dir_dependencies in
...
let dirs_in_build_order project_root ~dirs ~additional_dependencies libmap =
  ...
  if verbose
  then
    Debug.ams [%here] "dir_dependencies" dir_dependencies [%sexp_of: Dir_dependencies.t];
          ...
          | Some file ->
            (match%map
               try_with (fun () ->
                 Reader.load_sexps_exn
                   (Abspath.to_string
                      (Abspath.extend dir (File_name.of_string dot_d_file)))
                   [%of_sexp: string])
             with
             | Error _ -> None
             | Ok depends_on ->
...
module Representative : sig
  ...
  let%test_unit _ =
    List.iter [ "a.ml"; "a.mli" ] ~f:(fun string ->
      [%test_result: t]
        ~expect:(of_string "a.ml")
...
let files_in_build_order ~dir files =
  ...
  let edges =
    List.concat_map
      (Hashtbl.to_alist dependencies)
      ~f:(fun (representative, depends_on) ->
      List.map (Set.to_list depends_on) ~f:(fun depends_on ->
        { Edge.from = depends_on; to_ = representative }))
  in
  ...
  let files_by_representative =
    ...
    |> Hashtbl.map ~f:(fun files ->
         List.sort files ~compare:(fun file1 file2 ->
           pos_in_class file1 - pos_in_class file2))
  in
    ...
    | Some prefix ->
      ...
      (match
       ...
       | Some files1, Some files2 ->
         ...
         Hashtbl.set
           files_by_representative
           ~key:desired_representative
           ~data:(files1 @ files2)));
  let files =
...
let compute
  ...
  | Ok libmap ->
    ...
    (match%bind
       dirs_in_build_order
         project_root
         ~dirs:(Hashtbl.keys files_by_dir)
         ~additional_dependencies:additional_dir_dependencies
         libmap
     with
         ...
         Deferred.List.map dirs_in_build_order ~how:`Parallel ~f:(fun dir ->
           ...
           | Some files ->
             ...
             List.map files ~f:(fun file ->
               ...
               if (not (Hashtbl.mem file_dependencies file)) && expect_dependencies file
...
let staged_sort repo_root list (path_in_repo : _ -> Path_in_repo.t) =
  ...
  | Ok (project_root, repo_root_path_in_project) ->
    let to_path_in_project path_in_repo =
      Path_in_project.append
        repo_root_path_in_project
    ...
    (match%map
     ...
     | Build_order (build_order, missing_dependencies) ->
       ...
       Build_order
         (fun list ->
           let sorted =
             List.sort list ~compare:(fun d1 d2 ->
...
let sort_raised exn =
  eprintf
    "%s"
    (concat
       [ "Warning: files are in alphabetical order rather than build order due to an \
          exception:\n\n"
       ; exn |> [%sexp_of: exn] |> Sexp.to_string_hum
       ; "\n\
          ------------------------------------------------------------------------------------------\n"
       ])
;;
...
let staged_sort repo_root_and_kind_or_error list path_in_repo =
  ...
  then
    ...
    let default_sort list =
      List.sort list ~compare:(fun a1 a2 ->
    ...
    let repo_root_option =
      ...
      | Ok (repo_root, repo_root_kind) ->
        let repo_may_have_applicable_build_artifacts =
          ...
          | Workspace -> true
        in
        if repo_may_have_applicable_build_artifacts then Some repo_root else None
    in
    match repo_root_option with
    | None ->
      return (fun list ->
        eprintf
          "Warning: files are in alphabetical order because don't have a repo with build \
           artifacts.\n\n";
        default_sort list)
    | Some repo_root ->
      (match%map
       ...
       | Ok (No_build_order error) ->
         fun list ->
           eprintf
             "Warning: files are in alphabetical order because there are no build \
              artifacts.\n\n";
         ...
         fun list ->
           (match sort list with
            ...
            | list, missing_dependencies ->
              ...
              then (
                eprintf
                  "Warning: files may not be in build order because of missing build \
                   artifacts.\n\n";
                if verbose
                then
                  Debug.ams
