...
let main (action : Fe.Update.Action.t) =
  ...
  | Satellites_only ->
    ...
    then
      failwithf
        "[%s] can be used only if workspaces are enabled"
  ...
  Command.async'
    ...
    and satellites_only =
      no_arg_flag
        satellites_only_switch
        ~doc:
          " update only satellites repos in the scaffold repo (require workspaces \
           enabled, and cwd = center repo)"
    in
    fun () ->
      ...
      then (
        ...
        then
          failwithf
            "when [%s] is provided you can't pass a feature argument"
            satellites_only_switch
            ();
