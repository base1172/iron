...
let apply_updates_exn
  ...
  then
    raise_s
      [%sexp
        (sprintf "must request at least one change" : string)
        , (feature_path : Feature_path.t)];
  let%map result = rpc_to_server_exn ~feature_path ~updates in
  let print result =
    result |> [%sexp_of: update * unit Or_error.t] |> Sexp.to_string_hum |> print_endline
  in
  ...
  then raise_s [%sexp "there were problems", (errors : (update * unit Or_error.t) list)]
;;
...
let command =
  Command.async'
    ...
    and add_owners = user_list_option ~switch:"-add-owners"
    and add_reviewing = users_option ~switch:"-add-reviewing"
    and add_whole_feature_followers = users_option ~switch:"-add-whole-feature-followers"
    and add_whole_feature_reviewers =
      users_option ~switch:Switch.add_whole_feature_reviewers
    and lock = no_arg_flag "-lock" ~doc:"error -- use [fe lock] or [fe unlock]"
    and remove_owners = users_option ~switch:"-remove-owners"
    and remove_properties =
      properties_set_option ~switch:"-remove-property" ~verb:"remove"
    and remove_reviewing = users_option ~switch:"-remove-reviewing"
    ...
    and set_crs_are_enabled = flag "-set-crs-are-enabled" (optional bool) ~doc:"BOOL "
    ...
    and set_owners = user_list_option ~switch:"-set-owners"
    and set_properties =
      property_values_flag ~switch:"-set-property" ~doc:"set user-defined properties"
    and set_reviewing = users_option ~switch:"-set-reviewing"
    and set_reviewing_all = no_arg_flag "-set-reviewing-all" ~doc:""
    and set_reviewing_none = no_arg_flag "-set-reviewing-none" ~doc:""
    and set_reviewing_whole_feature_only =
      no_arg_flag Switch.set_reviewing_whole_feature_only ~doc:""
    and set_whole_feature_followers = users_option ~switch:"-set-whole-feature-followers"
    and set_whole_feature_reviewers = users_option ~switch:"-set-whole-feature-reviewers"
    ...
    and set_release_process =
      enum_optional
        "-set-release-process"
        ~doc:"PROCESS how features are released into this one"
        (module Release_process)
    and set_who_can_release_into_me =
      enum_optional
        "-set-who-can-release-into-me"
        ~doc:"WHO owner who can release"
        (module Who_can_release_into_me)
    ...
    and remove_inheritable_crs_shown_in_todo_only_for_users_reviewing =
      no_arg_flag "-remove-inheritable-crs-shown-in-todo-only-for-users-reviewing" ~doc:""
    ...
    and set_inheritable_owners = user_list_option ~switch:"set-inheritable-owners"
    and add_inheritable_owners = user_list_option ~switch:"add-inheritable-owners"
    and remove_inheritable_owners = users_option ~switch:"remove-inheritable-owners"
    and set_inheritable_properties =
      property_values_flag ~switch:"set-inheritable-property" ~doc:""
    and remove_inheritable_properties =
      inheritable_properties_set_option ~switch:"remove-inheritable-property"
    ...
    and set_lines_required_to_separate_ddiff_hunks =
      flag "-set-lines-required-to-separate-ddiff-hunks" (optional int) ~doc:""
    ...
    fun () ->
      ...
      let updates =
        List.filter_opt
          [ Option.map add_owners ~f:(fun users -> `Add_owners users)
          ; Option.map add_reviewing ~f:(fun users -> `Add_reviewing users)
          ; Option.map add_whole_feature_followers ~f:(fun users ->
              `Add_whole_feature_followers users)
          ; Option.map add_whole_feature_reviewers ~f:(fun users ->
              `Add_whole_feature_reviewers users)
          ; Option.map remove_owners ~f:(fun users -> `Remove_owners users)
          ; Option.map remove_properties ~f:(fun attrs -> `Remove_properties attrs)
          ; Option.map remove_reviewing ~f:(fun users -> `Remove_reviewing users)
          ; Option.map remove_whole_feature_followers ~f:(fun users ->
              `Remove_whole_feature_followers users)
          ; Option.map remove_whole_feature_reviewers ~f:(fun users ->
              `Remove_whole_feature_reviewers users)
          ; Option.map set_crs_are_enabled ~f:(fun bool -> `Set_crs_are_enabled bool)
          ; Option.map set_crs_shown_in_todo_only_for_users_reviewing ~f:(fun bool ->
              `Set_crs_shown_in_todo_only_for_users_reviewing bool)
          ; Option.map set_xcrs_shown_in_todo_only_for_users_reviewing ~f:(fun bool ->
              `Set_xcrs_shown_in_todo_only_for_users_reviewing bool)
          ; Option.map set_owners ~f:(fun users -> `Set_owners users)
          ; Option.bind set_properties ~f:(fun properties ->
              if Map.is_empty properties then None else Some (`Set_properties properties))
          ; Option.map set_reviewing ~f:(fun users ->
              let reviewing =
          ...
          ; (if set_reviewing_all then Some (`Set_reviewing `All) else None)
          ...
          ; Option.map set_whole_feature_followers ~f:(fun users ->
              `Set_whole_feature_followers users)
          ; Option.map set_whole_feature_reviewers ~f:(fun users ->
          ...
          ; Option.map set_release_process ~f:(fun how -> `Set_release_process how)
          ; Option.map set_who_can_release_into_me ~f:(fun who ->
              `Set_who_can_release_into_me who)
          ; Option.map set_send_email_to ~f:(fun whom -> `Set_send_email_to whom)
          ; Option.map add_send_email_to ~f:(fun whom -> `Add_send_email_to whom)
          ; Option.map remove_send_email_to ~f:(fun whom -> `Remove_send_email_to whom)
          ; Option.map set_send_email_upon ~f:(fun send_email_upon ->
              `Set_send_email_upon send_email_upon)
          ; Option.map add_send_email_upon ~f:(fun send_email_upon ->
              `Add_send_email_upon send_email_upon)
          ; Option.map remove_send_email_upon ~f:(fun send_email_upon ->
              `Remove_send_email_upon send_email_upon)
          ; Option.map
          ...
          ; Option.map set_inheritable_owners ~f:(fun owners ->
              `Set_inheritable_owners owners)
          ; Option.map add_inheritable_owners ~f:(fun owners ->
              `Add_inheritable_owners owners)
          ; Option.map remove_inheritable_owners ~f:(fun owners ->
              `Remove_inheritable_owners owners)
          ; Option.map set_inheritable_properties ~f:(fun attrs ->
              `Set_inheritable_properties attrs)
          ; Option.map remove_inheritable_properties ~f:(fun attrs ->
              `Remove_inheritable_properties attrs)
          ; Option.map set_inheritable_release_process ~f:(fun how ->
              `Set_inheritable_release_process (Option.return how))
          ; (if remove_inheritable_release_process
            then Some (`Set_inheritable_release_process None)
            else None)
          ; Option.map set_inheritable_who_can_release_into_me ~f:(fun whom ->
          ...
          ; Option.map set_inheritable_send_email_to ~f:(fun send_email_to ->
              `Set_inheritable_send_email_to send_email_to)
          ; Option.map add_inheritable_send_email_to ~f:(fun send_email_to ->
              `Add_inheritable_send_email_to send_email_to)
          ; Option.map remove_inheritable_send_email_to ~f:(fun send_email_to ->
              `Remove_inheritable_send_email_to send_email_to)
          ; Option.map set_inheritable_send_email_upon ~f:(fun send_email_upon ->
              `Set_inheritable_send_email_upon send_email_upon)
          ; Option.map add_inheritable_send_email_upon ~f:(fun send_email_upon ->
              `Add_inheritable_send_email_upon send_email_upon)
          ; Option.map remove_inheritable_send_email_upon ~f:(fun send_email_upon ->
          ...
          ; Option.map
              add_inheritable_whole_feature_followers
              ~f:(fun whole_feature_followers ->
              `Add_inheritable_whole_feature_followers whole_feature_followers)
          ; Option.map
              remove_inheritable_whole_feature_followers
              ~f:(fun whole_feature_followers ->
          ...
          ; Option.map
              add_inheritable_whole_feature_reviewers
              ~f:(fun whole_feature_reviewers ->
              `Add_inheritable_whole_feature_reviewers whole_feature_reviewers)
          ; Option.map
              remove_inheritable_whole_feature_reviewers
              ~f:(fun whole_feature_reviewers ->
              `Remove_inheritable_whole_feature_reviewers whole_feature_reviewers)
          ; Option.map set_lines_required_to_separate_ddiff_hunks ~f:(fun lines ->
              `Set_lines_required_to_separate_ddiff_hunks lines)
          ]
      in
