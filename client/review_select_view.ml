...
let read_index s = Option.try_with (fun () -> pred (Int.of_string s))
...
let parse_input input =
  ...
  let indexes =
    ...
    then
      ...
      |> function
      | [] -> None
      | _ :: _ as indexes -> Some (Int.Hash_set.of_list indexes)
    else None
  in
  match indexes with
  ...
  | None ->
    ...
    in
    String.chop_prefix ~prefix:"re:" input |> Option.value ~default:input |> regex
;;
...
let%test_unit _ =
  let check is_match (index, file) input =
    let result =
      ...
      | Error str -> raise_s [%sexp "parse error", { input : string; str : string }]
      | Ok res ->
        (match res with
         | `Toggle_indexes hset -> Hash_set.mem hset (pred index)
         | `Toggle_regex regex -> Regex.matches regex file)
    in
    if not (Bool.equal is_match result)
    then raise_s [%sexp "invalid result", { index : int; file : string; input : string }]
  in
    ...
    Some
      (let by_name a b = String.compare (to_string a) (to_string b) in
       let current =
         ...
         | `Pred_view -> List.min_elt configuration ~compare:by_name
         | `Succ_view -> List.max_elt configuration ~compare:by_name
       in
       let index =
...
let toggle ~menu_name ~to_string ~display_prefix_in_list ~available ~configuration =
  let to_string =
    ...
    | Some prefix ->
      let max_prefix_size =
        List.fold available ~init:0 ~f:(fun acc elt ->
          max acc (String.length (prefix elt)))
      in
      fun elt ->
        sprintf
          "%*s%s%s"
          max_prefix_size
          (prefix elt)
          (if max_prefix_size > 0 then " " else "")
          (to_string elt)
  in
  let available = Array.of_list available in
  ...
  let rec loop ?(help = false) ?message () =
    printf "%s:\n" menu_name;
    let max_size = menu |> Array.length |> Int.to_string_hum |> String.length in
    ...
    | `Show_again ->
      let selection = List.map (Queue.to_list selection) ~f:(fun i -> available.(i)) in
      if List.is_empty selection
      then loop ~message:"current selection is empty" ()
      else return (`New_configuration selection)
    | `All ->
      let present = Int.Hash_set.of_list (Queue.to_list selection) in
    ...
    | (`Toggle_indexes _ | `Toggle_regex _) as toggle ->
      let toggle =
        ...
        | `Toggle_regex regex -> fun index -> Regex.matches regex (fst menu.(index))
      in
      let present = Int.Hash_set.of_list (Queue.to_list selection) in
