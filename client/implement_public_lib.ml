...
let implement (type action reaction) m f =
  ...
  let module T_conv = struct
    ...
    let implementation _invocation ~version query =
      ...
      | Error exn ->
        raise_s
          [%sexp
            ...
            , (M.name, version, Monitor.extract_exn exn : string * int * exn)]
    ;;
  end
  in
  rpc_implementations
...
let () =
  implement
    (module Fe.Supported_command_rpcs)
    (fun () ->
      List.concat_map !rpc_implementations ~f:(fun (module T) ->
        let name = T.name in
        List.map
          (Set.to_list (T.versions ()))
          ~f:(fun version -> Iron_command_rpc.find_rpc_exn ~name ~version))
      |> return)
;;
