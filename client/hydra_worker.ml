    ...
    then (
      ...
      | { start; _ } :: next ->
        let rec widest_column acc l =
          List.fold_left l ~init:acc ~f:(fun acc { name; children; _ } ->
          ...
          List.iter l ~f:(fun { name; start; end_; children } ->
            let diff_from src =
              ...
              sprintf
                ...
                (Time.Span.(to_sec span /. to_sec total) *. 100.)
            in
            printf
              "%-*s : %s%s\n"
              width
...
let dont_fail_if_server_is_down f =
  ...
  | Ok () -> Deferred.unit
  | Error e ->
    (match%map Deferred.Or_error.try_with (fun () -> Ping.rpc_to_server_exn ()) with
     | Ok () ->
...
let use_or_compute_and_store worker_cache_session key rev ~fake_attribute compute =
  ...
  | `Do_not_fake ->
    Worker_cache.Worker_session.use_or_compute_and_store
      ...
      key
      rev
      (compute ~fake_attribute:`Do_not_fake)
;;
...
let compute_worker_rev_facts_uncached
  ...
  Event.nest "compute_worker_rev_facts" (fun () ->
    let cached_facts_for_incremental_computation =
      ...
      | Ok due_now, Ok due_soon ->
        let%bind { Worker_obligations.obligations_version; _ } =
          Worker_cache.Worker_session.find
            worker_cache_session
            base_rev
            Worker_obligations
        in
    ...
    let%bind { Worker_obligations.obligations_are_valid
             ; obligations
             ; obligations_version
             }
      =
  ...
  let%bind tag =
    Process.run
      ...
      ~args:
        [ "-c"
        ; sprintf
            !"echo %{sh} | grep ^%{sh}- | sort -g | tail -n 1"
            (ok_exn all_tags)
            tag_root
        ]
      ()
...
let infer_base_command =
  Command.async'
    ...
    let%map_open () = return ()
    and rev = rev
    and remote_repo_path = anon (maybe ("remote_repo_path" %: string)) in
    fun () ->
...
let all_obligations
  ...
  | Ok obligations_at_tip, Ok obligations_at_base, true ->
    ...
    let obligations_by_rev =
      Map.add obligations_by_rev ~key:base ~data:obligations_at_base
    in
    let all_revs =
          ...
          | Ok
            ...
            let%bind info =
              Deferred.Or_error.try_with (fun () ->
                let need_diff4s_starting_from =
                  List.map need_diff4s_starting_from ~f:fst
                in
                run_without_server
                  ~repo_root
                  ~feature_path
                  ...
                  ~worker_cache_session)
            in
            let info = if testing_incremental_rev_facts then Ok (ok_exn info) else info in
...
let command =
  Command.async'
    ...
    and fake_attribute =
      no_arg_flag
        "-fake-valid-obligations"
        ~doc:"pretend to call the obligations lib, for testing"
    and attributes_by_rev =
      flag
        "-fake-attribute-by-rev"
        (optional string)
        ~doc:
          "map eg. ((rev1 attribute) (rev2 attributes)), for use with \
           -fake-valid-obligations"
    and run_between_rpcs =
      flag
        ...
        (optional string)
        ~doc:
          "script a string to be interpreted by bash to run between the two rpcs, to \
    ...
    fun () ->
      ...
      let fake_attribute =
        ...
        | true, Some fake_attribute ->
          `Fake_by_rev
            (Fake_attribute_by_rev.t_of_sexp
...
let serverless_command =
  Command.async'
    ...
    and unparsed_review_managers = flag "review-manager" (listed string) ~doc:"base,tip"
    ...
    fun () ->
      ...
      in
      printf
        !"Assuming base = %s, tip = %s, bookmark = %s\n"
        (Rev.to_string_40 base)
      ...
      let%bind review_managers =
        Deferred.List.map unparsed_review_managers ~f:(fun unparsed ->
          ...
          in
          { Review_edge.base = ok_exn base; tip = ok_exn tip })
      in
      ...
      in
      print_endline (result |> [%sexp_of: Update_bookmark.Info.t] |> Sexp.to_string_hum);
      Deferred.unit)
;;
