...
let command =
  Command.async'
    ...
    ~readme:(fun () ->
      concat
        [ "[fe is-unlocked] exits status zero if the supplied locks are unlocked. \
           Otherwise, it\n\
           exits nonzero and prints a message on stderr.  You must supply at least one \
           lock, or\n\
           use ["
        ; Switch.all_locks
    ...
    and check_all = no_arg_flag Switch.all_locks ~doc:"check all locks" in
    fun () ->
      ...
      then raise_s [%sexp "locked", (locked : (Lock_name.t * Feature.Locked.t list) list)])
;;
