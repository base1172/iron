...
let main
  ...
  let%bind () =
    ...
    | Some new_base ->
      ...
      in
      let%bind () =
        Hg.pull ~repo_is_clean repo_root ~from:parent.remote_repo_path (`Rev parent.tip)
      in
      ...
      then
        failwithf
          !"new base (%{Rev#12}) must be an ancestor of the parent feature's tip \
            (%{Rev#12})"
          new_base
          parent.tip
          ();
      ...
      then
        failwithf
          !"new base (%{Rev#12}) must descend from the feature's current base (%{Rev#12})"
          new_base
          feature.base
          ()
  in
  let%bind { Prepare_to_rebase.Reaction.old_tip
           ; old_base
           ; new_base
           ; remote_repo_path
           ; feature_id
  ...
  in
  let%bind local_feature = Hg.create_rev repo_root (Revset.feature_tip feature_path) in
  let local_feature = ok_exn local_feature in
  if not (Rev.equal_node_hash old_tip local_feature)
  then
    raise_s
      [%sexp
        "local bookmark is not the same as Iron server's"
        , { local = (local_feature : Rev.t); iron_server = (old_tip : Rev.t) }];
  let%bind () = Hg.pull ~repo_is_clean repo_root ~from:remote_repo_path (`Rev new_base) in
  ...
  let%bind () =
    ...
    then (
      ...
      let%bind () =
        Hg.set_bookmark repo_root (Feature feature_path) ~to_:(`Rev new_base) `Do_not_push
      in
  ...
  let%bind () =
    let%map result =
      Hg.push
        ...
        ~to_:remote_repo_path
        ~overwrite_bookmark:false
    in
    ok_exn result
  in
  let%bind () =
    match%map try_with (fun () -> Cmd_hg_hooks.maybe_send_push_event ~repo_root) with
    | Ok () | Error (_ : Exn.t) -> ()
  in
  let%bind () =
    ...
    then Deferred.unit
      ...
      | None -> Deferred.unit
      | Some cmd ->
        let%bind process = Process.create ~prog:"/bin/bash" ~args:[ "-c"; cmd ] () in
  ...
  Command.async'
    ...
    and abort_on_merge_conflicts =
      no_arg_flag
        ...
        ~doc:
          " abort the rebase and fail if a merge is performed and creates conflicts \
           markers"
    and () = interactive in
    fun () ->
