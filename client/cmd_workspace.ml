...
module If_enabled = struct
  let if_enabled f = if not (workspaces_are_enabled ()) then Deferred.unit else f ()
    ...
    if_enabled (fun () ->
      ...
      let renames =
        List.sort renames ~compare:(fun (r1 : Rename.t) r2 ->
          Int.compare (Feature_path.num_parts r2.from) (Feature_path.num_parts r1.from))
      in
      Deferred.List.iter
        ~how:`Sequential
        renames
  ...
  let update_satellite_repos ~center_repo_root =
    if_enabled (fun () ->
      ...
      | None -> Deferred.unit
      | Some scaffold -> Scaffold.update_satellite_repos scaffold ~center_repo_root)
  ;;
end
...
let dir_command =
  Command.async'
    ~summary:"output a feature's workspace directory if it exists or fail otherwise"
    (let open Command.Let_syntax in
    let%map_open () = return ()
    and what_to_print =
      let%map feature_path_option = feature_path_option
      and basedir = no_arg_flag "-basedir" ~doc:" print the user's workspaces basedir" in
    ...
    let check_repo_family_exn =
        ...
        then return ()
          ...
          | Error e2 ->
            raise_s
              [%sexp
                (sprintf
                   !"your working directory must be inside a clone of the \
                     [%{Feature_name}] family,\n\
                     and fe cannot determine the repo family it's in"
                   feature_root
                  : string)
                , (e1 : Error.t)
                , (e2 : Error.t)])
      | Ok repo_root_feature ->
        ...
        then return ()
          ...
          failwithf
            !"your working directory must be inside a clone of the [%{Feature_name}] \
              family,\n\
...
module Select_features_in_my_todo = struct
  ...
  let select
    ...
    then
      ...
      List.filter_map
        ~f:(fun (cons, bool) -> Option.some_if bool cons)
  ...
  | _ :: _ as select ->
    let%map todo =
      Todo.rpc_to_server_exn
        { for_; include_active_cr_soons = false; include_all_owned_features = true }
    in
...
let run_concurrent_actions_exn ~get_feature_path ~action ~max_concurrent_jobs list ~f =
  let%map failed_features =
    Deferred.List.map ~how:(`Max_concurrent_jobs max_concurrent_jobs) list ~f:(fun elt ->
      ...
      | Error exn ->
        ...
        prerr_endline
          ((feature_path, exn) |> [%sexp_of: Feature_path.t * Exn.t] |> Sexp.to_string_hum);
        Some feature_path)
  in
  ...
  then
    raise_s
      [%sexp
        (sprintf
           "failed to %s workspace%s"
           action
           (if List.length failed_features > 1 then "s" else "")
          : string)
...
let create_command =
  Command.async'
    ...
    ~readme:(fun () ->
      concat
        [ "Create specified workspace(s).  If ["
        ; features_in_my_todo_switch
        ; "] is supplied,\n\
    ...
    and desired_num_spares =
      flag "num-spares" (optional int) ~doc:"INT target number of spare shares"
    and for_ = for_
    ...
    and which_features_in_my_todo =
      let%map features_in_my_todo =
        no_arg_flag
          features_in_my_todo_switch
          ~doc:"create a workspace for each feature assigned or owned in the user's todo"
      ...
      Select_features_in_my_todo.select
        ...
        ~features_owned_by_me
        ~features_watched_by_me
    and feature_path_option = feature_path_option in
    fun () ->
      let open! Deferred.Let_syntax in
      let%bind () =
        ...
        then Deferred.unit
          ...
          Deferred.List.iter
            ...
            | Some desired_num_spares ->
              Repo_clone.refresh_spare_shares
                clone
                ~desired_num_spares
                ~update_to:(Repo_clone.tip_on_server clone)))
      in
      let%bind features = get_features_in_my_todo ~for_ ~which_features_in_my_todo in
      let features =
...
let confirm_or_dry_run shares ~dry_run ~action ~process_share =
  ...
  let%bind should_proceed =
    ...
    else if dry_run || not !Async_interactive.interactive
  ...
  then Deferred.unit
    ...
    run_concurrent_actions_exn
      shares
      ~f:process_share
      ~get_feature_path:Workspace.feature_path
      ~action
...
let exclude =
  ...
  feature_path_flagged_listed
    ~label:exclude_switch
    ~doc:"FEATURE leave this feature alone (this flag may be passed serveral times)"
;;
...
let exclude_features_in_my_todo_param =
  ...
  and features_in_my_todo =
    no_arg_flag
      (concat [ "-exclude"; features_in_my_todo_switch ])
      ~doc:(sprintf "exclude features assigned or owned in the user's todo")
  and features_assigned_to_me =
    no_arg_flag
      (concat [ "-exclude"; features_assigned_to_me_switch ])
      ~doc:"exclude features assigned in the user's todo"
  and features_owned_by_me =
    no_arg_flag
      (concat [ "-exclude"; features_owned_by_me_switch ])
      ~doc:"exclude features owned in the user's todo"
  and features_watched_by_me =
    no_arg_flag
      (concat [ "-exclude"; features_watched_by_me_switch ])
...
let select_shares ~which_features ~exclusions ~for_ ~exclude_features_in_my_todo =
  ...
  let%bind exclude_features_in_my_todo =
    get_features_in_my_todo ~for_ ~which_features_in_my_todo:exclude_features_in_my_todo
  in
  let exclusions =
    Feature_path.Set.union_list
  ...
  List.filter shares ~f:(fun share ->
    ...
    (not (Set.mem exclusions share_path)) && Which_features.mem which_features share_path)
;;
...
let distclean_command =
  Command.async'
    ...
    ~readme:(fun () ->
      concat
        [ "Iter through the intersection of your local workspaces and the feature \
           selected via the\n\
           command line and in each repo root run:\n\n\
          \  $ hg distclean\n\n\
           The intended usage of this command is to be run in the background to keep the \
           local disk\n\
           use reasonable, or to be run manually from time to time on selected \
           workspaces.\n\n\
           To prevent some workspaces from being cleaned, add a [do_not_distclean] \
           stanza to\n\
           your .ferc, e.g:\n\n\
          \  (workspaces (\n\
          \    (do_not_distclean (\n\
          ...
          \  ))\n\n\
           Like ["
        ; exclude_switch
        ; "], this will take priority over the features selected via the\n\
           command line, and result in skipping those features.\n\n\
           In doubt, run the command with ["
        ; dry_run_switch
    ...
    and exclusions = exclude
    and dry_run = dry_run ~done_:"distcleaned"
    and for_ = for_
    and exclude_features_in_my_todo = exclude_features_in_my_todo_param in
    fun () ->
      let open! Deferred.Let_syntax in
      let%bind shares =
...
let delete_command =
  ...
  Command.async'
    ~summary:"delete selected workspaces"
    ~readme:(fun () ->
      concat
        [ "This command deletes one's selected workspace directories on the local \
           machine.  It does\n\
           not archive the associated feature(s), and leave the remote repository \
           untouched.\n\n\
           Before deleting the directories, the command will check the cleanliness of \
           the workspace.\n\
           Refer to [fe workspace unclean check -help] to read more about the \
           cleanliness status.\n\
           Essentially, the command will not attempt to delete a workspace that has \
           uncommitted or\n\
    ...
    fun () ->
      ...
      (match which_features with
       | Features [] when not workspaces_without_feature ->
         failwithf
           "Specify some features, or use [%s]."
           workspaces_without_feature_switch
           ()
       | _ -> ());
      let%bind exclude_features_in_my_todo =
        get_features_in_my_todo
          ~for_
          ~which_features_in_my_todo:exclude_features_in_my_todo
      in
      let excluded =
        Feature_path.Set.union_list
          [ Feature_path.Set.of_list (ok_exn exclude); exclude_features_in_my_todo ]
      in
      let%bind shares =
        let%bind shares = Workspace.list () in
        let%map existing_features =
          List_feature_names.rpc_to_server_exn
            { descendants_of = Any_root; depth = Int.max_value; use_archived = false }
        in
        let existing_features = Feature_path.Set.of_list existing_features in
        List.filter shares ~f:(fun share ->
          let share_path = Workspace.feature_path share in
          (not (Set.mem excluded share_path))
          && (Which_features.mem which_features share_path
             || (workspaces_without_feature && not (Set.mem existing_features share_path))
             ))
      in
      confirm_or_dry_run shares ~dry_run ~action:"delete" ~process_share:Workspace.delete)
;;
...
let exists_command =
  Command.async'
    ...
    ~readme:(fun () ->
      concat
        [ "Check on the localhost that the user has a workspace for a specified feature \
           or clone.\n"
        ])
    (let open Command.Let_syntax in
    let%map_open () = return ()
    ...
    and clone_or_feature =
      let%map feature = anon (maybe ("FEATURE" %: unverified_workspace_arg_type))
      and clone_of_root_feature_of =
        flag
          Switch.clone_of_root_feature_of
          (optional unverified_workspace_arg_type)
          ~doc:
            "FEATURE check for the clone associated with the specified feature's root \
             instead of checking for the feature share"
      in
      Or_error.try_with (fun () ->
        ...
        | Some _, Some _ ->
          failwith "one should supply either a feature or a repo clone but not both")
    in
    fun () ->
      let open! Deferred.Let_syntax in
      let%map exists =
    ...
    ~readme:(fun () ->
      concat
        [ "Attempt to kill the build process running in the selected workspace(s).\n\n\
           The command tries first to send the kill request to the build manager running \
           in emacs (if\n\
           applicable) and then defaults to killing the jenga process directly.  The \
           intent is for\n\
           this command to work for non emacs users, while offering a cleaner kill for \
           emacs users.\n"
        ])
    (let open Command.Let_syntax in
    let%map_open () = return ()
...
let pwd_command =
  Command.async'
    ~summary:
      "output the current feature path if you are currently in a workspace directory, or \
       fail otherwise"
    (let open Command.Let_syntax in
    let%map_open () = return () in
    fun () ->
