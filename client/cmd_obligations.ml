...
let create_obligations ?skip_full_repo_checks ?(resolve_aliases = false) repo_root ~dirs =
...
let check =
  Command.async'
    ...
    fun () ->
      ...
      let%map (_ : Obligations.t) = create_obligations repo_root ~dirs:(`Below below) in
      ())
;;
...
let load_obligations
  { Fe.Obligations.Which_obligations.repo_root; file_tree_of; aliases_resolution }
  =
  ...
  let load repo_root =
    let%map obligations, _ =
      Obligations_repo.load (module Hg) repo_root ~aliases >>| ok_exn
    in
    obligations
  in
...
let which_obligations_param =
  ...
  and aliases_resolution =
    ...
    |> map ~f:(fun de_alias ->
         if de_alias then `Using_latest_aliases_from_iron_server else `None)
  in
  { Fe.Obligations.Which_obligations.repo_root =
      Repo_root.program_started_in |> ok_exn |> Repo_root.to_abspath
  ; file_tree_of
  ; aliases_resolution
  }
;;
...
let list_users_in_groups =
  let group_blang_arg =
    Command.Param.blang_arg_type_with_completion
      (module Group_name)
      (fun () ->
        match
        ...
        | Error exn ->
          Or_error.error "cannot load obligations_repo.sexp" exn [%sexp_of: exn])
  in
  Command.async'
    ...
    and group_blang = anon ("<GROUP-BLANG>" %: group_blang_arg) in
    fun () ->
      let open! Deferred.Let_syntax in
      let%map groups = List_groups.main which_obligations in
...
let allow_review_for =
  Command.async'
    ...
    fun () ->
      ...
      let%map obligations = load_obligations which_obligations in
      print_endline
        (Sexp.to_string_hum [%sexp (obligations.allow_review_for : Allow_review_for.t)]))
;;
...
let projection =
  Command.async'
    ...
    (let open Command.Let_syntax in
    let%map_open () = return ()
    and projections = projections in
    fun () ->
      let open! Deferred.Let_syntax in
      let repo_root = ok_exn Repo_root.program_started_in in
...
let show =
  Command.async'
    ...
    and display_in_table =
      no_arg_flag "-display-in-table" ~doc:" display output as a table rather than a sexp"
    in
    fun () ->
      let open! Deferred.Let_syntax in
      ...
      in
      let%bind obligations = create_obligations repo_root ~dirs:(`Only_this dir) in
      ...
      let to_print =
        ...
        else review_attributes |> [%sexp_of: Review_attributes.t] |> Sexp.to_string_hum
      in
      printf "%s\n" to_print;
      return ())
;;
            ...
            ~contents:
              (concat
                 [ ";; This file was generated by:\n\
                    ...
                    ;;\n\
                    ;; If you get an error about [missing low-review files] or [files \
                    are not low review],\n\
                    ;; then you can run the above command to generate valid low-review-* \
                    files in this repo.\n\
                    ;; But in the case of [missing low-review files], you should \
                    probably adjust .fe.sexp\n\
                    ;; files so that the missing files have the desired scrutiny and \
                    enough reviewers.\n"
                 ; low_review
                   |> Set.to_list
...
let command =
  Command.group
    ~summary:"process Iron obligations files"
