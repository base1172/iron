...
module Flags = struct
  ...
  let spec_id () =
    flag "spec-id" (required Arg_types.spec_id) ~doc:"SPEC-ID name of fact spec"
  ;;

  let scope () =
    flag
      "scope"
      (required Arg_types.scope)
...
let cmd_add_fact =
  Command.async'
    ...
    and comment =
      flag "comment" (required string) ~doc:"STRING comment about fact assertion"
    and for_ = for_
    and assertion_time =
      flag "-time" (optional time) ~doc:"TIME override assertion time (defaults to now)"
    in
    fun () ->
...
let cmd_remove_fact =
  Command.async'
    ...
    let%map_open () = return ()
    and spec_id = Flags.spec_id ()
    and scope = Flags.scope () in
    fun () -> Fact_action.rpc_to_server_exn (spec_id, `remove_fact scope))
;;
  ...
  Command.async'
    ...
    and file =
      flag
        ...
        ~doc:"FILE to be parsed by Fact.Spec.t_of_sexp"
    in
    fun () ->
...
let cmd_spec_remove =
  Command.async'
    ...
    (let open Command.Let_syntax in
    let%map_open () = return ()
    and spec_id = Flags.spec_id () in
    fun () -> Fact_action.rpc_to_server_exn (spec_id, `remove_spec))
;;
...
let cmd_list =
  Command.async'
    ...
    fun () ->
      ...
      let l =
        List.sort l ~compare:(fun (sc1, _) (sc2, _) -> Fact.Scope.compare sc1 sc2)
      in
...
let cmd_spec_list =
  Command.async'
    ...
    let%map_open () = return ()
    and machine =
      Flags.machine () ~extra_doc:" with all details, including full authorization rules"
...
let cmd_spec =
  Command.group
    ~summary:"fact spec commands"
    [ "add", cmd_spec_add; "remove", cmd_spec_remove; "list", cmd_spec_list ]
;;
