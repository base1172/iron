...
let enable_command =
  Command.async'
    ...
    fun () ->
      ...
      Enable_review.rpc_to_server_exn { feature_path; add_whole_feature_reviewers })
;;
...
let disable_command =
  Command.async'
    ...
    let%map_open () = return ()
    and feature_path = feature_path_or_current_bookmark
    and and_crs = no_arg_flag "-and-crs" ~doc:"disable CRs as well" in
    fun () ->
      let feature_path = ok_exn feature_path in
      Cmd_change.change_feature
        ~feature_path
        ~updates:
          (if and_crs
