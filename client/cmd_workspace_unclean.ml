...
module Unclean_workspace_row = struct
  ...
  let empty = { feature = empty_column; reason = empty_column }
end
...
module Workspaces_status = struct
  ...
  let compute ?(workspace_not_found_is_an_error = false) features : t Deferred.t =
    ...
    let%map unclean_workspaces =
      Deferred.List.map ~how:(`Max_concurrent_jobs 10) features ~f:(fun feature_path ->
        ...
        match status with
        | Clean ->
          Hash_set.add clean_workspaces feature_path;
          None
  ...
  let compute_all () =
    ...
    shares |> List.map ~f:Workspace.feature_path |> compute
  ;;
end

let list_command =
  Command.async'
    ~summary:"find unclean workspaces on the local machine and show them"
    ~readme:(fun () ->
      concat
        [ "This command is slow because walks all of one's workspaces.  It is not \
           intended to be used\n\
           directly, but rather just occasionally for example for trouble shooting.  \
           There is a set\n\
           of background jobs that one can set up to cache this information in the \
           server, and access\n\
           it directly with:\n\n\
...
let update_server_command =
  Command.async'
    ~summary:
      "compute unclean workspaces statuses on the local machine and communicate this \
       information to Iron"
    ~readme:(fun () ->
      concat
        [ "A workspace is considered unclean if it has uncommitted changes, outgoing \
           changes, or if\n\
           an error occurred while trying to get this very information.\n\n\
           This command walks all of one's workspaces, compute unclean workspaces \
           statuses and\n\
           communicate the information to Iron to be displayed in one's [fe todo].\n\n\
           A user may run this command periodically in the background.  Typically it is \
           called by\n\
           the script [/j/office/app/fe/prod/etc/update-workspaces] which it called \
           hourly under the\n\
           standard setup.\n\n\
           One can disable this functionality by having the following line in one's \
           ferc:\n\n\
          \  (workspaces (\n\
          \    (unclean_workspaces_detection_is_enabled false)\n\
          \  ))\n\n\
           When this is disabled the command fails, unless "
    ...
    fun () ->
      ...
      if not
           (Client_config.Workspaces.unclean_workspaces_detection_is_enabled
              client_config)
      then
        if do_nothing_if_not_enabled
        then
          Async_interactive.printf
            "Unclean workspaces detection is not enabled, and the switch %s was supplied.\n\
             Exiting with code 0\n"
            Switch.do_nothing_if_not_enabled
        else (
          ...
          failwith
            (sprintf
               "Unclean workspaces detection is not enabled.\n\
                Consider enabling the functionality via your [.ferc], or supply %s"
               Switch.do_nothing_if_not_enabled))
      else (
        let%bind which_features = force which_features in
        (* Does the -for test in the client to fail early, since the walk is lengthy *)
        if (not am_functional_testing) && not (User_name.equal for_ User_name.unix_login)
        then
          raise_s
            [%sexp
          ...
          | Features features ->
            `Features
              (List.map features ~f:(fun { feature_path; include_descendants } ->
                 assert (not include_descendants);
                 (* allow_rec_flag is set to false *)
                 feature_path))
        in
...
let check_command =
  Command.async'
    ~summary:"fail if your workspaces have pending changes"
    ~readme:(fun () ->
      concat
        [ "Pending changes are unpushed or uncommitted changes in your workspaces' \
           repositories.\n\
           If a feature path is provided it will check the associated workspace or fail \
           if you have\n\
           no workspace set up for that feature.  If no feature path is given the flag \
           [-all] is\n\
           required, and the command will check all existing workspaces.\n\n\
           Note that the hgrc of a workspace repository is configured for the bookmark \
           matching the\n\
           feature path [1], so effectively this command only checks for outgoing \
           changes with that\n\
           bookmark name.\n\n\
           [1] from .hg/hgrc:\n\n\
           [defaults]\n\
...
let show_command =
  Command.async'
    ...
    fun () -> Cmd_dump.dump (Unclean_workspaces user_or_all))
;;
...
let remove_machine_command =
  Command.async'
    ...
    ~readme:(fun () ->
      "This command updates the set of workspaces known to be unclean by the server for \
       a given\n\
       USER: It removes from the unclean state all workspaces located on the given \
       MACHINE.\n\n\
       Running the command does not affect local workspaces.  It has no effect on the \
       local disk\n\
    ...
    let%map_open () = return ()
    and for_ = for_
    and machine = anon ("MACHINE" %: Arg_type.create Machine_name.of_string) in
    fun () -> With_unclean_workspaces.rpc_to_server_exn (Remove_machine (for_, machine)))
;;
