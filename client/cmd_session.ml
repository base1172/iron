...
let commit =
  Command.async'
    ...
    fun () ->
      ...
      let%bind () =
        Cmd_review.may_modify_others_review_exn
          feature_path
...
let diff =
  Command.async'
    ...
    fun () ->
      ...
      let%bind { status; remote_repo_path; _ } =
      ...
      | `Review_session
          { Get_review_session.Review_session.review_session_tip
          ; reviewer_in_session
          ; diff4s_in_session
        ...
        let diff4s_in_session =
          ...
          | (`All | `Files _) as which_file ->
            Command.Param.Which_files.restrict_exn
              which_file
              diff4s_in_session
...
let forget =
  Command.async'
    ...
    and which_session = which_session
    and files =
      flag "-file" (listed path_in_repo_arg_type) ~doc:"FILE forget only specified files"
    and all =
    ...
    fun () ->
      ...
      in
      match%bind
        Cmd_review.confirm_review_session_id_exn
          repo_root
          ~action
          ~feature_path
          ~for_
...
let set_lock ~set_is_locked_to ~summary ~readme =
  Command.async'
    ...
    and feature_path = feature_path_or_current_bookmark
    and for_ = for_
    and which_session = which_session in
    fun () ->
      let open! Deferred.Let_syntax in
      let feature_path = ok_exn feature_path in
      let%bind () =
      ...
      Session.Set_lock.rpc_to_server_exn
        { feature_path; for_; which_session; set_is_locked_to })
;;

let lock =
  set_lock
    ~set_is_locked_to:true
    ~summary:"lock the current session, prevent it from being deleted if not reviewed"
    ~readme:(fun () ->
    concat
      [ "When a session is not locked, if it has no reviewed diff and stays idle for a \
         while,\n\
         it can be silently deleted on the server and replaced by a session that will \
         make the\n\
         user review the files to a more recent changeset in the feature, as the tip \
         advances.\n\n\
         A session may be manually locked, using this command, or automatically locked \
         as soon\n\
         ...
         * all files in the session become reviewed\n\n\
         Locking a session is idempotent, but the command will fail if the user has no \
         current\n\
         session, or if someone is trying to act on behalf of someone else without \
         having the\n\
         required permissions.\n\n\
         [1] However, as of now the automatic locking is not the default behavior for \
         everyone.\n\
         It requires one to be in a special opt-in set. See [fe admin users \
...
let unlock =
  set_lock
    ~set_is_locked_to:false
    ~summary:"unlock the current session"
    ~readme:(fun () ->
    concat
      [ "One may choose to run this command in order to unlock a session, without \
         committing it.\n\n\
         Unlocking a session is idempotent, but the command will fail if the user has no \
         current\n\
         session, or if someone is trying to act on behalf of someone else without \
         having the\n\
         required permissions.\n"
...
let mark_file ~deprecated =
  ...
  Command.async'
    ...
    and paths_in_repo = paths_in_repo in
    fun () ->
      let open! Deferred.Let_syntax in
      if deprecated && am_functional_testing
      then
        failwithf
          "This command is deprecated.  Tests should be updated to use %s instead"
          subsumed_by
          ();
      let feature_path = ok_exn feature_path in
      let is_reviewing_for = `User for_ in
      let create_catch_up_for_me = create_catch_up_for_me ~is_reviewing_for |> ok_exn in
      let%bind () =
        Cmd_review.may_modify_others_review_exn
          feature_path
          ~reason:(`This reason)
      ...
      | `Up_to_date | `Bookmark_update_is_pending ->
        raise_s
          [%sexp
            "review is up to date, cannot mark file"
            , (paths_in_repo : Path_in_repo.t list)]
      | `Review_session
          { Get_review_session.Review_session.review_session_id; diff4s_in_session; _ } ->
        ...
        let ids_to_mark, invalid_files =
          List.partition_map paths_in_repo ~f:(fun path_in_repo ->
            ...
            | None -> Second path_in_repo
            | Some diff4 -> First diff4)
        in
        let invalid_files =
          ...
          then Ok ()
            ...
            Or_error.error_s
              [%sexp
                "file not found in the current session"
      ...
      let show_session ~for_ =
        ...
        | `Review_session
          ...
          let get_attribute = function
            ...
            | Line_count_to_finish_session ->
              line_count_to_finish_session |> [%sexp_of: Line_count.Review.t]
            | May_be_reviewed_by ->
            ...
            | Num_lines_remaining_to_review_in_session ->
              Line_count.Review.total line_count_to_finish_session |> [%sexp_of: int]
            | Reviewer -> reviewer_in_session |> [%sexp_of: Reviewer.t]
          in
          (match attributes with
           ...
           | [ attribute ] when not as_sexp ->
             ...
             return ()
           | attributes ->
             List.map attributes ~f:(fun attribute -> attribute, get_attribute attribute)
...
let command =
  Command.group
    ~summary:"manage your current review session for a feature"
    [ "commit", commit
    ; "diff", diff
    ; "forget", forget
    ; "lock", lock
