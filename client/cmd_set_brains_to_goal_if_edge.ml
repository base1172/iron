...
let command =
  Command.async'
    ...
    ~readme:(fun () ->
      "This command is useful when Iron's notion of review goal changes, causing \
       features that\n\
       were previously fully reviewed to become unreviewed.  In that situation, we \
       typically want\n\
       the review-goal change to apply to unreviewed features, but not reviewed ones.  \
       So, we\n\
