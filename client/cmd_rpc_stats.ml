...
module Data = struct
  ...
  let add t1 t2 = { took = Time.Span.(t1.took + t2.took); hits = t1.hits + t2.hits }
end
...
module Merge = struct
  ...
  let of_args (args : Arg.t list) =
    ...
    { by = !by; rpc_name = !rpc_name; rpc_version = !rpc_version }
  ;;

  let not_shown =
    { Key.by = User_name.of_string "users"; rpc_name = "rpcs"; rpc_version = 0 }
  ;;

  let representant t { Key.by; rpc_name; rpc_version } =
    { Key.by = (if t.by then not_shown.by else by)
...
let command =
  Command.async'
    ~summary:"output the server's RPC statistics"
    ...
    and aggregate_columns =
      map
        ...
        (enum_list
           "-merge"
           ~doc:"COLUMN[,COLUMN...] aggregate columns by|rpc|version"
           (module Merge.Arg))
    and only_users = users_option ~switch:"-only-users" in
    fun () ->
      ...
      let stats =
        ...
        List.iter stats ~f:(fun (key, data) ->
          ...
          then
            Hashtbl.update
              stats_table
              (Merge.representant aggregate_columns key)
              ~f:(function
              | None -> data
      ...
      let rows =
        ...
        in
        List.sort stats ~compare:(fun (_, d1) (_, d2) -> cmp d1 d2)
      in
      let columns =
