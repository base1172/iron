...
let clear =
  Command.async'
    ...
    and revs =
      flag "-rev" (listed rev_arg_type) ~doc:"REV revision(s) to be cleared in the cache"
    in
    fun () ->
      ...
      in
      match List.filter_opt [ clear_features which_features; clear_revisions revs ] with
...
let set_max_items_per_rpc =
  Command.async'
    ...
    (let open Command.Let_syntax in
    let%map_open () = return ()
    and max_items_per_rpc = anon ("SIZE" %: int) in
    fun () ->
...
let set_status =
  Command.async'
    ...
    ~readme:(fun () ->
      concat
        [ "For robustness and testing purposes the worker cache can be partially or \
           totally disabled.\n\
    ...
    (let open Command.Let_syntax in
    let%map_open () = return ()
    and status = enum_anon "STATUS" (module Worker_cache.Status) in
    fun () -> With_worker_cache.rpc_to_server_exn (Set_status status))
;;
...
let show =
  ...
  Command.async'
    ~summary:"output the worker cache"
    (let open Command.Let_syntax in
    let%map_open () = return ()
    and what_to_dump =
      let%map revs =
        no_arg_flag "-revs" ~doc:" dump the revs for which there are cached values"
      and values =
        flag
          "-values-at-rev"
      ...
      Or_error.try_with (fun () ->
        ...
        | _, _, _ ->
          failwithf
            "The flags [%s], [%s] and [%s] are mutually exclusive"
            revs_switch
            values_switch
            obligations_switch
            ())
...
let analyze_obligations =
  Command.async'
    ...
    ~readme:(fun () ->
      concat
        [ "The command expects to be given obligations files obtained by call to the \
           command:\n\n\
          \  $ fe obligations report -stable\n\n\
           from the same repo, built at various revisions.  The files are then going to \
           be parsed,\n\
           and transformed into the worker-cache representation on the server.  Then \
           several stats\n\
           are given.\n"
        ])
    (let open Command.Let_syntax in
    ...
    and obligations_files =
      anon (sequence ("obligations-report.sexp, [..]" %: Filename_unix.arg_type))
    in
    fun () ->
      let open! Deferred.Let_syntax in
      ...
      let module Table_line = struct
        ...
        module Acc = struct
          ...
          let init = { cumulative_number_of_dirs = 0; cumulative_number_of_files = 0 }
        end
      end
      in
      let number_of_analyzed_files = List.length obligations_files in
      Print.printf
        "Will analyze obligations from %d report files\n"
      ...
      let%bind rows =
        deferred_fold_map
          ...
          let duration_to_build_tree = Time.diff (Time.now ()) time_build_tree_start in
          ...
          in
          let cumulative_number_of_dirs = cumulative_number_of_dirs + number_of_dirs in
          let cumulative_number_of_files = cumulative_number_of_files + number_of_files in
          Gc.compact ();
          Print.printf
            "Analyzing %s ... ( %3d / %d )\n"
            filename
          ...
          ( { Table_line.Acc.cumulative_number_of_dirs; cumulative_number_of_files }
          , { Table_line.filename
            ; tree
      ...
      let table =
          ...
          Ascii_table.Column.(
            int
              ...
              (cell (fun (line : Table_line.t) ->
                 match
                   List.Assoc.find ~equal:String.equal line.hash_consing_stats module_name
                 with
                 | None -> 0
        ...
        Ascii_table.create
          ~columns:
            Ascii_table.Column.
              ...
              ; string
                  ~header:"process"
                  (cell (fun t ->
                     Table_line.duration_to_build_tree t |> Time_ns.Span.to_string_hum))
              ; int ~header:"dirs (total)" (cell Table_line.cumulative_number_of_dirs)
              ; int ~header:"files (total)" (cell Table_line.cumulative_number_of_files)
          ...
          ~rows
      in
      print_string (Ascii_table.to_string table ~display_ascii ~max_output_columns);
      print_string
        (sprintf
...
let command =
  Command.group
    ...
    ~readme:(fun () ->
      concat [ "Operations side effecting the cache require admin privileges.\n" ])
    [ "clear", clear
