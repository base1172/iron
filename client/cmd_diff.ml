...
let command =
  Command.async'
    ...
    and raw = no_arg_flag "-raw" ~doc:"show Iron's internal representation of the diff"
    and sort_build_order = sort_build_order
    and summary = no_arg_flag "-summary" ~doc:"show a summary of the diff" in
    fun () ->
      ...
      then (
        ...
        | Pending_since since ->
          (match is_archived with
           ...
           | No ->
             die
               "diffs have been pending for"
               (Time.Span.to_short_string (how_long ~since))
               [%sexp_of: string])
        | Known (Error error) -> die "diffs cannot be computed" error [%sexp_of: Error.t]
        | Known (Ok diff4s) ->
          let%bind diff4s =
            if sort_build_order
            then
              ...
              return
                (List.sort diff4s ~compare:(fun d1 d2 ->
          ...
          then (
            ...
            let%bind () =
              ...
              then return ()
                ...
                Hg.pull repo_root ~from:remote_repo_path ~even_if_unclean:true (`Rev tip)
            in
