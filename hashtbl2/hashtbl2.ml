...
let to_sexp_repr t =
  List.concat_map (Hashtbl.to_alist t.by_key1) ~f:(fun (key1, table2) ->
    List.map (Hashtbl.to_alist table2) ~f:(fun (key2, data) -> key1, key2, data))
;;
...
let clear t = Hashtbl.clear t.by_key1
let mem1 t key1 = Hashtbl.mem t.by_key1 key1
...
let iter t ~f =
  Hashtbl.iteri t.by_key1 ~f:(fun ~key:key1 ~data:by_key2 ->
    Hashtbl.iteri by_key2 ~f:(fun ~key:key2 ~data -> f key1 key2 data))
;;
...
let remove_error t key1 key2 =
  ...
  failwiths
    ~here:[%here]
    "Hashtbl2.remove_exn of absent keys"
    (key1, key2)
    [%sexp_of: key1 * key2]
;;
...
let remove_exn t key1 key2 =
  ...
  | Some by_key2 ->
    ...
    then (
      ...
      if Hashtbl.is_empty by_key2 then Hashtbl.remove t.by_key1 key1)
    else remove_error t key1 key2
;;

module Make (Key1 : Key) (Key2 : Key) = struct
  module Table1 = Hashtbl.Make (struct
    include Key1

    let t_of_sexp _ = assert false
  end)

  module Table2 = Hashtbl.Make (struct
    include Key2
  ...
  let equal equal_data t1 t2 =
    Hashtbl.equal
      (fun by_key2_1 by_key2_2 -> Hashtbl.equal equal_data by_key2_1 by_key2_2)
      t1.by_key1
      t2.by_key1
  ;;
end
