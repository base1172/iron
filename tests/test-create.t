...
Persistence.
  ...
  $ fe-server stop
  $ fe tools feature-exists root |& matches "The Iron server is down unexpectedly." >/dev/null
  [1]
  $ fe-server start
  $ fe list
  |------------------------------------|
...
Cannot make child off known-bad base:
  ...
  $ fe create root/child -description 'child' 2>&1 | sanitize_output
  (error
   (create-feature
    ("invalid base revision {ELIDED}"
...
Non cr-clean base.
  ...
  $ fe create root/child/cr -description '' 2>&1 | sanitize_output
  (error
   (create-feature
    ("invalid base revision {ELIDED}"
     "revision is not CR clean -- consider using -allow-non-cr-clean-base")))
...
Confirm whole-feature-reviewers for root/child2
  $ fe show root/child2 -whole-feature-reviewers
