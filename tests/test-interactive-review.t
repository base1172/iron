...
Quit the review without having reviewed anything.
  ...
  $ echo 'q' | do_interactive_review | matches "Quit"
  $ fe internal session show-num-lines
  20

Requesting help on the first menu.

  $ echo -e "?\nq" | do_interactive_review | grep -A 30 'How do you want to do this review?'
...
Requesting help on the file menu.
  ...
  $ echo -e "F\n?\nq" | do_interactive_review | grep -A 30 'Mark as reviewed?'
...
supplying a reason is required.
  ...
  $ echo -e "F\ny\nq" | IRON_USER=user1 do_interactive_review -for unix-login-for-testing 2>&1 \
  >   | matches "must supply -reason when acting for someone"
  [1]

  $ echo -e "F\ny\nq" \
  >   | IRON_USER=user1 do_interactive_review -for unix-login-for-testing -reason 'reason' >/dev/null

When there are catch-up lines to review, they are shown first ...

  $ echo -e "q\n" | do_interactive_review | grep -A 300 'Catch-up.'
...
... unless one supplies [-skip-catch-up-review] ...
  ...
  $ echo -e "q\n" | do_interactive_review -skip-catch-up-review | grep -A 300 'Reviewing.'
  Reviewing root to *. (glob)
...
the switches are supplied.
  ...
  $ IRON_USER=user1 do_interactive_review -create-catch-up-for-me -for unix-login-for-testing
  Cannot use [-create-catch-up-for-me] when reviewing for other users only.
  [1]

  $ do_interactive_review -only-catch-up-review -for unix-login-for-testing -create-catch-up-for-me
  At most one of the switches -only-catch-up-review and -create-catch-up-for-me may be supplied.
  [1]

  $ do_interactive_review -only-catch-up-review -skip-catch-up-review
...
always allowed).
  ...
  $ echo -e 'F\ny\nq\n' \
  >   | IRON_USER=user1 do_interactive_review -only-catch-up-review -for unix-login-for-testing >/dev/null
...
and the proper indication is printed on stdout.
  ...
  $ echo -e 'F\ny\nq\n' \
  >   | IRON_USER=user1 do_interactive_review -only-catch-up-review -for unix-login-for-testing
