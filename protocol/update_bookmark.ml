module Stable = struct
  ...
  module Action = struct
    ...
    module V5 = struct
      ...
      let to_model { feature_path; feature_id; info } =
        { Model.feature_path
        ; feature_id
        ; info
      ...
      let of_model { Model.feature_path; feature_id; info; augment_worker_cache = _ } =
        { feature_path; feature_id; info }
      ;;
    end
  end
...
module Info = struct
  ...
  module Concise = struct
    ...
    let sexp_of_diff4s diff4s =
      ...
      diff4s
      |> if List.length diff4s > 5_000 then [%sexp_of: _] else [%sexp_of: Diff4.t list]
    ;;
