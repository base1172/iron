...
module To_default_host_and_port = struct
  type t = { may_connect_to_proxy : bool } [@@deriving compare, sexp, hash]
end
...
let make_fresh_connection (connect_to : Connect_to.t) =
  ...
  let%bind host_and_port =
    ...
    | Default_host_and_port { To_default_host_and_port.may_connect_to_proxy } ->
      Iron_config.Restricted_for_rpcs.load_as_per_IRON_CONFIG ~may_connect_to_proxy ()
  in
  ...
  match%bind
    ...
    | Error exn ->
      return
        (Or_error.error
           "could not connect to Iron server"
...
let rpc_to_server_gen_exn
  ...
  let response_or_error =
    ...
    print_elapsed [%here];
    Async_durable.with_ durable_connection ~f:(fun connection ->
...
module Make (Name : Name) (Version : Version) (Action : Action) (Reaction : Reaction) =
struct
  module Model = struct
    include Name
    ...
    type response = Reaction.t Or_error.Stable.V1.t Response.Stable.V1.t
    [@@deriving bin_io, sexp_of]
  end

  include Model

  module Both_convert = Versioned_rpc.Both_convert.Plain.Make (struct
    include Name
    module Caller = Model
    module Callee = Model
  end)
  ...
  struct
    let () =
      ...
      | Error error ->
        raise_s
          [%sexp
            "map_reaction_in_client cannot be registered multiple time"
            , (Name.name : string)
            , (error : Error.t)]
    ;;
  end
  ...
  let rpc_to_server_exn ?connection:(how_to_connect = `Default) action =
    ...
    if verbose then Verbose.message "rpc query" (name, query) [%sexp_of: string * query];
    rpc_to_server_gen_exn
      ~may_connect_to_proxy:true
      ~how_to_connect
      ~dispatch_callback:(fun connection -> Both_convert.dispatch_multi connection query)
      ~response_callback:(fun response ->
        if verbose
        then Verbose.message "rpc response" (name, response) [%sexp_of: string * response];
  ...
  module Register_old_rpc_converting_both_ways (Version : sig
    val version : int
  end)
  ...
  struct
    module Old_rpc = struct
      ...
      let callee_model_of_query query = Query.Stable.V1.map query ~f:Action.to_model

      let response_of_callee_model response =
        Response.Stable.V1.map response ~f:(Or_error.Stable.V1.map ~f:Reaction.of_model)
      ;;

      let query_of_caller_model query = Query.Stable.V1.map query ~f:Action.of_model
  ...
  module Register_old_rpc (Version : sig
    val version : int
  end)
  ...
  struct
    module Old_rpc = struct
      ...
      let unsupported () =
        raise_s
          [%sexp
            "it is not supported to use a new client with an old Iron server"
            , (Name.name : string)]
      ;;
...
struct
  ...
  module Both_convert = Versioned_rpc.Both_convert.Pipe_rpc.Make (struct
    include Name
    module Caller = Model
    module Callee = Model
  end)
      ...
      ~response_callback:(fun result ->
        ...
        let map_reaction =
          ...
          | Some of_server_reaction ->
            fun reaction ->
              Or_error.try_with (fun () -> of_server_reaction action reaction)
        in
        ...
        don't_wait_for
          (let%bind () =
           ...
           (match close_reason with
            ...
            | Error error -> Pipe.write_without_pushback_if_open writer (Error error));
           Pipe.close writer);
        reader)
  ;;
