module Stable = struct
  ...
  module Locked = struct
    ...
    module V1 = struct
      ...
      let of_v2 { V2.by; reason; at; is_permanent = _ } = { by; reason; at }
    end
  ...
  module V20 = struct
    ...
    let of_model m =
      ...
      in
      let locked =
        List.filter_map locked ~f:(fun (lock_name, locked) ->
          Option.map (Lock_name.Stable.V2.of_v3 lock_name) ~f:(fun lock_name ->
            lock_name, locked))
      in
      let next_steps = List.map next_steps ~f:Next_step.Stable.V5.of_v6 in
      let users_with_unclean_workspaces =
        Map.map users_with_unclean_workspaces ~f:Unclean_workspace_reason.Stable.V1.of_v2
      in
      { feature_id
      ; feature_path
      ; rev_zero
  ...
  module V19 = struct
    ...
    let of_model m =
      ...
      let line_count_by_user =
        ...
        |> Or_error.map
             ~f:
               (List.map ~f:(fun (user, line_count) ->
  ...
  module V18 = struct
    ...
    let of_model m =
      ...
      let line_count_by_user =
        ...
        |> Or_error.map
             ~f:
               (List.filter_map ~f:(fun (user, line_count) ->
                  let ({ Line_count.Stable.V3.review; follow; catch_up; completed } as
                      line_count)
                    =
  ...
  let line_count_by_user_of_v3 line_count_by_user =
    Or_error.V1.map line_count_by_user ~f:(fun by_user ->
      List.filter_map by_user ~f:(fun (user, line_count) ->
        let ({ Line_count.V2.review; catch_up; completed } as line_count) =
          Line_count.V2.of_v3 line_count
        in
  ...
  module V13 = struct
    ...
    let of_model m =
      ...
      ; reviewing
      ; allow_review_for
      ; included_features = List.map included_features ~f:Released_feature.Stable.V2.of_v3
      ; properties
      ; remote_repo_path
      ; has_children
      ; release_process
...
let released_features t ~sorted_by =
  ...
  | `Name ->
    List.sort by_release_time_decreasing ~compare:(fun (r1 : Released_feature.t) r2 ->
...
let reviewers_exn t ~sort =
  let sort =
    match sort with
    | `Alphabetically ->
      List.sort ~compare:(fun (u1, _) (u2, _) -> User_name.compare u1 u2)
    | `Decreasing_review -> Line_count.sort_decreasing_review
  in
  ok_exn t.line_count_by_user |> sort |> List.map ~f:fst
;;
...
let recover_diff_of_its_latest_release t =
  ...
  | Some { released_feature; diff_from_base_to_tip } ->
    ...
    then None
      ...
      let { Released_feature.feature_id = _
          ; feature_path = _
          ; description = _
          ; owners
          ; whole_feature_followers
