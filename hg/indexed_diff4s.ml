        ...
        Hashtbl.iteri table ~f:(fun ~key:diff2 ~data:diff4s ->
          List.iter diff4s ~f:(fun diff4 ->
            [%test_result: Diff2.Ignoring_rev.t]
              (by diff4 ~num_lines_in_diff:1)
              ~expect:diff2)))
    in
    Fields.iter ~by_input:(check_table Diff4.input) ~by_output:(check_table Diff4.output))
;;
...
let create diff4s =
  ...
  in
  { by_input = table ~by:Diff4.input; by_output = table ~by:Diff4.output }
;;
