module Stable = struct
  ...
  module Root = struct
    module V1 = struct
      type t = { tip_cr_soons : Cr_soon_multiset.V1.t }
  ...
  module Non_root = struct
    module V1 = struct
      type t = { active : Cr_soon_multiset.V1.t }
      [@@deriving bin_io, compare, fields, sexp]
...
module Root = struct
  ...
  let invariant t =
    Invariant.invariant [%here] t [%sexp_of: t] (fun () ->
      ...
      Fields.iter ~tip_cr_soons:(check Cr_soon_multiset.invariant))
  ;;
...
module Non_root = struct
  ...
  let invariant t =
    Invariant.invariant [%here] t [%sexp_of: t] (fun () ->
      let check f = Invariant.check_field t f in
      Fields.iter ~active:(check Cr_soon_multiset.invariant))
  ;;
  ...
  let create
    ...
    | exn ->
      error
        "Non_root.create"
        (feature_path, base_facts, tip_facts, exn)
...
module In_feature = struct
  ...
  let rename_non_root ({ feature_path; info } as t) ~to_ =
    ...
    | Root _ ->
      raise_s
        [%sexp
          "Cr_soons.In_feature_tree.rename_non_root cannot rename root feature"
...
module In_feature_tree = struct
  ...
  let invariant t =
    Invariant.invariant [%here] t [%sexp_of: t] (fun () ->
      ...
      let check_root_name feature_name =
        ...
        | Some root_name -> [%test_result: Feature_name.t] feature_name ~expect:root_name
      in
      Fields.iter
        ~root:
          (check (fun root ->
             Option.iter (Var.value root) ~f:(fun root ->
               Cr_soon_multiset.iter
                 (Root.tip_cr_soons root)
        ...
        ~inactive_by_assignee:
          (check (fun obs -> ignore (Observer.value_exn obs : _ Map.t))))
  ;;
    ...
    let active =
      Incr.bind (Var.watch non_roots) ~f:(fun non_roots ->
        Incr.map (Incr.all non_roots) ~f:(fun non_roots ->
          Cr_soon_multiset.unions (List.map non_roots ~f:Non_root.active)))
    in
    let active_by_assignee =
      Incr.map2
        active
        (Var.watch alternate_names_var)
  ...
  let is_empty t =
    Option.is_none (Var.value t.root) && Hashtbl.is_empty t.non_root_by_path
  ;;

  let set_non_roots t =
    Var.set t.non_roots (List.map (Hashtbl.data t.non_root_by_path) ~f:Var.watch)
  ;;

  let update_feature t (in_feature : In_feature.t) =
    (match in_feature.info with
     | Root root -> Var.set t.root (Some root)
     | Non_root non_root ->
  ...
  let remove t feature_path =
    ...
    else if Hashtbl.mem t.non_root_by_path feature_path
    then (
  ...
  let iter_inactive_assignees t ~f =
    ...
    | Ok map ->
      Map.iteri map ~f:(fun ~key:user_name ~data:multiset ->
        if not (Cr_soon_multiset.is_empty multiset) then f user_name)
  ;;
end
...
let invariant t =
  Invariant.invariant [%here] t [%sexp_of: t] (fun () ->
    let check f = Invariant.check_field t f in
    Fields.iter
      ~feature_tree_by_root:
        (check (fun feature_tree_by_root ->
           Hashtbl.iteri feature_tree_by_root ~f:(fun ~key:_ ~data ->
             In_feature_tree.invariant data)))
      ~alternate_names_var:
        (check (fun var -> User_name_by_alternate_name.invariant (Var.latest_value var))))
;;
...
let create () =
  ...
  { feature_tree_by_root = Feature_name.Table.create (); alternate_names_var = var }
;;

let get_feature_tree t feature_path =
  Hashtbl.find_or_add
    t.feature_tree_by_root
    (Feature_path.root feature_path)
    ~default:(fun () -> In_feature_tree.create t.alternate_names_var)
;;

let update_feature t (in_feature : In_feature.t) =
  In_feature_tree.update_feature (get_feature_tree t in_feature.feature_path) in_feature
;;
