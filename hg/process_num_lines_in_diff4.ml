...
let compute input =
  ...
  | Error error ->
    raise_s [%sexp "line count computation error", (input : Action.t), (error : Error.t)]
  | Ok stdout ->
    (try Sexp.of_string_conv_exn stdout Reaction.t_of_sexp with
     | exn ->
       raise_s
         [%sexp
           "line count process parse sexp error"
           , (input : Action.t)
           , (stdout : string)
           , (exn : Exn.t)])
;;
