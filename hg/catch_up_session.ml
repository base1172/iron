module Stable = struct
    ...
    module V2 = struct
      ...
      let of_v3 { V3.diff4_in_session; reason } =
        ...
        | Reviewed_by_someone_else { reviewed_by; reason } ->
          { diff4_in_session; reviewed_by; reason }
      ...
      let to_v3 { diff4_in_session; reviewed_by; reason } =
        { V3.diff4_in_session; reason = Reviewed_by_someone_else { reviewed_by; reason } }
      ;;
    end
  end
  ...
  module Action_query = struct
    module V3 = struct
      type t = Action.V3.t Query.V1.t [@@deriving sexp]
...
let invariant t =
  Invariant.invariant [%here] t [%sexp_of: t] (fun () ->
    ...
    Fields.iter
      ...
      ~diff4s_in_session_by_id:
        (check (fun diff4s_in_session_by_id ->
           let ids_in_table =
             Hashtbl.keys diff4s_in_session_by_id |> Diff4_in_session.Id.Hash_set.of_list
           in
           assert (Hash_set.equal ids_in_session ids_in_table)))
      ~diff4s_to_catch_up:
        (check (fun diff4s_to_catch_up ->
           Hashtbl.iteri diff4s_to_catch_up ~f:(fun ~key:id ~data:diff4_to_catch_up ->
             if not (Hash_set.mem ids_in_session id)
             then
               raise_s
                 [%sexp
                   "unknown id in to catch up"
                   , [%here]
                   , (diff4_to_catch_up : Diff4_to_catch_up.t)];
             [%test_result: Diff4_in_session.Id.t]
               ~expect:id
  ...
  let diff4s_to_catch_up, invalid_ids =
    List.partition_map ids_and_kinds ~f:(fun { Id_and_kind.id; kind } ->
      ...
      | Some diff4_in_session ->
        First { Diff4_to_catch_up.diff4_in_session; reason = kind }
      | None -> Second id)
  in
...
let set_feature_path_internal t feature_path = t.feature_path <- feature_path
...
let diff4s_to_catch_up t = Hashtbl.data t.diff4s_to_catch_up
...
let line_count_remaining_to_catch_up t =
  let actual_line_count =
    ...
    |> List.fold ~init:Line_count.Catch_up.zero ~f:(fun acc diff4_to_catch_up ->
         Line_count.Catch_up.add_count
           acc
           diff4_to_catch_up.reason
