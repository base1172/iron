...
module Cache = struct
  module Rev_pair = struct
    module T = struct
      type t = Rev.Compare_by_hash.t * Rev.Compare_by_hash.t [@@deriving compare, sexp_of]
  ...
  module Rev_and_path_in_repo = struct
    module T = struct
      type t = Rev.Compare_by_hash.t * Path_in_repo.t [@@deriving compare, sexp_of]
  ...
  let status t rev1 rev2 = Hashtbl.find_exn t.status_cache (rev1, rev2)
  let manifest t rev = Hashtbl.find_exn t.manifest_cache rev
  let cat t rev = Hashtbl.find_exn t.cat_cache rev
    ...
    List.fold
      ...
      ~f:(fun set (ob1, ob2) ->
        Hashtbl.fold ob1.by_path ~init:set ~f:(fun ~key:path ~data:attr1 set ->
          ...
          | None -> set
          | Some attr2 ->
            if Review_attributes.equal attr1 attr2 then set else Set.add set path))
  ;;
  ...
  let fill_in_manifest_cache t =
    Deferred.List.iter
      ~how:(`Max_concurrent_jobs 4)
      (Set.to_list t.all_revs)
      ~f:(fun rev ->
      let%map paths = Hg.manifest t.repo_root (`Revset (Hg.Revset.of_rev rev)) in
      Hashtbl.add_exn t.manifest_cache ~key:rev ~data:(Path_in_repo.Set.of_list paths))
  ;;

  let fill_in_cat_cache t dir =
    let superset_by_rev =
      Map.map
        ~f:Path_in_repo.Set.union_list
    ...
    Deferred.List.iter
      ~how:(`Max_concurrent_jobs 4)
      (Map.to_alist paths_to_cat_by_rev)
...
let attributed_files_diamonds cache revs attributed_files_by_paths
  ...
  let attributed_file_diamonds_by_path =
    Set.to_map paths_of_interest ~f:(fun path ->
      Diamond.map2 revs attributed_files_by_paths ~f:(fun rev attributed_file_by_path ->
           ...
           Or_error.try_with (fun () ->
             Diamond.map2 d1 d2 ~f:(fun x y ->
               ...
               | true, true ->
                 raise_s
                   [%sexp
                     "Tried to take into account a renaming but couldn't"
         ...
         Map.iteri renamings ~f:(fun ~key:src ~data:dsts ->
           List.iter dsts ~f:(fun (dst, renaming) ->
             ...
             | Some uf_left, Some uf_right ->
               if not (Union_find.same_class uf_left uf_right)
               then (
                 ...
                 | Ok files_merged ->
                   ...
                   Union_find.set
                     uf_left
                     (path_left, (files_merged, errors_left @ errors_right)))));
         map)
    |> Map.data
  ...
  in
  if verbose
  then Verbose.message "attributed_files_diamonds" (`Result result :: debug) sexp_of_debug;
  result
;;
...
let create cache revs ~lines_required_to_separate_ddiff_hunks =
  ...
  let file_by_path_by_rev =
    Rev.Compare_by_hash.Map.of_alist_fold
      ...
      ~f:(fun acc map ->
        Map.merge acc map ~f:(fun ~key:_ -> function
          | `Left x -> Some x
          | `Right x -> Some x
          | `Both (_, x) -> Some x))
  in
...
let create_using_fake_obligations
  repo_root
  rev_diamond
  ~lines_required_to_separate_ddiff_hunks
  =
...
let command =
  Command.async'
    ~summary:"compute the diffs between four revs"
    ...
    and lines_required_to_separate_ddiff_hunks =
      ...
      flag
        ...
        ~doc:
          (sprintf
             "INT required spacing to separate two ddiff hunks (default %d)"
             default)
    in
    fun () ->
      ...
      let%map result =
        create_using_fake_obligations
          ~lines_required_to_separate_ddiff_hunks
          repo_root
          (Diamond.map diamond ~f:Raw_rev.string)
      in
      print_endline (result |> ok_exn |> [%sexp_of: Diff4.t list] |> Sexp.to_string_hum))
;;
