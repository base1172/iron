module Stable = struct
  ...
  module Feature = struct
    module V2 = struct
      ...
      [@@deriving compare, sexp] [@@sexp.allow_extra_fields]
    end
    ...
    (module struct
      ...
      let%test_unit "write" =
        [%test_result: t]
          ...
          |> (fun s ->
               assert_all_lines_are_commented s;
               s)
...
let extract_info repo_root =
  ...
  Monitor.try_with_or_error ~extract_exn:true (fun () ->
    ...
    Commented_sexp.of_commented_string_hum contents |> [%of_sexp: Info.t])
;;

let info_of_t t =
  { Info.generated_by = Version_util.version
...
let t_of_info repo_root (info : Info.t) =
  { repo_root; remote_repo_path = info.remote_repo_path; kind = info.kind }
;;

let%test_unit "info" =
  let repo_root = Repo_root.of_abspath (Abspath.of_string "/dev/null") in
  let t =
    { repo_root; remote_repo_path = Remote_repo_path.null; kind = `Fake_for_testing }
  in
...
let default_line_regex = Regex.create_exn "^[ ]*default[ ]+=[ ]+(ssh://hg//hg/.*)$"
...
let extract_root_feature_from_hgrc repo_root =
  ...
  then
    Debug.ams
      [%here]
      "Workspace_hgrc.extract_root_feature_from_hgrc"
      res
      [%sexp_of: Feature_name.t Or_error.t];
  res
;;
