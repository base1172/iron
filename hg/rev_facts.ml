...
module Is_ancestor = struct
  ...
  let reflexive rev = { ancestor = rev; descendant = rev; status = true }

  let transitive t1 t2 =
    if Rev.equal_node_hash t1.descendant t2.ancestor
    then
      Ok
        { ancestor = t1.ancestor
  ...
  let check t ~ancestor ~descendant =
    ...
    then Ok t.status
      ...
      error
        "mismatched revs"
        (t, `ancestor ancestor, `descendant descendant)
...
module Is_conflict_free = struct
  ...
  let create ?repo_is_clean repo_root rev =
    let%bind Repo_is_clean = assert_working_copy_of ?repo_is_clean repo_root ~is:rev in
  ...
  let create ?repo_is_clean repo_root format ~incremental_based_on rev ~file_owner =
    ...
    | Ok format ->
      ...
      let%map ({ due_now; due_soon = _ } as grepped_crs) =
        Cr_comment.maybe_incremental_grep
          repo_root
          format
...
module Obligations_are_valid = struct
  ...
  let check t rev =
    if Rev.equal_node_hash t.rev rev
    then Ok t.status
    else error "Obligations_are_valid.check mismatched rev" (t, rev) [%sexp_of: t * Rev.t]
  ;;
...
let invariant t =
  Invariant.invariant [%here] t [%sexp_of: t] (fun () ->
    ...
    assert (revs_agree t);
    Fields.iter ~is_conflict_free:ignore ~is_cr_clean:ignore ~obligations_are_valid:ignore)
;;
...
let create is_conflict_free is_cr_clean obligations_are_valid =
  ...
  then Ok t
    ...
    error
      "Triple.create got mismatched revs"
...
let check_true ?allow_non_cr_clean ?error_msg_if_not_cr_clean t rev =
  match
    check_gen t rev ?allow_non_cr_clean ?error_msg_if_not_cr_clean ~if_check_fails:`Error
  with
  | Ok true -> Ok ()
