...
let for_sorted_output { rev_facts; crs; cr_soons } =
  { rev_facts
  ; crs = Or_error.map crs ~f:(List.sort ~compare:Cr_comment.For_sorted_output.compare)
  ; cr_soons =
      Or_error.map cr_soons ~f:(List.sort ~compare:Cr_soon.For_sorted_output.compare)
  }
;;
...
module Concise = struct
  ...
  let sexp_of_t { rev_facts; crs; cr_soons } =
    [%sexp
      { rev_facts : Rev_facts.t; crs : crs_count_or_error; cr_soons : crs_count_or_error }]
  ;;
end
      ...
      Or_error.map crs ~f:(fun crs ->
        ...
        Hashtbl.fold by_file ~init:[] ~f:(fun ~key ~data alist ->
          let deterministic_data =
            List.sort ~compare:X.Structurally_compared.compare data
          in
