module Stable = struct
  ...
  module Properties = struct
    ...
    module V1 = struct
      ...
      let to_model { max_size; status } =
        V2.to_model { V2.max_size; status; max_items_per_rpc = default_max_items_per_rpc }
      ;;
    end
  ...
  module By_rev = struct
    module V1 = struct
      type 'a t = (Rev.V1.t * 'a) list [@@deriving bin_io, compare, sexp]
  ...
  module From_server_to_worker = struct
    module V6 = struct
      ...
      let empty =
        { worker_obligations = []
        ; worker_rev_facts = []
        ; properties = { max_size = 1; status = Disabled; max_items_per_rpc = 1 }
        }
      ;;
    end
  ...
  module From_worker_back_to_server = struct
    module V5 = struct
      ...
      let empty = { worker_obligations = []; worker_rev_facts = [] }
    end
...
module Properties = struct
  ...
  include (
    Stable.Properties.Model : module type of Stable.Properties.Model with type t := t)
...
module Concise_worker_obligations = struct
  let sexp_of_t t = t |> List.map ~f:fst |> [%sexp_of: Rev.t list]
end

module Concise_worker_rev_facts = struct
  let sexp_of_one (rev, worker_rev_facts) =
    ignore rev;
    (* already printed 3 times in rev_facts *)
...
module Worker_session = struct
  ...
  module Cached = struct
    ...
    let to_server t ~f =
      List.fold ~init:Rev.Compare_by_hash.Map.empty t.to_server ~f:(fun map (key, data) ->
        (* Only retain the most recent in case of a race between multiple calls to
             [use_or_compute_and_store]. *)
        match Map.find map key with
        | Some _ -> map
        | None -> Map.set map ~key ~data:(f data))
      |> Map.to_alist
    ;;
  end
  ...
  let create { From_server_to_worker.worker_obligations; worker_rev_facts; properties } =
    { worker_obligations =
        Cached.create worker_obligations ~f:Worker_obligations.of_stable
  ...
  let back_to_server t =
    ...
    in
    { From_worker_back_to_server.worker_obligations =
        to_server t.worker_obligations ~f:Worker_obligations.to_stable
  ...
  module Key = struct
    ...
    let get_cached : type a. t_ -> a t -> a Cached.t =
     fun t key ->
      match key with
  ...
  let find t rev key = Hashtbl.find (Key.get_cached t key).by_rev rev
  let remove t rev key = Hashtbl.remove (Key.get_cached t key).by_rev rev
end
...
let invariant t =
  Invariant.invariant [%here] t [%sexp_of: t] (fun () ->
    ...
    Fields.iter
      ...
      ~worker_obligations:
        (check (By_rev.invariant Worker_obligations.On_server.invariant))
...
let send_to_worker t { Feature_revs.diff4s_revs; base; tip } =
  ...
  | Read_write ->
    ...
    let rev_facts_revs = Rev.Compare_by_hash.Set.of_list [ base; tip ] in
    let obligations_revs = Set.union rev_facts_revs diff4s_revs in
    { From_server_to_worker.worker_obligations =
        to_worker
          t.worker_obligations
          obligations_revs
