module Stable = struct
  ...
  module Creation = struct
    module Context = struct
      ...
      type t = { user_name : User_name.t } [@@deriving sexp_of]
    end
    ...
    module V4 = struct
      ...
      let to_model (context : Context.t) { reviewer; id; diff4s; sort_diff4s; tip; base } =
        V5.to_model
          context
          { V5.reviewer =
    ...
    module V2 = struct
      ...
      let to_model context { reviewer; id; diff4s; sort_diff4s; tip } =
        let diff4s =
          List.map diff4s ~f:(fun diff4 : Diff4.And_output_num_lines.V1.t ->
            { diff4; output_num_lines = 1 })
        in
        V3.to_model context { V3.reviewer; id; diff4s; sort_diff4s; tip }
      ;;
    end
    ...
    module V1 = struct
      ...
      let to_model context { id; diff4s; tip } =
        V2.to_model
          context
          { V2.reviewer = Whole_feature_reviewer; id; diff4s; sort_diff4s = true; tip }
      ;;
    end
  end
...
module Diff4_in_session = struct
  ...
  let compare_by_path_in_repo_at_f2_for_review t1 t2 =
    Path_in_repo.default_review_compare (path_in_repo_at_f2 t1) (path_in_repo_at_f2 t2)
  ;;
...
module Reviewable_diff4 = struct
  module Review_status = struct
    module Maybe_frozen_review_kind = struct
      ...
      type t = { mutable frozen_review_kind : Review_kind.t option } [@@deriving sexp_of]
    end
  ...
  let diff4_in_session { index; diff4; review_status } =
    { Diff4_in_session.index
    ; diff4
    ; is_reviewed = Review_status.is_reviewed review_status
    }
  ;;

  let is_implicitly_reviewed t reviewer = Diff4.is_implicitly_reviewed t.diff4 reviewer
  ...
  let invariant t reviewer =
    Invariant.invariant [%here] t [%sexp_of: t] (fun () ->
      ...
      Fields.iter
        ...
        ~review_status:
          (check (fun review_status ->
             if is_implicitly_reviewed t reviewer
             then assert (Review_status.is_reviewed review_status))))
  ;;

  let reviewed t ~frozen_review_kind = t.review_status <- Reviewed { frozen_review_kind }
  let unreviewed t = t.review_status <- Not_reviewed

  let check_unreviewed t =
    if is_reviewed t then Ok () else error "not reviewed yet" t.diff4 [%sexp_of: Diff4.t]
  ;;
  ...
  let num_lines_completed t reviewer =
    if is_reviewed t then Diff4.num_lines t.diff4 reviewer else 0
  ;;
end

module Reviewable_diff4s = struct
  type t = Reviewable_diff4.t array [@@deriving sexp_of]
  ...
  let create diff4s reviewer ~sort_diff4s =
    ...
    let t =
      ...
      |> List.mapi ~f:(fun index diff4 ->
           { Reviewable_diff4.index; diff4; review_status = Not_reviewed })
      |> Array.of_list
    in
  ...
  map_diff4s_not_implicitly_reviewed t ~f:(fun reviewable_diff4 ->
    let review_kind =
      ...
      | Reviewed ({ frozen_review_kind = None } as r) ->
        ...
        review_kind
    in
    { Diff4_in_session.And_review_kind.diff4_in_session =
        Reviewable_diff4.diff4_in_session reviewable_diff4
    ; review_kind
    })
;;
...
let filter_diff4s t ~f:predicate =
  let by_diff4 =
    Diff4.Table.of_alist_exn (List.map t.diff4s ~f:(fun ({ diff4; _ } as z) -> diff4, z))
  in
  Array.filter_map t.reviewable_diff4s ~f:(fun reviewable_diff4 ->
...
let reviewed_diff4s t = filter_diff4s t ~f:Reviewable_diff4.is_reviewed
...
let t_of_creation creation ~dynamic_upgrade_state ~serializer =
  let { Creation.reviewer; id; diff4s; sort_diff4s; tip; base } = creation in
  { reviewer
  ; id
  ; diff4s
  ; tip
...
let create
  ...
  let creation = { Creation.reviewer; id; diff4s; sort_diff4s; tip; base } in
  let context = { Creation.Context.user_name = reviewer.user_name } in
  let t = t_of_creation creation ~dynamic_upgrade_state ~serializer:(Some serializer) in
...
let check_session_id t session_id = Session_id.check ~actual:t.id ~supplied:session_id
let is_locked t = t.is_locked

let have_done_some_review t =
  Array.exists
    (diff4s_in_session_not_implicitly_reviewed t)
...
let set_is_locked_internal t is_locked = t.is_locked <- is_locked
...
let reviewed_internal t (diff4_ids : Diff4_in_session.Id.And_review_kind.t list) =
  List.iter diff4_ids ~f:(fun { diff4_id; review_kind } ->
    Reviewable_diff4.reviewed
      t.reviewable_diff4s.(diff4_id)
      ~frozen_review_kind:(Some review_kind))
;;
  ...
  | Ok () ->
    let already_reviewed_files, diff4_id_and_review_kinds =
      List.partition_map diff4_ids ~f:(fun diff4_id ->
        ...
        then First (Diff4.path_in_repo_at_f2 diff4_in_session.diff4)
        else (
          let review_kind = compute_review_kind diff4_in_session.diff4 in
          Second { Diff4_in_session.Id.And_review_kind.diff4_id; review_kind }))
    in
...
let num_lines_completed t =
  Array.sum
    (module Int)
    t.reviewable_diff4s
    ~f:(fun d -> Reviewable_diff4.num_lines_completed d t.reviewer)
;;
...
let deserializer =
  Deserializer.with_serializer (fun serializer ->
    ...
    and queries = sequence_of (module Persist.Action_query) ~in_file:queries_file in
