...
let problems t =
  ...
  let is_cr_clean node =
    let rev_facts : Rev_facts.t = get_node node in
    let rev = Rev_facts.rev rev_facts in
    Rev_facts.Is_cr_clean.check rev_facts.is_cr_clean rev, Problem.Is_not_cr_clean node
  in
  ...
  |> List.concat
  |> List.filter_map ~f:(fun (result, problem) ->
       if is_ok_true result then None else Some problem)
;;
...
let create diff2s ~base_facts ~tip_facts ~base_is_ancestor_of_tip =
  let t = { diff2s; base = base_facts; tip = tip_facts; base_is_ancestor_of_tip } in
  Problem.or_error t (problems t)
;;

let review_edge t = { Review_edge.base = base_rev t; tip = tip_rev t }
...
let by_diff2 (t : t) ~f =
  Hashtbl.map
    (By_diff2.of_alist_multi (List.map t.diff2s ~f:(fun a -> a, f a)))
    ~f:List.hd_exn
;;
