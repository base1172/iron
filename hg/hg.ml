...
module Stable = struct
  ...
  module Rev = struct
    module Human_readable = struct
      module V1 = struct
        module Unshared = struct
          type t = string option [@@deriving bin_io, compare, sexp, hash]
...
module Node_hash = struct
  ...
  module First_12 = struct
    ...
    let invariant (t : t) =
      Invariant.invariant [%here] t [%sexp_of: t] (fun () ->
        check_hash_exn (t :> string) ~length)
    ;;
  ...
  let to_first_12 (t : t) =
    String.sub (t :> string) ~pos:0 ~len:First_12.length |> First_12.shared_t
  ;;
...
module Tag = struct
  ...
  let%test_unit _ =
    List.iter
      ~f:(fun (t, for_tree_rooted_at, expect) ->
        if not
             (Bool.equal
                expect
                (is_nice_name
        ...
        then
          raise_s
            [%sexp "mismatch", { t : t; for_tree_rooted_at : string; expect : bool }])
...
let hg_with_optional_repo_root
  ...
  =
 fun ~how ?(accept_nonzero_exit = []) ?repo_root subcommand ~args ->
  if verbose
  then
    Verbose.message
      "hg"
  ...
  let args =
    ...
    then
      args
      @ [ "--config"
        ; "extensions.silence_lock_message=/j/office/app/hg/prod/jhg/silence_lock_message_v2.py"
        ]
    else args
  in
  ...
  match how with
  | Capture_stdout ->
    (Process.run ~accept_nonzero_exit ?working_dir ~prog:hg_executable ~args ~env ()
      : a Or_error.t Deferred.t)
  | Capture_output ->
    (match%bind Process.create ?working_dir ~prog:hg_executable ~args ~env () with
     ...
     | Ok process ->
       ...
       Ok output)
  | Create_process -> Process.create ?working_dir ~prog:hg_executable ~args ~env ()
  | Share_io ->
    let cwd =
...
module Rev = struct
  ...
  let invariant (t : t) =
    Invariant.invariant [%here] t sexp_of_t_round_trippable (fun () ->
      ...
      Unshared.Fields.iter ~human_readable:ignore ~node_hash:(check Node_hash.invariant))
  ;;
  ...
  let with_human_readable t ~human_readable =
    shared_t
      { Unshared.node_hash = node_hash t
      ; human_readable = Human_readable.some human_readable
      }
  ;;

  let without_human_readable t =
    if Human_readable.is_none (human_readable t) then t else of_node_hash (node_hash t)
  ;;
  ...
  module Compare_by_hash = struct
    module T = struct
      ...
      let hash_fold_t state t = Node_hash.hash_fold_t state (node_hash t)
    end
...
module Bookmark = struct
  ...
  let to_string = function
    ...
    | Release feature_path -> concat [ "[release]"; Feature_path.to_string feature_path ]
  ;;
end
...
module Revset = struct
  ...
  let to_string = function
    | String str ->
      str
      (* the toplevel of the revset is parsed differently, so let's
    ...
    | t ->
        ...
        :: snd
             (List.fold
                ...
                ~f:(fun (insert_op, ac) t ->
                  let ac = if insert_op then op :: ac else ac in
      ...
      and to_strings t ac =
        ...
        | First_greatest_common_ancestor revs ->
          "ancestor("
          :: (List.intersperse ~sep:", " (List.map revs ~f:Rev.to_string_40) @ (")" :: ac))
        | Limit (t, limit) ->
...
let diff repo_root ~from ~to_ ~args =
  ...
  in
  hg ~repo_root ~how:Capture_stdout "diff" ~args:("-r" :: Rev.to_string_40 from :: tail)
;;
...
let log ?(args = []) ?template repo_root revset =
  ...
  hg
    ...
    "log"
    ~how:Capture_stdout
    ~args:([ "-r"; Revset.to_string revset ] @ template @ args)
;;

let output_to_revs =
  Or_error.map ~f:(fun output ->
    String.split_lines output |> List.map ~f:Rev.of_string_40)
;;
...
let create_rev ?human_readable repo_root revset =
  ...
  | Ok [ rev ] ->
    let human_readable = Option.value human_readable ~default:(Revset.to_string revset) in
    Ok (Rev.with_human_readable rev ~human_readable)
  | Ok revs ->
    error
      "[Hg.create_rev] got a revset not of size one"
      (revset, revs)
...
let get_remote_rev remote_repo_path what_rev =
  ...
  let%map stdout =
    hg_with_optional_repo_root
      ...
      ~args:
        [ "-r"
        ; rev (* We tried using the "--full-id" switch, but that is a jane extension. *)
        ; "--id"
  ...
  | `Error e ->
    raise_s
      [%sexp
        (sprintf "cannot find rev %s of remote repo" rev : string)
        , (remote_repo_path : Remote_repo_path.t)
        , (e : Error.t)]
  | `Not_one_line lines ->
    raise_s
      [%sexp
        (sprintf "wrong output while finding rev %s of remote repo" rev : string)
        , (remote_repo_path : Remote_repo_path.t)
    ...
    | None ->
      let%map result =
        Async_interactive.Job.run
          !"Checking cleanliness of %{Repo_root#hum}"
          repo_root
...
let active_bookmark repo_root =
  ...
  | `Not_one_line lines ->
    error
      "unexpected multi-line output from 'hg active-bookmark'"
      lines
      [%sexp_of: string list]
;;

let current_bookmark repo_root =
  if verbose then Verbose.message "current_bookmark" repo_root [%sexp_of: Repo_root.t];
  let path = Repo_root.to_string repo_root ^ "/.hg/bookmarks.current" in
  match%map Monitor.try_with ~extract_exn:true (fun () -> Reader.file_contents path) with
...
let list_shelves_exn repo_root =
  match%map
    hg
      ...
      ~how:Capture_stdout
      "shelve"
      ~args:[ "--list"; "--config"; "extensions.shelve=" ]
  with
  | Ok stdout -> String.split_lines stdout
  | Error err ->
    raise_s
      [%sexp
        "cannot list the existing shelves", (repo_root : Repo_root.t), (err : Error.t)]
;;
...
let greatest_common_ancestor repo_root rev1 rev2 =
  ...
  | [] ->
    raise_s
      [%sexp
        "could not determine greatest common ancestors", ([ rev1; rev2 ] : Rev.t list)]
  | _ :: _ :: _ as gcas ->
    raise_s
      [%sexp
        "multiple greatest common ancestors"
...
let list_bookmarks repo_root =
  ...
  result |> ok_exn |> String.split_lines
;;
...
let files ?include_ repo_root =
  ...
  in
  output |> ok_exn |> String.split_lines |> List.map ~f:Path_in_repo.of_string
;;
...
let grep_conflicts_exn ?(below = Path_in_repo.root) repo_root ~grep_display_option =
  ...
  in
  let working_dir = Abspath.to_string (Repo_root.append repo_root below) in
  let%bind grep_process = Process.create ~working_dir ~prog:"xargs" ~args:xargs_args () in
...
let outgoing repo_root (`Feature feature_path) ~to_ =
  Async_interactive.Job.run
    !"Checking for outgoing changesets in %{Repo_root#hum}"
    repo_root
    ~f:(fun () ->
    match%map
      hg
      ...
      let pull revs to_string to_string_hum =
        Async_interactive.Job.run
          ...
          (match revs with
           ...
           | _ ->
             concat
               ("\n" :: List.map revs ~f:(fun r -> concat [ "  "; to_string_hum r; "\n" ])))
          ...
          ~f:(fun () ->
            hg
              ...
              ~how:Share_io
              ~args:
                (List.concat_map revs ~f:(fun r -> [ "-r"; to_string r ])
                @ [ "-q"
                    (* makes sure we create at most one divergent bookmark @default
                          and not @1, @2 etc. *)
                  ; "--config"
                  ; "paths.default=" ^ Remote_repo_path.to_string from
                  ]))
      in
...
let parent repo_root =
  let fail (type a) msg (a : a) sexp_of_a =
    raise_s [%sexp (msg : string), { repo_root : Repo_root.t; error = (a : a) }]
  in
  let%map result =
...
let with_temp_share ?in_dir repo_root ~f =
  Path.with_temp_dir ?in_dir (File_name.of_string "fe-share") ~f:(fun temp_dir ->
    ...
    let%bind result =
      hg ~repo_root "share" ~how:Share_io ~args:[ "-U"; "."; Abspath.to_string temp_dir ]
    in
...
let push repo_root bookmarks ~to_ ~overwrite_bookmark =
  ...
  then return (Ok ()) (* Don't need to push and pushing would lock up. *)
    ...
    let msg =
      ...
      | _ ->
        sprintf
          ...
          (Remote_repo_path.to_string to_)
          (concat
             (List.map bookmarks ~f:(fun bookmark ->
    ...
    let%bind output =
      Async_interactive.Job.run "%s" msg ~f:(fun () ->
        ...
        hg
          ...
          ~how:Capture_output
          ~args:
            (List.concat_map bookmarks ~f:(fun bookmark ->
               [ flag; Bookmark.to_string bookmark ])
            @ [ "-f"; Remote_repo_path.to_string to_ ]))
    in
    ...
    | Error (`Exit_non_zero 1) ->
      if is_some
           (String.Search_pattern.index
              (force push_no_changes_pattern)
              ~in_:output.stdout)
  ...
  | `One_line line ->
    (match line with
     ...
     | _ ->
       raise_s
         [%sexp
           (sprintf "unexpected output while finding phase of %s" rev : string)
           , (line : string)])
;;
  ...
  let set_bookmark_locally repo_root to_ =
    ...
    let%map result =
      hg
        ...
        "bookmark"
        ~how:Share_io
        ~args:[ "-f"; "-r"; rev; Bookmark.to_string bookmark ]
    in
    ok_exn result
  in
  match maybe_push with
  | `Do_not_push -> set_bookmark_locally repo_root to_
  | (`Push_to_and_overwrite remote_repo_path | `Push_to_and_advance remote_repo_path) as
    overwrite_or_advance ->
    let%bind rev =
      match to_ with
      | `Rev rev -> return rev
...
let update
  ...
  let rev_for_hg, rev_for_hum =
    ...
    | `Feature path ->
      let str = Feature_path.to_string path in
      str, str
    | `Rev rev -> Rev.to_string_40 rev, Rev.to_string_hum rev
    | `Revset revset ->
      let str = Revset.to_string revset in
      str, str
  in
  ...
  let%map update_result =
    Async_interactive.Job.run
      ...
      | No -> return (Ok ())
      | Yes repo_is_clean -> clean repo_root repo_is_clean)
  in
...
let has_spec_txt repo_root rev =
  match%map
    hg
      ...
      "locate"
      ~how:Capture_stdout
      ~args:[ "-r"; Rev.to_string_40 rev; "path:.projections/spec.txt" ]
  with
  | Ok (_ : string) -> true
  | Error _ -> false
;;
...
let rebase
  ...
  in
  let%bind () =
    remove_dot_projections_if_rebasing_across_switch_to_dot_fe
      repo_root
      ~old_tip
      ~new_base
  in
      ...
      ~args:
        ...
        | None ->
          ...
          ; sprintf
              ...
              (sprintf
                 "'new base: %s [%s]'"
                 (Feature_path.to_string (ok_exn (Feature_path.parent feature_path)))
        ...
        | Some { Merge_tool.executable; args } ->
          ...
          in
          config "executable" (Abspath.to_string executable) @ config "args" args)
  with
  | Error e ->
    ...
    abort_rebase
      ...
      "Merge failed"
      (error "aborted rebase because merge failed" e [%sexp_of: Error.t])
  | Ok () ->
    ...
    then return (Ok ())
      ...
      match%bind
        Monitor.try_with_or_error ~extract_exn:true (fun () ->
          is_conflict_free_exn repo_root)
      with
      | Ok true -> return (Ok ())
      | Ok false ->
        abort_rebase
          ~clean_after_update:(Yes repo_is_clean)
          (sprintf
             "Merge has conflicts and %s is provided"
      ...
      | Error err ->
        abort_rebase
          ~clean_after_update:(Yes repo_is_clean)
          (sprintf
             "Failed while grepping for conflicts and %s is provided"
             Switch.abort_on_merge_conflicts)
          (error
             "aborted rebase, error while grepping for conflicts"
             err
             [%sexp_of: Error.t])))
    >>=? fun () ->
       ...
       (match%bind Monitor.try_with_join_or_error ~extract_exn:true f with
        ...
        | Error err ->
          abort_rebase
            ...
            (error
               "aborted rebase because of post merge validation error"
               err
               [%sexp_of: Error.t])))
    >>=? fun () ->
    ...
    >>=? fun () -> clean repo_root repo_is_clean
;;
...
let rename ?repo_is_clean repo_root remote_repo_path (renames : Rename.t list) =
  ...
  let%bind () =
    push repo_root bookmarks_to_push ~to_:remote_repo_path ~overwrite_bookmark:true
...
module Status = struct
  ...
  let parse lines =
    let rec aux acc = function
      ...
      | ('A', dst) :: (' ', src) :: rest ->
        aux
          (Copied { src = Path_in_repo.of_string src; dst = Path_in_repo.of_string dst }
          :: acc)
          rest
    ...
    in
    try aux [] (List.map lines ~f:split_line) with
    | exn ->
      raise_s
        [%sexp
          "failed to parse output of hg status", (lines : string list), (exn : Exn.t)]
  ;;

  let src_path_in_repo diffs =
    (* A file can be appear up to N + 1 times, if it is modified and copied N times. *)
    List.dedup_and_sort
      ~compare:Path_in_repo.compare
...
let status repo_root (changed : Status.Changed.t) =
  let might_have_changes =
    match changed with
    | Changed_by _ | Between { dst = `Working_copy; _ } -> true
  ...
  then return []
    ...
    in
    let%map lines = hg ~repo_root "status" ~how:Capture_stdout ~args:("-marC" :: args) in
     ...
     | tags ->
       ...
       (match%bind unix_sort ~args:[ "-gr" ] (List.map tags ~f:Tag.to_string) with
        ...
        | tag_string :: _ ->
          ...
          (match%map create_rev repo_root (Revset.tag tag) ~human_readable:tag_string with
           | Error _ -> None
           | Ok tagged_rev ->
             if Rev.equal_node_hash rev tagged_rev then Some tagged_rev else None)))
;;
...
let cat_one_revset repo_root revset file =
  let args = [ "-r"; Revset.to_string revset; Path_in_repo.to_string file ] in
  hg ~repo_root "cat" ~how:Capture_stdout ~args
;;

let cat_one repo_root rev file = cat_one_revset repo_root (Revset.of_rev rev) file
...
let cat repo_root rev files ~dir =
  ...
  | _ :: _ ->
    ...
    in
    let args =
      [ "-o"; filename_format; "-r"; Rev.to_string_40 rev; "set: 'listfile:/dev/stdin'" ]
    in
...
module Scaffold = struct
  ...
  let shallow_scaffold_of_sexp =
    Sexp.of_sexp_allow_extra_fields_recursively shallow_scaffold_of_sexp
  ;;

  let resolve_revision repo ~revision ~scaffold_requires_global_tag_or_rev_hash =
    let extended_PATH =
      concat
        [ Filename.dirname hg_executable
    ...
    let%map revision =
      match%map
        Process.run
          ~env:(`Extend [ dont_load_any_hgrc, ""; "PATH", extended_PATH ])
          ~prog:"/j/office/app/hg/prod/bin/resolve-global-tag"
          ~args:[ Remote_repo_path.to_string repo; revision ]
          ()
      with
      | Error _ -> revision
  ...
  | Ok () ->
    let res =
      let%bind scaffold_data =
        Reader.load_sexp
          (Abspath.to_string abs_scaffold_sexp)
          [%of_sexp: Scaffold.shallow_scaffold]
        >>=? fun { repo; id } ->
        Scaffold.resolve_revision
          repo
          ~revision:id
          ~scaffold_requires_global_tag_or_rev_hash
        >>|? fun revision -> repo, revision
      in
      ...
      | Ok (repo, revision) ->
        ...
        in
        (match host_opt with
         | None -> cat_one_revset repo_root_from_scaffold (Revset.of_string revision) file
         | Some host ->
           ...
           Process.run
             ...
             ~args:
               ...
               ; "hg"
               ; "--cwd"
               ; Repo_root.to_string repo_root_from_scaffold
               ; "cat"
               ; "-r"
               ; revision
               ; "--"
...
let hg_command =
  Command.async'
    ...
    let%map_open () = return ()
    and args =
      map (flag "--" escape ~doc:" arguments to pass to hg") ~f:(Option.value ~default:[])
    in
    fun () ->
      let open! Deferred.Let_syntax in
      let%map env = env () in
      Unix.exec ~env ~prog:hg_executable ~argv:(hg_executable :: args) () |> never_returns)
;;
