...
module Cache = struct
  module Key = struct
    module T = struct
      type t = (File_contents_hash.t option Diamond.t[@sexp.opaque])
...
let as_from_scratch_to_diff2 { diamond; errors = _; num_lines_in_diff } =
  ...
  if Rev.equal_node_hash b1 f1 && Rev.equal_node_hash b1 b2
  then Some (Diff2.create ~base:diamond.b2 ~tip:diamond.f2 ~num_lines_in_diff)
  else None
;;

let diamond_is_rev_update { Diamond.b1; b2; f1; f2 } =
  Attributed_file.Ignoring_rev.equal b1 b2 && Attributed_file.Ignoring_rev.equal f1 f2
;;
...
let create_rev_update ~(from_ : Diff2.t) ~(to_ : Diff2.t) =
  ...
  else Ok { diamond; errors = []; num_lines_in_diff = 0 }
;;
...
let create
  ...
  | _ ->
    let%bind num_lines_in_diff =
      ...
      | `b1_b2_f1_f2 | `b1_f1__b2_f2 | `b1_b2__f1_f2 | `b2_f1_f2 -> return 0
      | _ ->
           ...
           let def =
             let%map { num_lines_in_diff4 } =
               Process_num_lines_in_diff4.compute
                 { files; lines_required_to_separate_ddiff_hunks }
             in
             num_lines_in_diff4
           in
...
let invariant t =
  Invariant.invariant [%here] t [%sexp_of: t] (fun () ->
    ...
    Fields.iter
      ...
      ~errors:ignore
      ~num_lines_in_diff:
        (check ([%test_pred: int] (fun num_lines_in_diff -> num_lines_in_diff >= 0)));
...
let may_review_internal t reviewer =
  ...
  let may_review =
    Diamond.map t.diamond ~f:(fun file ->
      ...
      | `Present attributes ->
        `Present
          (Attributed_file.Attributes.may_review
             attributes
             ~include_may_follow:false
             reviewer))
  in
...
let may_review t reviewer =
  ...
  | (`Dropped_from_review | `Review_ownership_change | `Review_lines) as review -> review
  | `Nothing_to_review ->
...
let num_lines ({ diamond; errors; num_lines_in_diff } as t) reviewer =
  ...
  then 1 + num_lines_in_diff
    ...
    | `Dropped_from_review | `Dropped_from_follow ->
      ...
      1
    | `Review_ownership_change | `Follow_lines | `Review_lines ->
      let raw_num_lines_in_diff =
        ...
        then
          ...
          +
          if File_scrutiny.( = ) (Attributed_file.scrutiny src) File_scrutiny.ignored
          then 0
      ...
      let review_client_wont_show_more_stuff =
        let equal_attributes_for_review attributes1 attributes2 =
          ...
          let followers f1 f2 =
            Bool.equal (Set.mem f1 reviewer.user_name) (Set.mem f2 reviewer.user_name)
          in
        ...
        let equal_attributed_file_for_review
          (at1 : Attributed_file.t)
          (at2 : Attributed_file.t)
          =
      ...
      let conservative_approach_waiting_for_proper_fix =
        if review_client_wont_show_more_stuff then 0 else 1
      in
...
let summary ts ~sort =
  let ts =
    ...
    then ts
      ...
      List.sort ts ~compare:(fun d1 d2 ->
