...
let and_ ts =
  ...
  | [] -> none
  | [ t ] -> t
  | _ -> And (List.sort conjuncts ~compare)
;;
...
let%test_module _ =
  (module struct
    ...
    let users_generator ?(at_least = 0) () =
      ...
      Int.gen_incl 1 5
      >>= fun len ->
      List.gen_with_length len Char.gen_lowercase
      >>| fun name ->
    ...
    let t_generator =
      ...
      Generator.fixed_point (fun t ->
        ...
        let or_wide =
          if size = 0
          then None
          else Some (map ~f:(fun ts -> Or_wide ts) (List.gen_non_empty t))
        in
        let and_ =
          if size = 0
          then None
          else Some (map ~f:(fun ts -> And ts) (List.gen_non_empty t))
        in
        filter_map
