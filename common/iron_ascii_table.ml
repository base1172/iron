...
module Attr = struct
  type color = Ansi_kernel.Color.primary
...
module Column = struct
  ...
  let string
    ...
    create_attr
      header
      (if show_zero then f else f_no_zero)
      ~show
      ?align
      ?min_width
      ~max_width:2000
...
let to_string ?force_unicode_bars (T t) ~display_ascii ~max_output_columns =
  ...
  then ""
    ...
    let bars =
      if display_ascii && Option.is_none force_unicode_bars then `Ascii else `Unicode
    in
    (if display_ascii then Ascii_table.to_string_noattr else Ascii_table.to_string)
      t.columns
      t.rows
      ~bars
      ~limit_width_to:max_output_columns)
;;
