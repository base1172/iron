...
let with_human_readable t name = { t with human_readable = Some name }
...
let not_in_repo sexp = error_s [%sexp "not inside an hg repo", (sexp : Sexp.t)]
...
let containing_root_blocking =
  ...
  fun ?human_readable start ->
    let rec loop path =
      match
        Core_unix.access (Abspath.to_string (Abspath.extend path dot_hg)) [ `Exists ]
      with
...
let program_started_in =
  ...
  | Ok started_in -> containing_root_blocking ~human_readable:"local repo" started_in
;;
...
let append t path_in_repo = Abspath.append t.path (Path_in_repo.to_relpath path_in_repo)

let relativize_exn t abspath =
  ok_exn (Abspath.chop_prefix ~prefix:(to_abspath t) abspath) |> Path_in_repo.of_relpath
;;
