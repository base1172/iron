...
module Map = struct
  ...
  include Map.Provide_bin_io (T)
end

let invariant t = Relpath.invariant (unshared_t t)
let root = shared_t Relpath.empty
let chop_prefix ~prefix t = Relpath.chop_prefix ~prefix:(unshared_t prefix) (unshared_t t)
...
let low_review_file build_projection_name =
  extend
    ...
    (File_name.of_string
       (concat
          [ "low-review-in-"; Build_projection_name.to_string build_projection_name ]))
;;
