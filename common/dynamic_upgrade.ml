...
let%test_unit _ =
  List.iter all ~f:(fun t -> [%test_result: t] (of_int (to_int t)) ~expect:t)
;;
...
module State = struct
  ...
  let deserializer =
    Deserializer.with_serializer (fun serializer ->
      ...
      let%map_open values = one_opt (module Persist.State) ~in_file:Persist.state_file in
      match values with
      | Some { committed_to; current_value } ->
        { committed_to; current_value; serializer }
      | None -> { committed_to = U1; current_value = U1; serializer })
  ;;

  let persist_internal { committed_to; current_value; serializer } =
    Serializer.set_contents
      serializer
      { Stable.State.Model.committed_to; current_value }
      (module Persist.State)
      ~file:Persist.state_file
  ;;

  let dump { committed_to; current_value; serializer = _ } =
    [%sexp { committed_to : T.t; current_value : T.t }]
  ;;
end
...
let set_exn (t : State.t) trying_to_set_as =
  ...
  then (
    ...
    raise_s
      [%sexp
        "Cannot lower the upgrade state to that value anymore, associated functionality \
         was already used"
        , (State.dump t : Sexp.t)
