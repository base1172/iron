...
module T = struct
  ...
  [@@deriving hash, sexp, enumerate, compare]
...
let%test_unit _ =
  let node b =
    b |> [%sexp_of: [ `b1 | `b2 | `f1 | `f2 ]] |> Sexp.to_string |> String.uppercase
  in
  let group group =
    if Int.( <= ) (List.length group) 1
    then None
...
module Shown_class = struct
  ...
  module T = struct
    ...
    [@@deriving hash, sexp, compare, enumerate]
  end
  ...
  let of_class = function
    | `b1_b2_f1_f2 | `b1_b2__f1_f2 | `b1_f1__b2_f2 | `b2_f1_f2 -> None
    | ( `b1_b2_f1
      | `b1_b2_f2
      | `b1_b2
      | `b1_f1_f2
...
let classify ~equal:eq ~b1 ~b2 ~f1 ~f2 =
  ...
  then
    if eq b2 f1
    then if eq f1 f2 then `b1_b2_f1_f2 else `b1_b2_f1
    else if eq b2 f2
    then `b1_b2_f2
    else if eq f1 f2
    then `b1_b2__f1_f2
  ...
  then if eq f1 f2 then `b1_f1_f2 else if eq b2 f2 then `b1_f1__b2_f2 else `b1_f1
  else if eq b1 f2
  then if eq b2 f1 then `b1_f2__b2_f1 else `b1_f2
  else if eq b2 f1
  then if eq f1 f2 then `b2_f1_f2 else `b2_f1
  else if eq b2 f2
  then `b2_f2
  else if eq f1 f2
  then `f1_f2
