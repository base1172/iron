module Scope = struct
  ...
  module Stable = struct
    ...
    module V1 = struct
      type t = string Key.Stable.V1.Map.t [@@deriving sexp, bin_io, compare, hash]
    ...
    module Authorization_rule = struct
      module V1 = struct
        type t =
          ...
          ; scope_restrictions :
              Iron_string.Stable.V1.Set.t Scope.Key.Stable.V1.Map.t option
               [@sexp.option]
          }
    ...
    module V1 = struct
      ...
      let t_of_sexp sexp =
        ...
        List.iter t.authorization_rules ~f:(fun rule ->
          Option.iter
            rule.Authorization_rule.V1.scope_restrictions
            ~f:(fun scope_restrictions ->
            let invalid_scope_keys =
              Set.diff (Scope.Key.Set.of_list (Map.keys scope_restrictions)) t.scope_keys
            in
            if not (Set.is_empty invalid_scope_keys)
            then
              raise
                (Of_sexp_error
                   ( Error.to_exn
    ...
    module V1 = struct
      type t =
        ...
        * [ `add_spec of Spec.Stable.V1.t
          | `remove_spec
          | `add_fact of
            Scope.Stable.V1.t * User_name.Stable.V1.t * [ `comment of string ]
          | `remove_fact of Scope.Stable.V1.t
          ]
      [@@deriving sexp, bin_io, compare]
...
module Db = struct
  ...
  let invariant (_ : t) = ()
  let create () = { specs = Spec.Id.Table.create (); evidence = Spec.Id.Table.create () }

  let spec t spec_id =
    Result.of_option
      (Hashtbl.find t.specs spec_id)
  ...
  let evidence t spec_id scope =
    ...
    Result.of_option
      (Hashtbl.find t.evidence spec_id)
      ~error:(Error.create "no evidence for spec id" spec_id Spec.Id.sexp_of_t)
    >>= fun evidence_by_scope ->
    Result.of_option
      (Hashtbl.find evidence_by_scope scope)
      ~error:
        (Error.create_s
           [%sexp "no evidence for scope", { scope : Scope.t; spec_id : Spec.Id.t }])
  ;;
  ...
  let add_fact t spec_id scope ~asserter ~comment ~assertion_time =
    ...
    then
      Or_error.error
        "unexpected scope keys"
        (spec_id, `Expected expected_scope_keys, `Got provided_scope_keys)
        [%sexp_of:
          Spec.Id.t * [ `Expected of Scope.Key.Set.t ] * [ `Got of Scope.Key.Set.t ]]
    else if not (Spec.is_asserter spec scope asserter)
    then
      Or_error.error
        "user not authorized for assertion"
        (spec_id, asserter)
  ...
  let remove_fact t spec_id scope =
    ...
    Result.of_option
      ...
      ~error:
        (Error.create_s
           [%sexp "no evidence for scope", { scope : Scope.t; spec_id : Spec.Id.t }])
    >>= fun _ -> Ok (Hashtbl.remove evidence_by_scope scope)
  ;;
  ...
  let handle_action t (spec_id, which) =
    match which with
    | `add_spec spec -> add_spec t spec_id spec
    | `remove_spec -> remove_spec t spec_id
    | `add_fact (scope, asserter, `comment comment, assertion_time) ->
      add_fact t spec_id scope ~asserter ~comment ~assertion_time
    | `remove_fact scope -> remove_fact t spec_id scope
  ;;
