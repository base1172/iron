...
let to_string_hum str =
  ...
  then "<no error message>"
    ...
    String.concat
      [ Char.to_string (Char.uppercase str.[0])
      ; String.drop_prefix str 1
      ; (if Char.equal str.[String.length str - 1] '.' then "" else ".")
      ]
;;
...
let async' ~summary ?readme param = Command.async ~summary ?readme (in_async param)
