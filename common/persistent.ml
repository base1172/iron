...
module Reader = struct
  ...
  let map (type a b) (module Sa : S_reader with type t = a) ~(f : a -> b) =
...
module Writer = struct
  ...
  let prepend (type a b) (module Sa : S_writer with type t = a) ~(f : b -> a) =
    let module Sb = struct
      type t = b
...
module Make_with_context (Context : Context) (Version : Version) (Model : Model) = struct
  let of_sexp_default_fct sexp (_ : Context.t) = Model.t_of_sexp sexp
  let of_sexps = ref [ Version.version, of_sexp_default_fct ]
  let find_version version = List.Assoc.find !of_sexps ~equal:Int.equal version
      ...
      let sexp_of_t (context, model) =
        let version, sexp_of_model =
          ...
          | Some (write_old_version, override) ->
            if write_old_version context then override else to_sexp_default
        in
        { version; value = sexp_of_model context model } |> [%sexp_of: Versioned.t]
      ;;
    end
  end
    ...
    module Persist = struct
      ...
      let t_of_sexp sexp context =
        ...
        | None ->
          of_sexp_error_exn
            (Error.to_exn
               (Error.create "could not parse versioned sexp" sexp [%sexp_of: Sexp.t]))
            sexp
      ;;
    end
  end
  ...
  struct
    ...
    let () =
      ...
      then
        raise_s
          [%sexp
            "Register_read_write_old_version called more than once"
            , (Version.version : int)];
      let sexp_of_model context model = Conv.of_model context model |> Conv.sexp_of_t in
      to_sexp_override
...
module Make (Version : Version) (Model : Model) = struct
  ...
  struct
    include
      With_context.Register_read_old_version
        (Version)
        (struct
