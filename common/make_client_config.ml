...
module Make (X : Config) = struct
  let load_sexps_conv_or_errors file f =
    ...
    Sexp_macro.Blocking.load_sexps_conv file f
  ;;
  ...
  let blocking_load_files files =
    ...
    List.iter files ~f:(fun file ->
      Or_error.try_with (fun () ->
        ...
        if Sys_unix.file_exists_exn file
        then (
      ...
      | Error error ->
        add_error
          (Error.tag_arg
             error
             "uncaught exception while loading file"
             file
             [%sexp_of: Abspath.t]));
    x, List.rev !errors
  ;;
  ...
  let default_files_doc =
    lazy
      (List.map ~f:Abspath.to_string X.always_loaded_if_present
      @ [ "$HOME/" ^ X.home_basename ])
  ;;

  let blocking_load_and_memoize_exn = lazy (blocking_load_files (force default_files))

  let%test_unit _ =
    (* We depend on [load_sexps_conv_or_errors] to catch exceptions and returns location,
       so we better make sure it stays that way or better, gets cleaned up *)
    let open Core in
    let file = Filename_unix.temp_file "make_client_config" "unit_test" in
    ...
    (try Core_unix.unlink file with
     | _ -> ());
    match res with
    (* | [ `Error (A.E,_) ] -> () *)
    | [ `Error (_, _) ] -> ()
  ...
  let get () = fst (Lazy.force blocking_load_and_memoize_exn)
  let errors () = snd (Lazy.force blocking_load_and_memoize_exn)
      ...
      fun () ->
        ...
        let%bind files =
          ...
          then return (force default_files)
            ...
            in
            files |> Or_error.combine_errors |> ok_exn)
        in
        let errors = snd (blocking_load_files files) in
        List.iter errors ~f:(fun exn -> eprintf "%s\n" (Error.to_string_hum exn));
  ...
  let flag flag param_of_sexp f sexps =
    let rec filter rev = function
      ...
      | Sexp.Atom str :: arg :: tl when String.equal flag str ->
        let value =
          try param_of_sexp arg with
          | exn ->
            raise_s
              [%sexp
                "exception while parsing sexp"
                , { flag : string; sexp = (arg : Sexp.t); exn : Exn.t }]
        in
        f value;
