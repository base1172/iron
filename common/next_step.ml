...
let rec to_string_hum = function
  ...
  | Review -> "review"
  | Unlock lock_name -> concat [ "unlock "; Lock_name.to_string_hum lock_name ]
...
let should_be_red = function
  | Fix_build | Fix_problems | Report_iron_bug | Restore_base | Restore_bookmark -> true
  | Add_code
  | Add_whole_feature_reviewer
  | Archive
  | Ask_seconder
...
module Assigned = struct
  ...
  let rec to_string_hum t =
    ...
    | Wait_for_continuous_release
    | Wait_for_hydra
    | Widen_reviewing -> t_to_string_hum t
  ;;
