...
module Pauser : sig
  ...
  type t = { mutable state : State.t } [@@deriving fields, sexp_of]
  ...
  let pause_exn t ~query:by ~with_timeout =
    ...
    | Paused { by; _ } ->
      raise_s
        [%sexp "serializer is already paused", By_previous_query (by : unit Query.t)]
...
let path_to_string t path = Abspath.to_string (Abspath.append (dir t) path)
...
let pause_exn t ~query ~with_timeout =
  ...
  | `No ->
    ...
    Deferred.create (fun paused ->
      don't_wait_for
        (Throttle.enqueue t.shared.todo (fun () ->
           Ivar.fill paused ();
           resume)))
;;
...
let create ?(should_do_effects = true) ~root_directory () =
  let shared =
    { Shared.should_do_effects
    ; root_directory
    ; todo = create_todo ()
    ; pauser = Pauser.create ()
    }
  in
  let t = { shared; relative_to_root = Relpath.empty; cache_invalidators = [] } in
  if should_do_effects
  then
    Shutdown.at_shutdown (fun () ->
      (try resume_exn t with
       | _ -> ());
...
let enqueue t work =
  ...
  if t.shared.should_do_effects then don't_wait_for (Throttle.enqueue t.shared.todo work)
;;

let mkdir t path = Unix.mkdir ~p:() (path_to_string t path)
...
let add_subtree t ~dir =
  enqueue t (fun () ->
    if verbose then Debug.ams [%here] "add_subtree" (t, dir) [%sexp_of: t * Relpath.t];
    mkdir t dir)
;;
...
module Compress_or_uncompress = struct
  ...
  let source_and_destination operation t dir =
    ...
    | Compress -> { source = uncompressed_dir; destination = compressed_dir }
    | Uncompress -> { source = compressed_dir; destination = uncompressed_dir }
  ;;
  ...
  let process operation ~source ~destination =
    ...
    | Compress ->
      ...
      Process.run
        ...
        ~env:(`Extend [ "XZ_OPT", "-1" ])
        ~prog:"/bin/tar"
        ~args:[ "-cJf"; Filename.basename destination; Filename.basename source ]
        ()
    | Uncompress ->
      Process.run
        ~working_dir:(Filename.dirname source)
...
let idempotent_compress_or_uncompress operation t ~dir =
  ...
  enqueue t (fun () ->
    if verbose
    then Debug.eprint_s [%sexp (operation_name : string), [%here], { dir : Relpath.t }];
    ...
    | `To_be_created ->
      ...
      (match%bind Compress_or_uncompress.process operation ~source ~destination with
       ...
       | Ok (_stdout : string) -> remove_subtree_if_present_internal_exn source))
;;
