...
let map t ~f =
  ...
  | Partially_known { to_finish_session; from_session_end_to_goal } ->
    Partially_known { to_finish_session = f to_finish_session; from_session_end_to_goal }
;;
...
let fully_known_exn = function
  ...
  | Partially_known { from_session_end_to_goal; _ } ->
    let err =
      try Or_pending.known_exn from_session_end_to_goal with
      | e -> Error.of_exn e
    in
