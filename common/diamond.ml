module Stable = struct
  ...
  module V1 = struct
    ...
    let map t ~f =
      ...
      Fields.map ~b1:f ~b2:f ~f1:f ~f2:f
    ;;
  end
end
...
include Applicative.Make (struct
  ...
  let apply f y = { b1 = f.b1 y.b1; b2 = f.b2 y.b2; f1 = f.f1 y.f1; f2 = f.f2 y.f2 }
  let map = `Custom map
end)
...
include Container.Make (struct
  ...
  let fold t ~init ~f =
    ...
    Fields.fold ~init ~b1:f ~b2:f ~f1:f ~f2:f
  ;;
  ...
  let iter =
    `Custom
      (fun t ~f ->
        ...
        Fields.iter ~b1:f ~b2:f ~f1:f ~f2:f)
  ;;

  let length =
    let f a _fld = a + 1 in
    `Custom (fun (_ : 'a t) -> Fields.fold ~init:0 ~b1:f ~b2:f ~f1:f ~f2:f)
  ;;
end)
...
let map2 a b ~f =
  { b1 = f a.b1 b.b1; b2 = f a.b2 b.b2; f1 = f a.f1 b.f1; f2 = f a.f2 b.f2 }
;;
...
let for_all t ~f = f t.b1 && f t.b2 && f t.f1 && f t.f2
let for_all2 a b ~f = f a.b1 b.b1 && f a.b2 b.b2 && f a.f1 b.f1 && f a.f2 b.f2
let old_base = b1
let new_base = b2
let old_tip = f1
let new_tip = f2
let to_list { b1; b2; f1; f2 } = [ b1; b2; f1; f2 ]
let edges { b1; b2; f1; f2 } = [ b1, b2; b1, f1; b2, f2; f1, f2 ]
let of_one_edge a b = { b1 = a; b2 = a; f1 = a; f2 = b }
...
let%test _ =
  List.for_all Node.all ~f:(fun (dispatch : Node.t) ->
    Node.equal (get self dispatch) dispatch)
;;
...
module Deferred = struct
  ...
  let map ?(how = `Sequential) { b1; b2; f1; f2 } ~f =
    ...
    | `Sequential ->
      f b1
      >>= fun b1 ->
      f b2 >>= fun b2 -> f f1 >>= fun f1 -> f f2 >>= fun f2 -> return { b1; b2; f1; f2 }
    | `Parallel ->
      Deferred.both (Deferred.both (f b1) (f b2)) (Deferred.both (f f1) (f f2))
      >>| fun ((b1, b2), (f1, f2)) -> { b1; b2; f1; f2 }
  ;;

  let map2 ?how t1 t2 ~f = map ?how (both t1 t2) ~f:(fun (x, y) -> f x y)

  let all { b1; b2; f1; f2 } =
    b1 >>= fun b1 -> b2 >>= fun b2 -> f1 >>= fun f1 -> f2 >>| fun f2 -> { b1; b2; f1; f2 }
  ;;
end
...
let param xflag ~doc =
  ...
  and f2 = flag "-new-tip" xflag ~doc:(doc "new-tip") ~aliases:[ "-f2"; "new-feature" ] in
  { b1; f1; b2; f2 }
;;
...
let group diamond ~by:diff4_class =
  ...
  List.iter groups ~f:(fun group ->
    ...
    List.iter group ~f:(fun node -> get' result node := contents));
...
let%test_unit _ =
  List.iter Diff4_class.all ~f:(fun diff4_class ->
    ...
    then raise_s [%sexp "unvisited node", (diff4_class : Diff4_class.t)])
;;

let classify ~equal { b1; b2; f1; f2 } = Diff4_class.classify ~equal ~b1 ~b2 ~f1 ~f2
...
let pretty_short_description ~label { b1; b2; f1; f2 } =
  ...
  then
    ...
    then (* All are equal *)
      ...
      [ label "old base", b1; label "old tip", f1 ]
  else (
    match same_bs, same_fs with
    | true, true ->
      if String.equal b1 f1
      then [ label "", b1 ]
      else [ label "base", b1; label "tip", f1 ]
    | true, false ->
      if String.equal b1 f1
      then [ label "base", b1; label "tip", f2 ]
      else [ label "base", b1; label "old tip", f1; label "new tip", f2 ]
    | false, true ->
      [ label "old base", b1; label "new base", b2; label "old & new tip", f2 ]
    | false, false ->
      [ label "old base", b1
      ; label "old tip", f1
      ; label "new base", b2
...
let pretty_short_rev_names ~equal { b1; b2; f1; f2 } =
  ...
  then
    ...
    then (* All are equal *)
      ...
      (* This is a forget *)
      { b1 = "old base"; b2 = "old tip"; f1 = "old tip"; f2 = "old tip" }
  else
    { b1 = (if same_bs then "base" else "old base")
    ; b2 = (if same_bs then "base" else "new base")
    ; f1 =
        (if same_bs && equal b1 f1 then "base" else if same_fs then "tip" else "old tip")
    ; f2 =
        (if same_bs && equal b1 f1 then "tip" else if same_fs then "tip" else "new tip")
    }
;;

let pretty_short_rev_names_const =
  { b1 = "old base"; b2 = "new base"; f1 = "old tip"; f2 = "new tip" }
;;
