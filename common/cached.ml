...
module Invalidator = struct
  ...
  let invalidate_dependents t =
    ...
    Option.iter t.callback ~f:(fun f ->
      try f () with
      | _ when not am_functional_testing -> ())
  ;;

  let set_callback t ~callback_no_exn = t.callback <- Some callback_no_exn

  let create ~debug_information =
    { last_changed_at = Virtual_time.tick (); callback = None; debug_information }
  ;;
end
...
module Cached_result = struct
  ...
  let needs_recomputation t ~depends_on =
    (not (List.equal Invalidator.equal depends_on t.depends_on))
    || List.exists depends_on ~f:(fun invalidator ->
         Virtual_time.( <= )
           t.computation_initiated_at
           (Invalidator.last_changed_at invalidator))
  ;;
end
...
let set t cached_result = t.cached_result <- Some cached_result
...
let compute_cached_result (type a) (t : a t) ~depends_on =
  ...
  let cached_result : a Cached_result.t =
    { computation_initiated_at; computation_finished_at; depends_on; result }
  in
  ...
  then (
    ...
    { cached_result with
      result =
        Or_error.error
          "Cached.compute_result modified one of its invalidators"
          sexp
          [%sexp_of: Sexp.t]
    })
  else cached_result
    ...
    then Ok ()
      ...
      error_s
        [%sexp
          ...
          , { was_cached_as : a Or_error.t; set_cache_to : a Or_error.t }])
;;

let force_set_for_test_exn t result =
  if not am_functional_testing
  then failwith "Cached.force_set_for_test_exn may be called during functional tests only";
