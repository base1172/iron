...
module Temporary_message = struct
  ...
  let create ~expires_in message =
    { message = String.rstrip message; expires_at = Time.add (Time.now ()) expires_in }
  ;;
end

type t =
  { message : string
  ; temporary_message : Temporary_message.t option [@sexp.option]
  }
[@@deriving sexp]

let create message = { message = String.rstrip message; temporary_message = None }
...
let save_exn t ~perm file = Writer.save_sexp ~perm (Abspath.to_string file) (sexp_of_t t)

let load_exn file =
  Reader.load_sexp_exn
    (Abspath.to_string file)
    (Sexp.of_sexp_allow_extra_fields_recursively t_of_sexp)
;;
