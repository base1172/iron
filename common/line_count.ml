module Stable = struct
  ...
  module V4 = struct
    ...
    let of_v5
      ...
      let have_uncommitted_and_potentially_blocking_session =
        have_potentially_blocking_review_session_in_progress
      in
      { review; catch_up; completed; have_uncommitted_and_potentially_blocking_session }
    ;;
  end
  ...
  module V3 = struct
    ...
    let of_v4
      { V4.review = { must_review; follow; may_review; ownership_changes }
      ; catch_up
      ; completed
      ...
      let review : Review_or_commit.V1.t =
        ...
        if review = 0
        then if have_uncommitted_and_potentially_blocking_session then Commit else Num 0
        else Num (review + may_review)
      in
      let catch_up =
        let { Catch_up.V1.create_catch_up_for_me
            ; follower
            ; unfinished_review
          ...
          catch_up
        in
        create_catch_up_for_me + follower + unfinished_review + reviewed_by_someone_else
      in
      { review; follow; catch_up; completed }
    ;;
  end
  ...
  module V2 = struct
    ...
    let of_v3 { V3.review; follow; catch_up; completed } =
      { review =
          (match review with
           | Commit -> 1
           | Num n -> n)
...
module Review = struct
  ...
  let zero = { must_review = 0; follow = 0; may_review = 0; ownership_changes = 0 }

  let total t =
    let f acc field = acc + Field.get field t in
    Fields.fold ~init:0 ~must_review:f ~follow:f ~may_review:f ~ownership_changes:f
  ;;
  ...
  let ( + ) t1 t2 =
    ...
    Fields.map ~must_review:f ~follow:f ~may_review:f ~ownership_changes:f
  ;;

  let to_review_column_shown
    (review : t)
    ~have_potentially_blocking_review_session_in_progress
    =
...
let _must_review_or_commit t =
  ...
  if must_review = 0 && t.have_potentially_blocking_review_session_in_progress
  then Review_or_commit.Commit
  else Num must_review
;;

let to_review_column_shown t =
  Review.to_review_column_shown
    t.review
    ~have_potentially_blocking_review_session_in_progress:
...
let is_shown
  { review; catch_up; completed; have_potentially_blocking_review_session_in_progress }
...
module Increasing_review = struct
  let compare =
    ...
    then
      ...
      ; (fun t -> if t.have_potentially_blocking_review_session_in_progress then 1 else 0)
...
let sort_decreasing_review us =
  List.sort us ~compare:(fun (u1, c1) (u2, c2) ->
