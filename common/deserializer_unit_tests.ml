...
module M :
  ...
  let%test_unit _ =
    (* [sequence] of a nonexistent file gives an empty sequence *)
    Thread_safe.block_on_async_exn (fun () ->
      Path.with_temp_dir temp_path ~f:(fun root_directory ->
        Deferred.map
          (load
             (sequence_of (module Int) ~in_file:(Relpath.of_string "not-present"))
             ~root_directory
    ...
    Thread_safe.block_on_async_exn (fun () ->
      Path.with_temp_dir temp_path ~f:(fun root_directory ->
        ...
        Serializer.set_contents
          (Serializer.relativize serializer ~dir:file_name4)
          ~file:file_name5
          int2
          (module Int);
        ...
        for i = 1 to num_children do
          let serializer =
            Serializer.relativize
              serializer
              ~dir:(Relpath.append file_name6 (Relpath.of_string (Int.to_string i)))
          in
        ...
        [%test_result: (File_name.t * int) list]
          (List.sort (Map.to_alist map) ~compare:(fun (_, i1) (_, i2) ->
             Int.compare i1 i2))
          ~expect:
