...
module T = struct
  ...
  type _ t =
    ...
    | Upgrade_one :
        'a Persistent.Reader.t
        * Relpath.t
        * 'b Persistent.Reader.t
        * Relpath.t
        * ('a -> 'b)
        -> 'b t
  ...
  let load t ~root_directory ~serializer =
    ...
    let rec load
      : type a. a t -> root_directory:Abspath.t -> serializer:Serializer.t -> a Deferred.t
      =
      ...
      | All_subdirs t ->
        ...
        Deferred.List.map children ~how:`Parallel ~f:(fun child_name ->
          ...
          | `File ->
            raise_s
              [%sexp
                "expected a directory but got a file", (child : Abspath.t), (t : _ t)]
          | `Char | `Block | `Link | `Fifo | `Socket ->
            raise_s
              [%sexp
                "unexpected file kind"
                , (child : Abspath.t)
                , (stat.kind : Unix.File_kind.t)
                , (t : _ t)])
    in
  ...
  include (
    Monad.Make (struct
      ...
      Monad.S_without_syntax with type 'a t := 'a t)
end
