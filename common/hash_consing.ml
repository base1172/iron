...
let stats t = detailed_stats t |> [%sexp_of: (Module_name.t * Set_stats.t) list]
...
module Make (X : Unshared) () = struct
  ...
  module Hash_data = struct
    ...
    let create ~table_length value =
      ...
      { value; hash; hash_mod_length = hash mod table_length }
    ;;
...
let container_hash ~length ~fold ~fold_hash container =
  ...
  let index = ref (-1) in
  fold container ~init:(Int.hash length) ~f:(fun acc elt ->
    incr index;
    if filter !index then fold_hash acc elt else acc)
;;
